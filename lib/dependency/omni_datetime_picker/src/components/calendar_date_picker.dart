...
const double _maxDayPickerHeight = 262;

const int _yearPickerColumnCount = 3;
const double _yearPickerRowHeight = 52.0;
const double _yearPickerRowSpacing = 0.0;

const double _subHeaderHeight = 38.0;
...
class _CalendarDatePickerState extends State<CalendarDatePicker> {
  ...
  void didChangeDependencies() {
    ...
    if (!_announcedInitialDate) {
      ...
      SemanticsService.announce(_localizations.formatFullDate(_selectedDate), _textDirection);
    }
  }
  ...
  Widget _buildPicker() {
    switch (_mode) {
      ...
      case DatePickerMode.year:
        return Padding(
          padding: const EdgeInsets.only(top: _subHeaderHeight - 8),
          child: YearPicker(
            key: ValueKey(_currentDisplayedMonthDate),
            currentDate: widget.currentDate,
            firstDate: widget.firstDate,
            lastDate: widget.lastDate,
...
class _DatePickerModeToggleButtonState extends State<_DatePickerModeToggleButton> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: EdgeInsets.only(left: 16, right: 4, top: 11),
      height: _subHeaderHeight + 8,
      child: Row(
        children: <Widget>[
          Flexible(
                  ...
                  children: <Widget>[
                    Flexible(
                      child: Text(
                        ...
                        style: textTheme.titleSmall?.copyWith(color: context.outlineVariant, fontWeight: FontWeight.normal).appFont(context),
                      ),
                    ),
                    SizedBox(width: 8),
                    RotationTransition(
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  @override
  Widget build(BuildContext context) {
    // final Color controlColor = context.onSurface.withValues(alpha:0.6);
    ...
    return Semantics(
      child: Column(
        ...
        children: <Widget>[
          Container(
            margin: const EdgeInsets.only(left: 16, right: 16, top: 11),
            height: _subHeaderHeight - 3,
            child: Row(
              children: <Widget>[
                const Spacer(),
                VisirButton(
...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle? headerStyle = textTheme.labelMedium?.textColor(context.inverseSurface).appFont(context);
    // final TextStyle dayStyle = textTheme.bodySmall!;
    final Color enabledDayColor = colorScheme.onSurface.withValues(alpha: 0.87);
    // final Color disabledDayColor = colorScheme.onSurface.withValues(alpha:0.38);
    // final Color selectedDayColor = colorScheme.onPrimary;
    // final Color selectedDayBackground = colorScheme.primary;
    // final Color todayColor = colorScheme.secondary;
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        if (isSelectedDay) {
          ...
          dayColor = context.onSecondary;
          decoration = BoxDecoration(color: context.secondary, shape: BoxShape.rectangle, borderRadius: BorderRadius.circular(4));
        } else if (isToday) {
          // The current day gets a different text color (if enabled) and a circle stroke
          // border.
          if (!isDisabled) {
            dayColor = context.isDarkMode ? context.onPrimary : enabledDayColor;
          }
        ...
        Widget dayWidget = Center(
          child: Container(
            ...
            child: Center(
              child: Text(localizations.formatDecimal(day), style: Theme.of(context).textTheme.labelLarge?.apply(color: dayColor).appFont(context)),
            ),
          ),
        );

        if (!isDisabled) {
          dayWidget = VisirButton(
            type: VisirButtonAnimationType.scaleAndOpacity,
            focusNode: _dayFocusNodes[day - 1],
            onTap: () => widget.onChanged(dayToBuild),
            style: VisirButtonStyle(borderRadius: BorderRadius.circular(4), margin: EdgeInsets.symmetric(horizontal: 1)),
            child: dayWidget,
          );
        }
...
class _YearPickerState extends State<YearPicker> {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    const double decorationWidth = 56.0;

    final Color textColor;
    if (isSelected) {
      textColor = colorScheme.onPrimary;
    } else if (isDisabled) {
      textColor = colorScheme.outlineVariant.withValues(alpha: 0.38);
    } else if (isCurrentYear) {
      textColor = colorScheme.outlineVariant;
    } else {
      textColor = colorScheme.outlineVariant.withValues(alpha: 0.87);
    }
    ...
    return VisirButton(
      ...
      style: VisirButtonStyle(height: decorationHeight, width: decorationWidth, borderRadius: BorderRadius.circular(4)),
      ...
      child: Text(year.toString(), style: itemStyle?.appFont(context)),
    );
  }
...
class _YearPickerGridDelegate extends SliverGridDelegate {
  ...
  @override
  SliverGridLayout getLayout(SliverConstraints constraints) {
    final double tileWidth = 88;
    return SliverGridRegularTileLayout(
      childCrossAxisExtent: 89,
