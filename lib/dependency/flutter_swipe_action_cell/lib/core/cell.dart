...
class SwipeActionCellState extends State<SwipeActionCell> with TickerProviderStateMixin {
  ...
  void _onHorizontalDragUpdate(DragUpdateDetails details) {
    ...
    final bool leadingActionCanFullSwipe = whenLeadingActionShowing && leadingActionsCount > 0 && widget.leadingActions![0].performsFirstActionWithFullSwipe;
  ...
  Future<void> closeWithAnim() async {
    ...
    _resetAnimValue();
    if (mounted) {
      animation = Tween<double>(begin: currentOffset.dx, end: 0.0).animate(closeCurvedAnim)
        ..addListener(() {
          if (lockAnim) return;
          this.currentOffset = Offset(animation.value, 0);
          setState(() {});
    ...
    return {
      ...
      if (widget.isDraggable)
        _DirectionDependentDragGestureRecognizer:
            GestureRecognizerFactoryWithHandlers<_DirectionDependentDragGestureRecognizer>(() => _DirectionDependentDragGestureRecognizer(), (instance) {
          instance
  ...
  Widget build(BuildContext context) {
    ...
    final Widget selectedButton =
        widget.controller != null && (widget.controller!.isEditing.value || editController.isAnimating) ? _buildSelectedButton(selected) : const SizedBox();
  ...
  Widget _buildTrailingActionButtons() {
    ...
    final List<Widget> actionButtons = List.generate(trailingActionsCount, (index) {
      ...
      if (trailingActionsCount == 1 && !widget.trailingActions![0].forceAlignmentToBoundary && widget.trailingActions![0].performsFirstActionWithFullSwipe) {
