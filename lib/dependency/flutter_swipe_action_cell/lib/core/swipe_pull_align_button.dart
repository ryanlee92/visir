...
class _SwipePullAlignButtonState extends State<SwipePullAlignButton> with TickerProviderStateMixin {
  ...
  void _pullActionButton(bool isPullingOut) {
    ...
    if (isPullingOut) {
      var tween = AlignmentTween(begin: alignment, end: trailing ? Alignment.centerLeft : Alignment.centerRight).animate(alignCurve);
      tween.addListener(() {
        if (lockAnim) return;
      ...
      var tween = AlignmentTween(begin: alignment, end: trailing ? Alignment.centerRight : Alignment.centerLeft).animate(alignCurve);
      tween.addListener(() {
        if (lockAnim) return;
        alignment = tween.value;
        setState(() {});
  ...
  void _listenEvent() {
    ...
    pullLastButtonToCoverCellEventSubscription = SwipeActionStore.getInstance().bus.on<PullLastButtonToCoverCellEvent>().listen((event) {
      if (event.key == data.parentKey) {
        _animToCoverCell();
      }
    });

    closeNestedActionEventSubscription = SwipeActionStore.getInstance().bus.on<CloseNestedActionEvent>().listen((event) {
  ...
  Future<void> _animToCoverCell() async {
    ...
    animation = Tween<double>(begin: offsetX, end: trailing ? -data.contentWidth : data.contentWidth).animate(offsetCurve)
      ..addListener(() {
        if (lockAnim) return;
  ...
  void _animToCoverPullActionContent() async {
    ...
    if (action.nestedAction?.nestedWidth != null && action.nestedAction!.nestedWidth! > data.totalActionWidth) {
      data.parentState.adjustOffset(offsetX: action.nestedAction!.nestedWidth!, curve: action.nestedAction!.curve, trailing: trailing);
    }
  ...
  Widget build(BuildContext context) {
    ...
    final bool shouldShowNestedActionInfo = widget.actionIndex == 0 && action.nestedAction != null && whenNestedActionShowing;
  ...
  void _initAnim() {
    ...
    if (widget.actionIndex == 0 && action.nestedAction != null) {
      ...
      widthFillActionContentCurve = CurvedAnimation(parent: widthFillActionContentController!, curve: action.nestedAction!.curve);
    }
  }
