    ...
    for (final result in results) {
      ...
      if (result.start.isOnlyWeekdayComponent() && refMoment.isAfter(result.start.dayjs())) {
        if (refMoment.weekday() >= result.start.get(Component.weekday)!) {
          refMoment = refMoment.setWeekday(result.start.get(Component.weekday)! + 7);
        } else {
          refMoment = refMoment.setWeekday(result.start.get(Component.weekday)!);
        }
        ...
        if (result.end != null && result.end!.isOnlyWeekdayComponent()) {
          ...
          if (refMoment.weekday() > result.end!.get(Component.weekday)!) {
            refMoment = refMoment.setWeekday(result.end!.get(Component.weekday)! + 7);
          } else {
            refMoment = refMoment.setWeekday(result.end!.get(Component.weekday)!);
          }
      ...
      if (result.start.isDateWithUnknownYear() && refMoment.isAfter(result.start.dayjs())) {
        for (int i = 0; i < 3 && refMoment.isAfter(result.start.dayjs()); i++) {
          result.start.imply(Component.year, result.start.get(Component.year)! + 1);

          if (result.end != null && !result.end!.isCertain(Component.year)) {
            result.end!.imply(Component.year, result.end!.get(Component.year)! + 1);
          }
        }
      }
    }
