...
final TIMEZONE_OFFSET_PATTERN = RegExp(r'^\s*(?:\(?(?:GMT|UTC)\s?)?([+-])(\d{1,2})(?::?(\d{2}))?\)?', caseSensitive: false);
...
class ExtractTimezoneOffsetRefiner implements Refiner {
  ...
  List<ParsingResult> refine(ParsingContext context, List<ParsingResult> results) {
    for (final result in results) {
      ...
      final minuteOffset = int.parse(match[_TIMEZONE_OFFSET_MINUTE_OFFSET_GROUP] ?? "0");
