...
import './results.dart' show ReferenceWithTimezone, ParsingComponents, ParsingResult;
import './types.dart' show Component, ParsedResult, ParsingOption, ParsingReference, RegExpChronoMatch;
...
class ChronoInstance {
  ...
  ChronoInstance([Configuration? configuration])
    : parsers = (configuration ?? defaultConfig.createCasualConfiguration()).parsers,
      refiners = (configuration ?? defaultConfig.createCasualConfiguration()).refiners;

  /// Create a shallow copy of the Chrono object with the same configuration (`parsers` and `refiners`)
  ChronoInstance clone() {
    return ChronoInstance(Configuration(parsers: [...parsers], refiners: [...refiners]));
  }
  ...
  DateTime? parseDate(String text, [dynamic referenceDate, ParsingOption? option]) {
    assert(referenceDate == null || referenceDate is ParsingReference || referenceDate is DateTime);
    final results = parse(text, referenceDate, option);
    return results.isNotEmpty ? results[0].start.date() : null;
  }

  List<ParsedResult> parse(String text, [dynamic referenceDate, ParsingOption? option]) {
    assert(referenceDate == null || referenceDate is ParsingReference || referenceDate is DateTime);
  ...
  static List<ParsingResult> _executeParser(ParsingContext context, Parser parser) {
    ...
    var match = RegExpChronoMatch.matchOrNull(pattern.firstMatch(remainingText));
    ...
    while (match != null) {
      ...
      if (result == null) {
        ...
        remainingText = match.index + 1 < originalText.length ? originalText.substring(match.index + 1) : '';
...
class ParsingContext implements DebugHandler {
  ...
  ParsingContext(this.text, dynamic irefDate, ParsingOption? option)
    : assert(irefDate == null || irefDate is ParsingReference || irefDate is DateTime),
  ...
  ParsingComponents createParsingComponents([dynamic components]) {
    assert(components == null || components is ParsingComponents || components is Map<Component, num>);
    if (components is ParsingComponents) {
      return components;
    }

    /// TODO: WARNING: forcing double to int; needs research.
    final cmps = components == null ? null : Map<Component, num>.from(components).map((key, val) => MapEntry(key, val.toInt()));

    return ParsingComponents(reference, cmps);
  }

  ParsingResult createParsingResult(int index, dynamic textOrEndIndex, [dynamic startComponents, dynamic endComponents]) {
    assert(textOrEndIndex is int || textOrEndIndex is String);
    assert(startComponents == null || startComponents is ParsingComponents || startComponents is Map<Component, num>);
    assert(endComponents == null || endComponents is ParsingComponents || endComponents is Map<Component, num>);
    final text = textOrEndIndex is String ? textOrEndIndex : this.text.substring(index, textOrEndIndex);

    final start = startComponents != null ? createParsingComponents(startComponents) : null;
    final end = endComponents != null ? createParsingComponents(endComponents) : null;
