...
import 'package:flutter/cupertino.dart'
    show CupertinoApp, CupertinoColors, CupertinoDynamicColor, CupertinoTheme, CupertinoThemeData, CupertinoUserInterfaceLevel, CupertinoUserInterfaceLevelData;
import 'package:flutter/material.dart' show Colors, MaterialLocalizations, Theme, ThemeData, debugCheckHasMaterialLocalizations;
import 'package:flutter/services.dart';
import 'package:flutter/widgets.dart';

import '../../modal_bottom_sheet.dart';

const double _kPreviousPageVisibleOffset = -8;
...
class _CupertinoBottomSheetContainer extends StatelessWidget {
  ...
  const _CupertinoBottomSheetContainer({required this.child, this.backgroundColor, required this.topRadius, this.overlayStyle, this.shadow});

  @override
  Widget build(BuildContext context) {
    final scopedOverlayStyle = overlayStyle;
    final topSafeAreaPadding = MediaQuery.of(context).padding.top;
    final topPadding = max(0.0, topSafeAreaPadding);
    ...
    if (scopedOverlayStyle != null) {
      bottomSheetContainer = AnnotatedRegion<SystemUiOverlayStyle>(value: scopedOverlayStyle, child: bottomSheetContainer);
    }
  ...
  final result = await Navigator.of(context, rootNavigator: useRootNavigator).push(
    CupertinoModalBottomSheetRoute<T>(
      ...
      containerBuilder: (context, _, child) =>
          _CupertinoBottomSheetContainer(child: child, backgroundColor: backgroundColor, topRadius: topRadius, shadow: shadow, overlayStyle: overlayStyle),
  ...
  Widget buildTransitions(BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation, Widget child) {
    ...
    return AnimatedBuilder(
      builder: (context, child) => Transform.translate(
        ...
        child: Transform.scale(scale: scale, child: child, alignment: Alignment.topCenter, filterQuality: FilterQuality.none),
      ),
      child: child,
...
class _CupertinoModalTransition extends StatelessWidget {
  ...
  const _CupertinoModalTransition({required this.secondaryAnimation, required this.body, required this.topRadius, this.backgroundColor = Colors.black, this.animationCurve});

  @override
  Widget build(BuildContext context) {
    var startRoundCorner = 0.0;
    final paddingTop = max(0, MediaQuery.of(context).padding.top - 8);
  ...
  CupertinoThemeData createPreviousRouteTheme(BuildContext context, Animation<double> animation) {
    ...
    if (cTheme.scaffoldBackgroundColor is CupertinoDynamicColor) {
      ...
      previousRouteTheme = previousRouteTheme.copyWith(
        scaffoldBackgroundColor: ColorTween(begin: systemBackground, end: elevatedScaffoldBackgroundColor.resolveFrom(context)).evaluate(animation),
    ...
    if (cTheme.barBackgroundColor is CupertinoDynamicColor) {
      ...
      previousRouteTheme = previousRouteTheme.copyWith(
        barBackgroundColor: ColorTween(begin: barBackgroundColor, end: elevatedBarBackgroundColor.resolveFrom(context)).evaluate(animation),
        primaryColor: CupertinoColors.placeholderText.resolveFrom(context),
      );
    }
...
class CupertinoScaffoldInheirted extends InheritedWidget {
  ...
  const CupertinoScaffoldInheirted({this.animation, required super.child, this.topRadius, required this.transitionBackgroundColor}) : super();
...
class CupertinoScaffold extends StatefulWidget {
  static CupertinoScaffoldInheirted? of(BuildContext context) => context.dependOnInheritedWidgetOfExactType<CupertinoScaffoldInheirted>();
  ...
  const CupertinoScaffold({super.key, required this.body, this.topRadius = _kDefaultTopRadius, this.transitionBackgroundColor = Colors.black, this.overlayStyle});
  ...
  static Future<T?> showCupertinoModalBottomSheet<T>({
    ...
    RouteSettings? settings,
    BoxShadow? shadow,
    @Deprecated('Will be ignored. OverlayStyle is computed from luminance of transitionBackgroundColor') SystemUiOverlayStyle? overlayStyle,
  }) async {
    assert(debugCheckHasMediaQuery(context));
    ...
    final overlayStyle = overlayStyleFromColor(backgroundColor ?? transitionBackgroundColor);
    final result = await Navigator.of(context, rootNavigator: useRootNavigator).push(
      CupertinoModalBottomSheetRoute<T>(
        closeProgressThreshold: closeProgressThreshold,
        builder: builder,
        ...
        containerBuilder: (context, _, child) =>
            _CupertinoBottomSheetContainer(child: child, backgroundColor: backgroundColor, topRadius: topRadius ?? _kDefaultTopRadius, shadow: shadow, overlayStyle: overlayStyle),
        expanded: expand,
        barrierLabel: barrierLabel,
        bounce: bounce,
        isDismissible: isDismissible ?? expand == false ? true : false,
...
class _CupertinoScaffoldState extends State<CupertinoScaffold> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return CupertinoScaffoldInheirted(
      ...
      child: _CupertinoModalTransition(secondaryAnimation: animationController, body: widget.body, topRadius: widget.topRadius, backgroundColor: widget.transitionBackgroundColor),
    );
  }
}
