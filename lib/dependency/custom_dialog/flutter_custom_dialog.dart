...
class YYDialog {
  ...
  Color barrierColor = Colors.black.withValues(alpha: .3); //弹窗外的背景色
  ...
  void dismiss() {
    if (_isShowing) {
      Navigator.of(Utils.mainContext, rootNavigator: useRootNavigator).pop();
    }
  }
...
class CustomDialog {
  ...
  show() async {
    ...
    if (_barrierColor == Colors.transparent) {
      _barrierColor = Colors.white.withValues(alpha: 0.0);
    }

    await showGeneralDialog(
      context: _context,
      barrierColor: _barrierColor ?? Colors.black.withValues(alpha: .3),
