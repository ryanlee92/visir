...
import 'package:Visir/dependency/fl_chart/src/chart/base/axis_chart/axis_chart_extensions.dart';
...
import 'package:Visir/dependency/fl_chart/src/extensions/text_align_extension.dart';
...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  Path generateNormalBarPath(
    ...
    for (var i = 1; i < size; i++) {
      ...
      if (barData.preventCurveOverShooting) {
        if ((next - current).dy <= barData.preventCurveOvershootingThreshold || (current - previous).dy <= barData.preventCurveOvershootingThreshold) {
          temp = Offset(temp.dx, 0);
        }

        if ((next - current).dx <= barData.preventCurveOvershootingThreshold || (current - previous).dx <= barData.preventCurveOvershootingThreshold) {
  ...
  double _getDotHeight({
    ...
    for (final info in showingTooltipSpots) {
      ...
      final indicators = holder.data.lineTouchData.getTouchedSpotIndicator(lineData, [info.spotIndex]);

      final indicatorData = indicators.elementAtOrNull(0);
      if (indicatorData != null && indicatorData.touchedSpotDotData.show) {
        final xPercentInLine = (getPixelX(info.x, viewSize, holder) / getBarLineXLength(lineData, viewSize, holder)) * 100;
        final dotPainter = indicatorData.touchedSpotDotData.getDotPainter(info, xPercentInLine, lineData, info.spotIndex);
