...
import 'package:Visir/dependency/fl_chart/src/chart/base/axis_chart/axis_chart_painter.dart';
import 'package:Visir/dependency/fl_chart/src/chart/base/base_chart/base_chart_painter.dart';
import 'package:Visir/dependency/fl_chart/src/extensions/bar_chart_data_extension.dart';
import 'package:Visir/dependency/fl_chart/src/extensions/paint_extension.dart';
import 'package:Visir/dependency/fl_chart/src/extensions/path_extension.dart';
import 'package:Visir/dependency/fl_chart/src/extensions/rrect_extension.dart';
import 'package:Visir/dependency/fl_chart/src/utils/canvas_wrapper.dart';
import 'package:Visir/dependency/fl_chart/src/utils/utils.dart';
...
class BarChartPainter extends AxisChartPainter<BarChartData> {
  ...
  void paint(BuildContext context, CanvasWrapper canvasWrapper, PaintHolder<BarChartData> holder) {
    if (holder.chartVirtualRect != null) {
      ...
      canvasWrapper
        ..saveLayer(canvasRect, _clipPaint)
        ..clipRect(canvasRect);
    }
    ...
    _groupBarsPosition = calculateGroupAndBarsPosition(usableSize, groupsXAdjusted, data.barGroups);

    if (!data.extraLinesData.extraLinesOnTop) {
      super.drawHorizontalLines(context, canvasWrapper, holder, usableSize);
    }
    ...
    for (var i = 0; i < data.barGroups.length; i++) {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        drawTouchTooltip(context, canvasWrapper, _groupBarsPosition!, targetData.barTouchData.touchTooltipData, barGroup, i, barRod, j, holder);
      }
    }
  }
  ...
  List<GroupBarsPosition> calculateGroupAndBarsPosition(Size viewSize, List<double> groupsX, List<BarChartGroupData> barGroups) {
    ...
    for (var i = 0; i < barGroups.length; i++) {
      ...
      if (barGroup.groupVertically) {
        groupBarsPosition.add(GroupBarsPosition(groupX, List.generate(barGroup.barRods.length, (index) => groupX)));
        continue;
      }
  ...
  @visibleForTesting
  void drawBars(CanvasWrapper canvasWrapper, List<GroupBarsPosition> groupBarsPosition, PaintHolder<BarChartData> holder) {
    final data = holder.data;
    ...
    for (var i = 0; i < data.barGroups.length; i++) {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        final cornerHeight = max(borderRadius.topLeft.y, borderRadius.topRight.y) + max(borderRadius.bottomLeft.y, borderRadius.bottomRight.y);

        RRect barRRect;

        /// Draw [BackgroundBarChartRodData]
        if (barRod.backDrawRodData.show && barRod.backDrawRodData.toY != barRod.backDrawRodData.fromY) {
          if (barRod.backDrawRodData.toY > barRod.backDrawRodData.fromY) {
            // positive
            final bottom = getPixelY(max(data.minY, barRod.backDrawRodData.fromY), viewSize, holder);
            final top = min(getPixelY(barRod.backDrawRodData.toY, viewSize, holder), bottom - cornerHeight);
            ...
            // negative
            final top = getPixelY(min(data.maxY, barRod.backDrawRodData.fromY), viewSize, holder);
            final bottom = max(getPixelY(barRod.backDrawRodData.toY, viewSize, holder), top + cornerHeight);
          ...
          _barPaint.setColorOrGradient(backDraw.color, backDraw.gradient, barRRect.getRect());
        ...
        if (barRod.toY != barRod.fromY) {
          if (barRod.toY > barRod.fromY) {
            ...
            final top = min(getPixelY(barRod.toY, viewSize, holder), bottom - cornerHeight);
            ...
            final bottom = max(getPixelY(barRod.toY, viewSize, holder), top + cornerHeight);
          ...
          _barPaint.setColorOrGradient(barRod.color, barRod.gradient, barRRect.getRect());
          canvasWrapper.drawRRect(barRRect, _barPaint);
          ...
          if (barRod.rodStackItems.isNotEmpty) {
            for (var i = 0; i < barRod.rodStackItems.length; i++) {
              ...
              final rect = isNegative ? Rect.fromLTRB(left, stackFromY, right, stackToY) : Rect.fromLTRB(left, stackToY, right, stackFromY);
              canvasWrapper
                ..save()
                ..clipRect(rect)
              ...
              drawStackItemBorderStroke(canvasWrapper, stackItem, i, barRod.rodStackItems.length, barRod.width, barRRect, viewSize, holder);
            }
          }
          ...
          if (borderSide.width > 0 && borderSide.color.a > 0) {
            ...
            canvasWrapper.drawPath(borderPath.toDashedPath(barRod.borderDashArray), _barStrokePaint);
          }
        }
      }
    }
  ...
  @visibleForTesting
  void drawErrorIndicatorData(CanvasWrapper canvasWrapper, List<GroupBarsPosition> groupBarsPosition, PaintHolder<BarChartData> holder) {
    final data = holder.data;
    ...
    for (var i = 0; i < data.barGroups.length; i++) {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        final top = getPixelY(barRod.toY + barRod.toYErrorRange!.upperBy, viewSize, holder) - y;

        final bottom = getPixelY(barRod.toY - barRod.toYErrorRange!.lowerBy, viewSize, holder) - y;

        final relativeErrorPixelsRect = Rect.fromLTRB(0, top, 0, bottom);

        final painter = errorIndicatorData.painter(BarChartSpotErrorRangeCallbackInput(group: barGroup, groupIndex: i, rod: barRod, barRodIndex: j));
        canvasWrapper.drawErrorIndicator(
          painter,
          FlSpot(barGroup.x.toDouble(), barRod.toY, yError: barRod.toYErrorRange),
          Offset(x, y),
  ...
  void drawTouchTooltip(
    ...
    final tooltipItem = tooltipData.getTooltipItem(showOnBarGroup, barGroupIndex, showOnRodData, barRodIndex);

    if (tooltipItem == null) {
      return;
    }

    final span = TextSpan(style: Utils().getThemeAwareTextStyle(context, tooltipItem.textStyle), text: tooltipItem.text, children: tooltipItem.children);

    final tp = TextPainter(text: span, textAlign: tooltipItem.textAlign, textDirection: tooltipItem.textDirection, textScaler: holder.textScaler)
    ...
    final barToYPixel = Offset(barX, getPixelY(showOnRodData.toY, viewSize, holder));

    final barFromYPixel = Offset(barX, getPixelY(showOnRodData.fromY, viewSize, holder));
    ...
    final drawTooltipOnTop = tooltipData.direction == TooltipDirection.top || (tooltipData.direction == TooltipDirection.auto && showOnRodData.isUpward());

    final tooltipOriginPoint = Offset(barX, drawTooltipOnTop ? barTopY : barBottomY);
    ...
    final tooltipTop = drawTooltipOnTop ? barTopY - tooltipHeight - tooltipData.tooltipMargin : barBottomY + tooltipData.tooltipMargin;

    final tooltipLeft = getTooltipLeft(barToYPixel.dx, tooltipWidth, tooltipData.tooltipHorizontalAlignment, tooltipData.tooltipHorizontalOffset);

    /// draw the background rect with rounded radius
    // ignore: omit_local_variable_types
    Rect rect = Rect.fromLTWH(tooltipLeft, tooltipTop, tooltipWidth, tooltipHeight);

    if (tooltipData.fitInsideHorizontally) {
      if (rect.left < 0) {
        final shiftAmount = 0 - rect.left;
        rect = Rect.fromLTRB(rect.left + shiftAmount, rect.top, rect.right + shiftAmount, rect.bottom);
      }
    ...
    if (tooltipData.fitInsideVertically) {
      if (rect.top < 0) {
        ...
        rect = Rect.fromLTRB(rect.left, rect.top + shiftAmount, rect.right, rect.bottom + shiftAmount);
      }
    ...
    final drawOffset = Offset(rect.center.dx - (tp.width / 2), rect.topCenter.dy + top - textRotationOffset.dy + rectRotationOffset.dy);
    ...
    canvasWrapper.drawRotated(
      ...
      drawCallback: () {
        // 먼저 그림자 그리기
        canvasWrapper.drawRRect(shadowRRect, shadowPaint);
        // 그 다음 배경과 테두리 그리기
        canvasWrapper
  ...
  BarTouchedSpot? handleTouch(Offset localPosition, Size size, PaintHolder<BarChartData> holder) {
    final data = holder.data;
    final targetData = holder.targetData;
    ...
    for (var i = 0; i < _groupBarsPosition!.length; i++) {
      ...
      for (var j = 0; j < groupBarPos.barsX.length; j++) {
        ...
        if (isUpward) {
          barTopY = getPixelY(targetData.barGroups[i].barRods[j].toY, viewSize, holder);
          barBotY = getPixelY(targetData.barGroups[i].barRods[j].fromY + targetData.barGroups[i].barRods[j].backDrawRodData.fromY, viewSize, holder);
        } else {
          barTopY = getPixelY(targetData.barGroups[i].barRods[j].fromY + targetData.barGroups[i].barRods[j].backDrawRodData.fromY, viewSize, holder);
          barBotY = getPixelY(targetData.barGroups[i].barRods[j].toY, viewSize, holder);
        }

        final backDrawBarY = getPixelY(targetData.barGroups[i].barRods[j].backDrawRodData.toY, viewSize, holder);
        final touchExtraThreshold = targetData.barTouchData.touchExtraThreshold;

        final isXInTouchBounds =
            (touchedPoint.dx <= barX + halfBarWidth + touchExtraThreshold.right) && (touchedPoint.dx >= barX - halfBarWidth - touchExtraThreshold.left);

        bool isYInBarBounds;
        if (isUpward) {
          isYInBarBounds = (touchedPoint.dy <= barBotY + touchExtraThreshold.bottom) && (touchedPoint.dy >= barTopY - touchExtraThreshold.top);
        } else {
          isYInBarBounds = (touchedPoint.dy >= barTopY - touchExtraThreshold.top) && (touchedPoint.dy <= barBotY + touchExtraThreshold.bottom);
        }

        bool isYInBarBackDrawBounds;
        if (isUpward) {
          isYInBarBackDrawBounds = (touchedPoint.dy <= barBotY + touchExtraThreshold.bottom) && (touchedPoint.dy >= backDrawBarY - touchExtraThreshold.top);
        } else {
          isYInBarBackDrawBounds = (touchedPoint.dy >= barTopY - touchExtraThreshold.top) && (touchedPoint.dy <= backDrawBarY + touchExtraThreshold.bottom);
        }

        final isYInTouchBounds = (targetData.barTouchData.allowTouchBarBackDraw && isYInBarBackDrawBounds) || isYInBarBounds;
