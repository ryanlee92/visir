...
class RadarChartPainter extends BaseChartPainter<RadarChartData> {
  ...
  double getFirstTickValue(RadarChartData data) {
    ...
    return dataSetMaxValue == dataSetMinValue ? (dataSetMaxValue - defaultCenterValue) / (data.tickCount + 1) + defaultCenterValue : dataSetMinValue;
  }
  ...
  void drawTicks(
    ...
    final tickDistance = data.isMinValueAtCenter ? radius / (ticks.length - 1) : radius / ticks.length;
  ...
  void drawTitles(
    ...
    for (var index = 0; index < data.titleCount; index++) {
      ...
      final threshold = 1.0 + (title.positionPercentageOffset ?? data.titlePositionPercentageOffset);
  ...
  RadarTouchedSpot? handleTouch(
    ...
    for (var i = 0; i < dataSetsPosition!.length; i++) {
      ...
      for (var j = 0; j < dataSetPosition.entriesOffset.length; j++) {
        ...
        if ((touchedPoint.dx - entryOffset.dx).abs() <= targetData.radarTouchData.touchSpotThreshold &&
            (touchedPoint.dy - entryOffset.dy).abs() <= targetData.radarTouchData.touchSpotThreshold) {
