...
import 'package:Visir/dependency/fl_chart/src/chart/scatter_chart/scatter_chart_data.dart';
...
class ScatterChartPainter extends AxisChartPainter<ScatterChartData> {
    ...
    if (data.scatterLabelSettings.showLabel) {
      for (var i = 0; i < data.scatterSpots.length; i++) {
        ...
        if (pixelY > centerChartY) {
          ...
          final off = (radius * 1.5 < tp.height || radius * 1.5 < tp.width) ? radius + tp.height : tp.height / 2;
          ...
          /// else draw the label inside the bubble.
          final off = (radius * 1.5 < tp.height || radius * 1.5 < tp.width) ? radius : -tp.height / 2;
          newPixelY = pixelY + off;
        }
  ...
  void drawTouchTooltip(
    ...
    var rect = Rect.fromLTWH(
      tooltipLeftPosition,
      tooltipOriginPoint.dy - tooltipHeight - (showOnSpot.size.height / 2) - tooltipItem.bottomMargin,
      tooltipWidth,
      tooltipHeight,
    );
    ...
    final textRotationOffset = Utils().calculateRotationOffset(drawingTextPainter.size, rotateAngle);

    final drawOffset = Offset(
      rect.center.dx - (drawingTextPainter.width / 2),
      rect.topCenter.dy + tooltipData.tooltipPadding.top - textRotationOffset.dy + rectRotationOffset.dy,
    );
