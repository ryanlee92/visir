...
abstract class AxisChartPainter<D extends AxisChartData> extends BaseChartPainter<D> {
  ...
  void drawGrid(CanvasWrapper canvasWrapper, PaintHolder<D> holder) {
    ...
    if (data.gridData.drawHorizontalLine) {
      final horizontalInterval = data.gridData.horizontalInterval ?? Utils().getEfficientInterval(viewSize.height, data.verticalDiff);
  ...
  void drawHorizontalLines(
    ...
    for (final line in holder.data.extraLinesData.horizontalLines) {
      ...
      final isLineOutsideOfChart = from.dy < 0 || to.dy < 0 || from.dy > viewSize.height || to.dy > viewSize.height;
  ...
  void drawVerticalLines(
    ...
    for (final line in holder.data.extraLinesData.verticalLines) {
      ...
      final isLineOutsideOfChart = from.dx < 0 || to.dx < 0 || from.dx > viewSize.width || to.dx > viewSize.width;
