...
class HtmlEditorController extends unsupported.HtmlEditorController {
  ...
  dynamic evaluateJavascriptWithResult({required source}) async {
    if (source == 'JSON.stringify(document.getSelection().getRangeAt(0).getClientRects());') {
      final result = await evaluateJavascriptWeb('getSelectionPosition', hasReturnValue: true);
      return result;
    }
  }
}
