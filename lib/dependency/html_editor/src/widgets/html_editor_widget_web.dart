...
import 'package:Visir/dependency/html_editor/utils/shims/dart_ui.dart' as ui;
...
class _HtmlEditorWidgetWebState extends State<HtmlEditorWidget> {
  ...
  void initSummernote() async {
    ...
    var summernoteScripts = """
      <script type="text/javascript">
        ...
        document.addEventListener('wheel', function(event) {
          window.parent.postMessage(JSON.stringify({"view": "$createdViewId", "wheelX": event.deltaX, "wheelY": event.deltaY}), "*");
        }, { passive: false });
        ...
        document.addEventListener('touchmove', function(event) {        
          ...
          start.x = event.touches[0].pageX;
          start.y = event.touches[0].pageY;
          window.parent.postMessage(JSON.stringify({"view": "$createdViewId", "wheelX": deltaX, "wheelY": deltaY}), "*");
        }, { passive: false });
        ...
        function handleMessage(e) {
          if (e && e.data && e.data.includes("toIframe:")) {
            ...
            if (data["view"].includes("$createdViewId")) {
              ...
              if (data["type"].includes("getSelectionPosition")) { 
                window.parent.postMessage(JSON.stringify({"view": "$createdViewId", "type": "toDart: getSelectionPosition", "position": JSON.stringify(document.getSelection().getRangeAt(0).getClientRects())}), "*");
              }
    ...
    """;
    var filePath = 'assets/html_editor/summernote-no-plugins.html';
    ...
    @media (prefers-color-scheme: light) {
      ...
      body {
          background: ${Utils.lightTheme.colorScheme.outline.toHex()} !important;
      ...
      .note-btn:not(.note-color-btn) {
          background-color: ${Utils.lightTheme.colorScheme.outline.toHex()} !important;
      }
      ...
      .note-handle .note-control-selection .note-control-handle,.note-handle .note-control-selection .note-control-holder,.note-handle .note-control-selection .note-control-sizing {
        width: 7px;
        height: 7px;
        border: 1px solid ${Utils.lightTheme.colorScheme.primary.toHex()};
      }
    ...
    @media (prefers-color-scheme: dark) {
      ...
      body {
          background: ${Utils.darkTheme.colorScheme.outline.toHex()} !important;
      ...
      .note-btn:not(.note-color-btn) {
          background-color: ${Utils.darkTheme.colorScheme.outline.toHex()} !important;
      }
      ...
      .note-handle .note-control-selection .note-control-handle,.note-handle .note-control-selection .note-control-holder,.note-handle .note-control-selection .note-control-sizing {
        width: 7px;
        height: 7px;
        border: 1px solid ${Utils.darkTheme.colorScheme.primary.toHex()};
      }
...
''');
    ...
    final iframe = web.HTMLIFrameElement()
      ...
      ..height = max(actualHeight + 20, widget.htmlEditorOptions.minHeight ?? 0).toString()
      ...
      ..onLoad.listen((event) async {
        ...
        web.window.onMessage.listen((event) {
          ...
          if (data['type'] != null && data['type'].contains('toDart: getSelectionPosition') && data['view'] == createdViewId) {}
  ...
  Widget build(BuildContext context) {
    return Container(
      height: max(actualHeight + 20, widget.htmlEditorOptions.minHeight ?? 0),
      child: SingleChildScrollView(
        physics: NeverScrollableScrollPhysics(),
        child: Container(
          height: max(actualHeight + 200, widget.htmlEditorOptions.minHeight ?? 0),
          child: Column(
            children: <Widget>[
