...
import 'package:Visir/dependency/html_editor/src/html_editor_controller_unsupported.dart' as unsupported;
...
class HtmlEditorController extends unsupported.HtmlEditorController {
  HtmlEditorController({this.processInputHtml = true, this.processNewLineAsBr = false, this.processOutputHtml = true});
  ...
  Future<String> getText() async {
    var text = await evaluateJavascriptWithResult(source: "\$('#summernote-2').summernote('code');") as String?;
  ...
  @override
  void clear() {
    _evaluateJavascript(source: "\$('#summernote-2').summernote('code', '');");
  }
  ...
  @override
  void clearFocus() {
    _evaluateJavascript(source: "\$('.note-editable').blur();");
  ...
  dynamic evaluateJavascriptWithResult({required source}) async {
    if (!kIsWeb) {
      if (editorController == null) {
        throw Exception('HTML editor is still loading, please wait before evaluating this JS: $source!');
      }
  ...
  Future<void> addJavaScriptHandler({required String handlerName, required void Function(List<dynamic> arguments) callback}) async {
    // if (handlerName != 'setHeight') return;
    if (!kIsWeb) {
      if (editorController == null) {
        throw Exception('HTML editor is still loading, please wait before adding a JS handler!');
      }
      editorController!.removeJavaScriptHandler(handlerName: handlerName);
      editorController!.addJavaScriptHandler(
        handlerName: handlerName,
        callback: (e) {
          final result = e.map((d) {
            return jsonDecode(d);
          }).toList();
          callback(result);
        },
