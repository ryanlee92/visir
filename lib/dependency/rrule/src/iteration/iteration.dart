...
Iterable<DateTime> getFirstRecurrenceRuleInstances(
  RecurrenceRule rrule, {
  ...
  rrule = _prepare(rrule, after);

  var count = rrule.count ?? 2;

  var currentStart = after;
  var timeSet = makeTimeSet(rrule, after.timeOfDay);
  ...
  while (true) {
    ...
    Iterable<DateTime> results;
    if (rrule.hasBySetPositions) {
      results = buildSetPositionsList(rrule, dateSet, timeSet).where((dt) => after <= dt);
    } else {
      results = dateSet.includedDates.expand((date) {
        return timeSet.map((time) => date.add(time));
      });
...
RecurrenceRule _prepare(RecurrenceRule rrule, DateTime start) {
  ...
  final byDatesEmpty = rrule.byWeekDays.isEmpty && rrule.byMonthDays.isEmpty && rrule.byYearDays.isEmpty && rrule.byWeeks.isEmpty;

  return RecurrenceRule(
    frequency: rrule.frequency,
    until: rrule.until == null ? null : DateTime.parse(rrule.until!.toLocal().toIso8601String() + 'Z'),
    count: rrule.count,
    ...
    byWeekDays: byDatesEmpty && rrule.frequency == Frequency.weekly ? [ByWeekDayEntry(start.weekday)] : rrule.byWeekDays,
    byMonthDays: byDatesEmpty && (rrule.frequency == Frequency.monthly || rrule.frequency == Frequency.yearly) ? [start.day] : rrule.byMonthDays,
    byYearDays: rrule.byYearDays,
    byWeeks: rrule.byWeeks,
    byMonths: byDatesEmpty && rrule.frequency == Frequency.yearly && rrule.byMonths.isEmpty ? [start.month] : rrule.byMonths,
