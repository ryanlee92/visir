...
class RecurrenceRuleToTextEncoder extends Converter<RecurrenceRule, String> {
  ...
  String convert(RecurrenceRule input) {
    ...
    final frequencyIntervalString = l10n.frequencyInterval(input.frequency, input.actualInterval);
  ...
  RecurrenceRule _normalize(RecurrenceRule input) {
    ...
    if (input.frequency == Frequency.monthly) {
      final byEveryWeekDay = [
        for (final weekDay in DateTime.monday.rangeTo(DateTime.sunday)) ByWeekDayEntry(weekDay),
      ];
      if (!input.hasBySeconds &&
          !input.hasByMinutes &&
          !input.hasByHours &&
          const DeepCollectionEquality.unordered().equals(input.byWeekDays, byEveryWeekDay) &&
          !input.hasByMonthDays &&
          !input.hasByYearDays &&
          !input.hasByWeeks &&
          !input.hasByMonths &&
  ...
  void _convertDaily(RecurrenceRule input, StringBuffer output) {
    ...
    output
      ...
      ..add(_formatByWeekDays(
        input,
        variant: input.hasBySetPositions ? InOnVariant.instanceOf : InOnVariant.simple,
      ))
      ..add(_formatByMonthDays(
        ...
        variant: input.hasByWeekDays
            ...
            : input.hasBySetPositions
                ...
                : InOnVariant.simple,
        combination: input.hasByWeekDays ? ListCombination.disjunctive : ListCombination.conjunctiveShort,
      ));
  }
  ...
  void _convertYearly(RecurrenceRule input, StringBuffer output) {
    ...
    final firstVariant = input.hasBySetPositions ? InOnVariant.instanceOf : InOnVariant.simple;
    ...
    final startWithByYearDays = input.hasByYearDays && !startWithByWeekDays && !startWithByMonthDays;
    if (startWithByYearDays) {
      output.add(_formatByYearDays(input, variant: firstVariant));
    }

    final startWithByWeeks = input.hasByWeeks && !startWithByWeekDays && !startWithByMonthDays && !startWithByYearDays;
    if (startWithByWeeks) {
      output.add(_formatByWeeks(input, variant: firstVariant));
    }

    final startWithByMonths = input.hasByMonths && !startWithByWeekDays && !startWithByMonthDays && !startWithByYearDays && !startWithByWeeks;
    if (startWithByMonths) {
      output.add(_formatByMonths(input, variant: firstVariant));
    }

    final daysOnlyByWeek = input.hasByWeekDays && !input.hasByMonthDays && !input.hasByYearDays;
    final daysOnlyByMonth = !input.hasByWeekDays && input.hasByMonthDays && !input.hasByYearDays;

    final appendByWeeksDirectly = daysOnlyByWeek && input.hasByWeeks;
    final appendByMonthsDirectly = (daysOnlyByWeek || daysOnlyByMonth) && !input.hasByWeeks && input.hasByMonths;
...
extension on Iterable<ByWeekDayEntry> {
  String occurrenceFreeFormattedForUser(
    ...
    final raw = map((e) => e.day).sortedBy<num>((it) => (it - DateTime.monday) % DateTime.daysPerWeek);
    ...
    for (var i = 0; i < raw.length; i++) {
      ...
      while (raw.length > i + 1 && raw[i + 1] == (current + 1) % DateTime.daysPerWeek) {
        i++;
        current = raw[i];
      }
  ...
  String formattedForUser(
    ...
    final grouped = groupBy<ByWeekDayEntry, int?>(this, (e) => e.occurrence).entries.sortedForUserGeneral(key: (it) => it.value.first.occurrence ?? 0);
    ...
    final strings = grouped.map((entry) {
      ...
      final daysOfWeek = entry.value.map((it) => ByWeekDayEntry(it.day)).occurrenceFreeFormattedForUser(
            l10n,
            addEveryPrefix: addEveryPrefix && !hasOccurrence,
            weekStart: weekStart,
            combination: ListCombination.conjunctiveShort,
          );
      return hasOccurrence ? l10n.nthDaysOfWeek(hasOccurrence ? [entry.key!] : [], daysOfWeek) : daysOfWeek;
    }).toList();
    ...
    return l10n.list(
      strings,
      atMostOneWeekDayPerOccurrence ? ListCombination.conjunctiveShort : ListCombination.conjunctiveLong,
    );
  }
}
