...
class RecurrenceRule {
  ...
  Iterable<DateTime> getInstances({
    ...
    start = DateTime.parse(start.toLocal().toIso8601String() + 'Z');
    after = after == null ? null : DateTime.parse(after.toLocal().toIso8601String() + 'Z');
    before = before == null ? null : DateTime.parse(before.toLocal().toIso8601String() + 'Z');
    ...
    return getRecurrenceRuleInstances(
      ...
      final newDate = DateTime(e.toUtc().year, e.toUtc().month, e.toUtc().day, e.toUtc().hour, e.toUtc().minute);
      return newDate;
    });
  }

  Iterable<DateTime> getFirstInstances({
    required DateTime after,
    bool includeAfter = false,
  }) {
    after = DateTime.parse(after.toLocal().toIso8601String() + 'Z');

    assert(after.isValidRruleDateTime);

    return getFirstRecurrenceRuleInstances(
      this,
      after: after,
      includeAfter: includeAfter,
    ).map((e) {
