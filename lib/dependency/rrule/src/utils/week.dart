...
extension DateTimeWeekInfoRrule on DateTime {
  ...
  int get weekOfMonth {
    var date = this;
    // 해당 날짜의 요일
    int targetWeekday = date.weekday;

    // 그 달의 첫 번째 날
    DateTime firstDayOfMonth = DateTime(date.year, date.month, 1);

    // 첫 번째 해당 요일 찾기
    int firstTargetWeekdayOffset = (targetWeekday - firstDayOfMonth.weekday + 7) % 7;
    DateTime firstTargetWeekday = firstDayOfMonth.add(Duration(days: firstTargetWeekdayOffset));

    // 주어진 날짜가 첫 번째 해당 요일로부터 몇 주 차인지 계산
    int weekIndex = ((date.day - firstTargetWeekday.day) ~/ 7) + 1;
    return weekIndex;
  }
}
...
class WeekInfo implements Comparable<WeekInfo> {
  const WeekInfo(this.weekBasedYear, this.weekOfYear) : assert(weekOfYear >= 1 && weekOfYear <= 53);
  ...
  bool operator ==(Object other) {
    return other is WeekInfo && other.weekBasedYear == weekBasedYear && other.weekOfYear == weekOfYear;
  }

  @override
  String toString() => 'WeekInfo(weekBasedYear = $weekBasedYear, weekOfYear = $weekOfYear)';
}
