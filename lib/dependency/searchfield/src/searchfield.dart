...
class SearchField<T> extends StatefulWidget {
  ...
  ///
  /// When not specified, the default value is `context.onSurface.withValues(alpha:0.1)`.
...
class SearchFieldState<T> extends State<SearchField<T>> {
  ...
  void initialize() {
    ...
    _focus.addListener(() {
      ...
      if (isSuggestionExpanded) {
        ...
        Overlay.of(Utils.mainContext).insert(_overlayEntry!);
      } else {
        if (_overlayEntry != null && _overlayEntry!.mounted) {
          _overlayEntry?.remove();
        }
  ...
  Future<bool> isSuggestionExists() async {
    return (await suggestionStream.stream.last)?.isNotEmpty == true;
  }
  ...
  ValueNotifier<String> hoverKey = ValueNotifier('');
    ...
    return StreamBuilder<List<SearchFieldListItem<T>?>?>(
      ...
      builder: (BuildContext context, AsyncSnapshot<List<SearchFieldListItem<T>?>?> snapshot) {
        if (snapshot.data == null || !isSuggestionExpanded) {
          ...
          final paddingHeight = 6;
          if (snapshot.data!.length > widget.maxSuggestionsInViewPort) {
            _totalHeight = widget.itemHeight * widget.maxSuggestionsInViewPort + paddingHeight * 2;
          } else if (snapshot.data!.length == 1) {
            _totalHeight = widget.itemHeight + paddingHeight * 2;
          } else {
            _totalHeight = snapshot.data!.length * widget.itemHeight + paddingHeight * 2;
          }

          final Widget listView = SuperListView.builder(
            ...
            itemBuilder: (context, index) => TextFieldTapRegion(
              child: VisirButton(
                ...
                onHover: (hover) {
                  if (hover) {
                    hoverKey.value = snapshot.data![index]!.searchKey;
                  } else {
                    hoverKey.value = '';
                  }
                },
  ...
  Widget build(BuildContext context) {
    ...
    return CompositedTransformTarget(
      link: _layerLink,
      child: StreamBuilder<List<SearchFieldListItem<T>?>?>(
          stream: suggestionStream.stream,
          builder: (BuildContext context, AsyncSnapshot<List<SearchFieldListItem<T>?>?> snapshot) {
            return TextFormField(
              ...
              style: widget.searchStyle ?? context.bodySmall,
              onFieldSubmitted: (x) {
                  ...
                  if (event is KeyDownEvent) {
                    switch (event.logicalKey) {
                      case LogicalKeyboardKey.arrowDown:
                        final newIndex = (snapshot.data!.indexWhere((element) => element!.searchKey == hoverKey.value) + 1) % snapshot.data!.length;
                        hoverKey.value = snapshot.data![newIndex]!.searchKey;
                      ...
                      case LogicalKeyboardKey.enter:
                        final index = (snapshot.data!.indexWhere((element) => element!.searchKey == hoverKey.value) - 1) % snapshot.data!.length;
                        searchController!.text = hoverKey.value;
                        searchController!.selection = TextSelection.fromPosition(
                          TextPosition(
                            offset: searchController!.text.length,
                          ),
              ...
              onChanged: (query) async {
                final searchResult = await widget.onSearchTextChanged(query) ?? [];
