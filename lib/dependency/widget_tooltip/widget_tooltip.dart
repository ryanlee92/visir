...
ValueNotifier<int> currentTooltipHashcode = ValueNotifier(0);
...
class _WidgetTooltipState extends State<WidgetTooltip> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    currentTooltipHashcode.addListener(onCurrentTooltipHashcodeChange);
  }

  void onCurrentTooltipHashcodeChange() {
    if (currentTooltipHashcode.value != widget.hashCode) {
      dismiss();
    }
  }
  ...
  void dispose() {
    dismiss();
    currentTooltipHashcode.removeListener(onCurrentTooltipHashcodeChange);
  ...
  void toggleOnPointer(PointerEvent event) {
    ...
    return CompositedTransformTarget(
      ...
      child: GestureDetector(
        ...
        child: MouseRegion(
          ...
          onHover: (widget.direction == WidgetTooltipDirection.pointer)
              ? (event) {
                  toggleOnPointer(event);
                }
              : null,
          child: widget.child,
        ),
  ...
  void show() {
              ...
              child: TapRegion(
                ...
                child: Stack(
                  children: [
                    ...
                    if (widget.direction == WidgetTooltipDirection.pointer)
                      ValueListenableBuilder(
                        valueListenable: mousePosition,
                        builder: (context, value, child) {
                          return Positioned(
                            top: max(6, value.dy - messageBoxSize.height - 6),
                            left: max(6, value.dx + 0),
                            child: Opacity(opacity: value.dy == 0 && value.dx == 0 ? 0 : 1, child: messageBox),
                          );
                        },
                      )
                    else
    ...
    widget.onShow?.call();
    currentTooltipHashcode.value = widget.hashCode;
  }
  ...
  ({Alignment targetAnchor, Alignment followerAnchor, Offset offset})? _builder(Size messageBoxSize) {
    ...
    return (targetAnchor: targetAnchor, followerAnchor: followerAnchor, offset: Offset(dx, dy));
  }
}
