  ...
  switch (shapeType) {
    ...
    case ShapeMarkerType.verticalLine:
      return _processVerticalLineShape(
          canvas: canvas, rect: rect, path: path, isNeedToReturnPath: isNeedToReturnPath, paint: borderPaint, shaderPaint: paint.shader != null ? paint : null);
    ...
    case ShapeMarkerType.splineRangeAreaSeries:
      return _processSplineAreaShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    ...
    case ShapeMarkerType.stackedArea100Series:
      return _processAreaShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    case ShapeMarkerType.stepAreaSeries:
      return _processStepAreaShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    ...
    case ShapeMarkerType.bubbleSeries:
      return _processBubbleShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    ...
    case ShapeMarkerType.histogramSeries:
      return _processColumnShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    case ShapeMarkerType.barSeries:
    case ShapeMarkerType.stackedBarSeries:
    case ShapeMarkerType.stackedBar100Series:
      return _processBarShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    case ShapeMarkerType.hiloSeries:
      return _processHiloShape(
          canvas: canvas, rect: rect, path: path, paint: borderPaint, shaderPaint: paint.shader != null ? paint : null, isNeedToReturnPath: isNeedToReturnPath);
    ...
    case ShapeMarkerType.boxAndWhiskerSeries:
      return _processWaterfallShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    case ShapeMarkerType.pieSeries:
      return _processPieShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    case ShapeMarkerType.doughnutSeries:
      return _processDoughnutShape(
          canvas: canvas, rect: rect, radius: radius, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    ...
    case ShapeMarkerType.pyramidSeries:
      return _processPyramidShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
    case ShapeMarkerType.funnelSeries:
      return _processFunnelShape(canvas: canvas, rect: rect, path: path, paint: paint, borderPaint: borderPaint, isNeedToReturnPath: isNeedToReturnPath);
...
Path _processPentagonShape(
  ...
  for (int i = 0; i <= numberOfSides; i++) {
    ...
    i == 0 ? path.moveTo(cos(angle) * radius + left, sin(angle) * radius + top) : path.lineTo(cos(angle) * radius + left, sin(angle) * radius + top);
  }
...
Path _processVerticalLineShape(
    {required Canvas canvas, required bool isNeedToReturnPath, required Path path, required Rect rect, required Paint? paint, Paint? shaderPaint}) {
  final double left = rect.left + rect.width / 2;
  final double top = rect.top + rect.height / 2;
...
Path _processHorizontalLineShape(
    {required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, required Paint? paint, Paint? shaderPaint}) {
  final double left = rect.left + rect.width / 2;
  final double top = rect.top + rect.height / 2;
...
Path _processStepLineShape(
    {required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? shaderPaint, bool? isDashArray}) {
  ...
  if (paint != null) {
    paint.shader = shaderPaint != null ? shaderPaint.shader : paint.shader;
    canvas.drawPath(
        isDashArray! ? _processDashPath(path, dashArray: _CircularIntervalList<double>(<double>[3, 2])) : path, paint..style = PaintingStyle.stroke);
  }
...
Path _processPieShape({required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? borderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
  final double height = rect.height;
  ...
  path.moveTo(x, y);
  path.lineTo(x + r, y);
  path.arcTo(Rect.fromCircle(center: Offset(x, y), radius: r), _degreesToRadians(0), _degreesToRadians(270), false);
  path.close();
  path.moveTo(x + width / 10, y - height / 10);
  path.lineTo(x + r, y - height / 10);
  path.arcTo(Rect.fromCircle(center: Offset(x + 2, y - 2), radius: r), _degreesToRadians(-5), _degreesToRadians(-80), false);
  path.close();
...
Path _processDoughnutShape(
    {required Canvas canvas, required Rect rect, required Path path, required bool isNeedToReturnPath, double? radius, Paint? paint, Paint? borderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  ...
  if (isNeedToReturnPath) {
    if (borderPaint != null) {
      path1 = _getArcPath(path, radius / 4, radius / 2, Offset(x, y), 0, 270, 270, true);
    } else {
      path2 = _getArcPath(path, radius / 4, radius / 2, Offset(x + 1, y - 1), -5, -85, -85, true);
    }
  ...
  if (borderPaint != null) {
    canvas.drawPath(path1, borderPaint..color = Colors.grey.shade300.withValues(alpha: 0.5));
  }
  canvas.drawPath(path2, paint);
  if (borderPaint != null) {
    canvas.drawPath(path2, borderPaint..color = Colors.grey.shade300.withValues(alpha: 0.5));
  }
...
Path _processRadialBarShape(
  ...
  if (isNeedToReturnPath) {
    if (borderPaint != null) {
      path1 = _getArcPath(path, (radius / 2) - 2, radius / 2, Offset(x, y), 0, 360 - 0.01, 360 - 0.01, true);
    } else {
      path2 = _getArcPath(path, (radius / 2) - 2, radius / 2, Offset(x, y), startAngle!, endAngle!, degree!, true);
    }
    return path;
  }
...
/// Get the path of arc
Path _getArcPath(Path path, double innerRadius, double radius, Offset center, double startAngle, double endAngle, double degree, bool isAnimate) {
  startAngle = _degreesToRadians(startAngle);
  endAngle = _degreesToRadians(endAngle);
  degree = _degreesToRadians(degree);

  final Point<double> innerRadiusStartPoint = Point<double>(innerRadius * cos(startAngle) + center.dx, innerRadius * sin(startAngle) + center.dy);
  final Point<double> innerRadiusEndPoint = Point<double>(innerRadius * cos(endAngle) + center.dx, innerRadius * sin(endAngle) + center.dy);

  final Point<double> radiusStartPoint = Point<double>(radius * cos(startAngle) + center.dx, radius * sin(startAngle) + center.dy);
  ...
  if (isFullCircle) {
    path.arcTo(Rect.fromCircle(center: center, radius: radius), startAngle, midpointAngle.toDouble() - startAngle, true);
    path.arcTo(Rect.fromCircle(center: center, radius: radius), midpointAngle.toDouble(), endAngle - midpointAngle.toDouble(), true);
  } else {
    path.lineTo(radiusStartPoint.x, radiusStartPoint.y);
    path.arcTo(Rect.fromCircle(center: center, radius: radius), startAngle, degree, true);
  }

  if (isFullCircle) {
    path.arcTo(Rect.fromCircle(center: center, radius: innerRadius), endAngle, midpointAngle.toDouble() - endAngle, true);
    path.arcTo(Rect.fromCircle(center: center, radius: innerRadius), midpointAngle.toDouble(), startAngle - midpointAngle.toDouble(), true);
  } else {
    path.lineTo(innerRadiusEndPoint.x, innerRadiusEndPoint.y);
    path.arcTo(Rect.fromCircle(center: center, radius: innerRadius), endAngle, startAngle - endAngle, true);
    path.lineTo(radiusStartPoint.x, radiusStartPoint.y);
  }
  return path;
}
...
Path _processHiloShape({required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? shaderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double height = rect.height;
...
Path _processHiloOpenCloseShape(
    {required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? shaderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
...
Path _processWaterfallShape(
    {required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? borderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
  final double height = rect.height;
...
Path _processFunnelShape({required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? borderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
  final double height = rect.height;
...
Path _processBubbleShape({required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? borderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
  final double height = rect.height;

  path.addArc(Rect.fromLTWH(x - width / 2, y - height / 2, width, height), 0.0, 2 * pi);
...
Path _processSplineAreaShape(
  ...
  path.quadraticBezierTo(x + width / 2, y - height / 2, x + width / 2, y + height / 2);
...
Path _processLineShape(
  ...
  if (isNeedMarker! && overlayMarkerType != null) {
    final Rect pathRect = Rect.fromCenter(center: rect.center, width: rect.width / 1.5, height: rect.height / 1.5);
    canvas.drawPath(getShapesPath(rect: pathRect, shapeType: overlayMarkerType), Paint()..color = paint!.color);
  }
...
Path _processColumnShape({required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? borderPaint}) {
  final double left = rect.left + rect.width / 2;
  final double top = rect.top + rect.height / 2;
  ...
  path.lineTo(left + space * (-rect.width / padding), top - (rect.height / temp));
  path.lineTo(left + space * (-rect.width / padding), top + (rect.height / 2));
  path.lineTo(left - space * (rect.width / temp), top + (rect.height / 2));
  path.close();

  path.moveTo(left - (rect.width / padding) - (rect.width / (padding * 2)), top - (rect.height / 4) - (padding / 2));
  path.lineTo(left + (rect.width / padding) + (rect.width / (padding * 2)), top - (rect.height / 4) - (padding / 2));
  path.lineTo(left + (rect.width / padding) + (rect.width / (padding * 2)), top + (rect.height / 2));
  path.lineTo(left - (rect.width / padding) - (rect.width / (padding * 2)), top + (rect.height / 2));
  path.close();
...
Path _processAreaShape({required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? borderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
  final double height = rect.height;
...
Path _processBarShape({required Canvas canvas, required Rect rect, required bool isNeedToReturnPath, required Path path, Paint? paint, Paint? borderPaint}) {
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
  final double height = rect.height;
