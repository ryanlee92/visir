...
class _SfLegendState extends State<SfLegend> {
      ...
      switch (widget.position) {
        ...
        case LegendPosition.left:
          current = Row(children: <Widget>[
            Align(alignment: _getEffectiveLegendItemsAlignment(widget.position, widget.alignment ?? LegendAlignment.center), child: current),
            _buildChild(baseConstraints)
          ]);
          break;
  ...
  Widget? _buildLegend() {
    ...
    // final double scrollbarThickness = defaultTargetPlatform == TargetPlatform.iOS || defaultTargetPlatform == TargetPlatform.android ? 4 : 5;
      ...
      if (widget.overflowMode == LegendOverflowMode.scroll) {
        current = ScrollConfiguration(
          ...
          child: SingleChildScrollView(
            ...
            scrollDirection:
                widget.scrollDirection ?? (widget.position == LegendPosition.top || widget.position == LegendPosition.bottom ? Axis.horizontal : Axis.vertical),
            child: current,
          ),
        );
                      ...
                      child: ScrollConfiguration(
                        ...
                        child: SingleChildScrollView(
                          ...
                          scrollDirection: widget.scrollDirection ??
                              (widget.position == LegendPosition.top || widget.position == LegendPosition.bottom ? Axis.horizontal : Axis.vertical),
                          child: current,
                        ),
                      ),
                    )
            ...
            Flexible(
              child: widget.overflowMode == LegendOverflowMode.scroll
                  ...
                  : widget.overflowMode == LegendOverflowMode.wrapScroll
                      ? SingleChildScrollView(scrollDirection: widget.direction == Axis.horizontal ? Axis.vertical : Axis.horizontal, child: current)
  ...
  EdgeInsetsGeometry _getEffectiveEdgeInsets() {
    ...
    switch (legendPosition) {
      case LegendPosition.top:
        return EdgeInsets.only(left: offset.dx > 0 ? offset.dx * 2 : 0, right: offset.dx < 0 ? offset.dx.abs() * 2 : 0, top: offset.dy > 0 ? offset.dy : 0);
      case LegendPosition.left:
        return EdgeInsets.only(top: offset.dy > 0 ? offset.dy * 2 : 0, bottom: offset.dy < 0 ? offset.dy.abs() * 2 : 0, left: offset.dx > 0 ? offset.dx : 0);
      case LegendPosition.right:
        return EdgeInsets.only(
            top: offset.dy > 0 ? offset.dy * 2 : 0, bottom: offset.dy < 0 ? offset.dy.abs() * 2 : 0, right: offset.dx < 0 ? offset.dx.abs() : 0);
      case LegendPosition.bottom:
        return EdgeInsets.only(
            left: offset.dx > 0 ? offset.dx * 2 : 0, right: offset.dx < 0 ? offset.dx.abs() * 2 : 0, bottom: offset.dy < 0 ? offset.dy.abs() : 0);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.itemBuilder == null) {
      _textStyle = themeData.textTheme.bodySmall!.copyWith(color: themeData.textTheme.bodySmall!.color!.withValues(alpha: 0.87)).merge(widget.textStyle);
    }
    if (!widget.isComplex) {
...
class _VectorLegendState extends State<_VectorLegend> with SingleTickerProviderStateMixin {
  ...
  Color? _getEffectiveToggledColor(ThemeData themeData) {
    ...
    if (widget.onToggledIndicesChanged != null) {
      ...
      if (toggledColor == null || toggledColor == Colors.transparent) {
        toggledColor = themeData.brightness == Brightness.light ? const Color.fromRGBO(230, 230, 230, 1) : const Color.fromRGBO(66, 66, 66, 1);
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final Widget current = Wrap(
      direction: widget.direction ?? (widget.position == LegendPosition.top || widget.position == LegendPosition.bottom ? Axis.horizontal : Axis.vertical),
              ...
              Text(
                ...
                style: widget.textStyle!.copyWith(
                  color: widget.textStyle!.foreground == null
                      ? widget.textStyle!.color!.withValues(alpha: _opacityTween.evaluate(_toggleAnimation))
...
class _SolidBarLegendState extends State<_SolidBarLegend> {
  ...
  Widget build(BuildContext context) {
    ...
    _direction = widget.direction ?? (widget.position == LegendPosition.top || widget.position == LegendPosition.bottom ? Axis.horizontal : Axis.vertical);
    _textDirection = textDirection == TextDirection.ltr ? textDirection : (_direction == Axis.vertical ? TextDirection.ltr : textDirection);
  ...
  String _getTrimmedText(String currentText, String? nextText, int index, int length) {
    if (widget.labelOverflow == LegendLabelOverflow.visible || currentText.isEmpty || (nextText != null && nextText.isEmpty) || nextText == null) {
      return currentText;
    }
    ...
    if (_direction == Axis.horizontal && widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
      ...
      if (index == length - 1) {
        ...
        refNextTextWidth =
            index + 1 == length - 1 && widget.edgeLabelsPlacement == LegendEdgeLabelsPlacement.inside ? _getTextWidth(nextText) : _getTextWidth(nextText) / 2;
      }
      ...
      if (widget.labelOverflow == LegendLabelOverflow.hide && _isOverlapSegmentText) {
        ...
        return _getTrimText(
            currentText, widget.textStyle!, _segmentSize.width + widget.itemSpacing! / 2, _textPainter, textWidth, refNextTextWidth, isLastInsideItem);
      }
    }
...
class __SolidBarLegendItemState extends State<_SolidBarLegendItem> {
  ...
  void _rebuild() {
    if (widget.pointerController!.segmentIndex == widget.index || widget.pointerController!.previousSegmentIndex == widget.index) {
      setState(() {
  ...
  Offset _getHorizontalTextOffset(int index, String text, int dataSourceLength) {
    ...
    if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
      final double width =
          _textDirection == TextDirection.rtl && widget.segmentSize!.width < _textPainter.width ? _textPainter.width : widget.segmentSize!.width;
    ...
    return Directionality(
      ...
      child: offset != Offset.zero
          ...
          : Text(
              ...
              softWrap: false,
              overflow: widget.labelOverflow == LegendLabelOverflow.ellipsis && widget.labelsPlacement == LegendLabelsPlacement.onItem
  ...
  Widget _getTextWidget(String? startText, String text) {
    ...
    if (widget.index == 0 && startText != null) {
      ...
      startTextOffset = _textDirection == TextDirection.rtl && widget.direction == Axis.horizontal ? -startTextOffset : startTextOffset;
    }
  ...
  Widget _buildPointer() {
    ...
    if (widget.index == widget.pointerController!.segmentIndex && widget.pointerController!.position != null) {
      if (widget.pointerBuilder != null && widget.pointerController!.colorValue != null) {
        ...
        current = RepaintBoundary(
          child: CustomPaint(
            size: widget.pointerSize!,
            painter: _LegendIconShape(
              color: widget.pointerColor ??
                  (Theme.of(context).brightness == Brightness.light ? const Color.fromRGBO(0, 0, 0, 0.54) : const Color.fromRGBO(255, 255, 255, 0.7)),
      ...
      if (widget.direction == Axis.horizontal) {
        matrix4 = Matrix4.identity()..translate(widget.pointerController!.position!.dx * widget.segmentSize!.width - (widget.pointerSize!.width / 2));
        ...
        matrix4 = Matrix4.identity()..translate(0.0, widget.pointerController!.position!.dy * widget.segmentSize!.width - (widget.pointerSize!.width / 2));
        current = Transform(transform: matrix4, child: current);
      }
    } else {
    ...
    if (widget.direction == Axis.horizontal) {
      return SizedBox(
        ...
        child: Column(
          crossAxisAlignment: _getCrossAxisAlignment(),
          children: <Widget>[
            Align(alignment: _textDirection == TextDirection.ltr ? Alignment.centerLeft : Alignment.centerRight, child: _buildPointer()),
            Padding(
...
class _GradientBarLegendState extends State<_GradientBarLegend> {
  ...
  void _updateSegmentSize(double shortestSide) {
    if (_direction == Axis.horizontal) {
      ...
      _segmentSize = widget.segmentSize == null
          ...
          : Size(widget.segmentSize!.width > availableWidth ? availableWidth : widget.segmentSize!.width, widget.segmentSize!.height);
      return;
    }
    ...
    _segmentSize = widget.segmentSize == null
        ...
        : Size(widget.segmentSize!.width, widget.segmentSize!.height > availableHeight ? availableHeight : widget.segmentSize!.height);
  }
  ...
  void _collectLabelsAndColors() {
    ...
    if (widget.items != null) {
      ...
      final double slab =
          _referenceArea / (widget.labelsPlacement == LegendLabelsPlacement.betweenItems && widget.items![0].text[0] != '{' ? length - 1 : length);
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (widget.items![0].text[0] == '{' && widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
          ...
          _labels.add(_GradientBarLabel(text, _getTextOffset(text, positionIndex, length - 1, slab), _isOverlapSegmentText));
        }
  ...
  void _collectRageColorMapperLabels(int i, LegendItem item, String text, double slab, int length) {
    if (i == 0 && widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
      ...
      if (_direction == Axis.horizontal && widget.labelsPlacement == LegendLabelsPlacement.betweenItems && startText.isNotEmpty && text.isNotEmpty) {
        final double refCurrentTextWidth =
            widget.edgeLabelsPlacement == LegendEdgeLabelsPlacement.inside ? _getTextWidth(startText) : _getTextWidth(startText) / 2;
  ...
  String _getTrimmedText(String currentText, int index, int length, double slab, [String? nextText]) {
    ...
    if (_direction == Axis.horizontal && widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
      ...
      if (widget.labelOverflow == LegendLabelOverflow.ellipsis && _isOverlapSegmentText) {
        if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
          final double textWidth = refCurrentTextWidth + refNextTextWidth;
          return _getTrimText(currentText, widget.textStyle!, slab, _textPainter, textWidth, refNextTextWidth, isLastInsideItem);
        }
      }
  ...
  Offset _getTextOffset(String? text, int positionIndex, int length, double slab) {
    _textPainter.text = TextSpan(text: text, style: widget.textStyle);
    _textPainter.layout();
    final bool canAdjustLabelToCenter = widget.edgeLabelsPlacement == LegendEdgeLabelsPlacement.center && (positionIndex == 0 || positionIndex == length) ||
    ...
    if (_direction == Axis.horizontal) {
      ...
      final double referenceTextWidth = canAdjustLabelToCenter ? _textPainter.height / 2 : (positionIndex == length ? _textPainter.height : 0.0);
      if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
  ...
  Offset _getHorizontalOffset(bool canAdjustLabelToCenter, int positionIndex, double slab, int length) {
    if (_isRTL) {
      final double referenceTextWidth = canAdjustLabelToCenter ? -_textPainter.width / 2 : (positionIndex == 0 ? -_textPainter.width : 0.0);
    ...
    final double referenceTextWidth = canAdjustLabelToCenter ? _textPainter.width / 2 : (positionIndex == length ? _textPainter.width : 0.0);
    if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
  ...
  Widget _buildPointer(ThemeData themeData) {
    ...
    if (widget.pointerController!.position != null) {
      if (widget.pointerBuilder != null && widget.pointerController!.colorValue != null) {
        ...
        current = RepaintBoundary(
          child: CustomPaint(
            ...
            painter: _LegendIconShape(
              color: widget.pointerColor ??
                  (themeData.brightness == Brightness.light ? const Color.fromRGBO(0, 0, 0, 0.54) : const Color.fromRGBO(255, 255, 255, 0.7)),
              iconType: ShapeMarkerType.invertedTriangle,
            ),
          ),
        );
      ...
      if (_direction == Axis.horizontal) {
        matrix4 = Matrix4.identity()..translate(widget.pointerController!.position!.dx * _segmentSize.width - (widget.pointerSize!.width / 2));
        if (_isRTL) {
          matrix4.invert();
        }
        current = Transform(transform: matrix4, child: current);
        ...
        current = RotatedBox(quarterTurns: 3, child: current);
        matrix4 = Matrix4.identity()..translate(0.0, widget.pointerController!.position!.dy * _segmentSize.height - (widget.pointerSize!.width / 2));
        current = Transform(transform: matrix4, child: current);
      }
    } else {
  ...
  Widget _getLabels() {
    return Stack(
      ...
      children: List<Widget>.generate(_labels.length, (int index) {
        if ((widget.labelOverflow == LegendLabelOverflow.hide && _labels[index].isOverlapping) || _labels[index].label.isEmpty) {
  ...
  Widget build(BuildContext context) {
    ...
    _textPainter = TextPainter(textDirection: TextDirection.ltr, textScaleFactor: MediaQuery.of(context).textScaleFactor);
    _direction = widget.direction ?? (widget.position == LegendPosition.top || widget.position == LegendPosition.bottom ? Axis.horizontal : Axis.vertical);
