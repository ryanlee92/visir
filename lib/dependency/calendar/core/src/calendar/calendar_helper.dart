...
dynamic addDuration(dynamic date, Duration duration) {
  ...
  if (date.timeZoneOffset != currentDate.timeZoneOffset) {
    currentDate = currentDate.add(date.timeZoneOffset - currentDate.timeZoneOffset);
  }
...
dynamic getPreviousMonthDate(dynamic date) {
  if (date is HijriDateTime) {
    return date.month == 1 ? HijriDateTime(date.year - 1, 12, 01) : HijriDateTime(date.year, date.month - 1, 1);
  }
  return date.month == 1 ? DateTime(date.year - 1, 12) : DateTime(date.year, date.month - 1);
}

/// Returns the next month start date for the given date..
dynamic getNextMonthDate(dynamic date) {
  if (date is HijriDateTime) {
    return date.month == 12 ? HijriDateTime(date.year + 1, 01, 01) : HijriDateTime(date.year, date.month + 1, 1);
  }
  return date.month == 12 ? DateTime(date.year + 1) : DateTime(date.year, date.month + 1);
}
...
List getVisibleDates(dynamic date, List<int>? nonWorkingDays, int firstDayOfWeek, int visibleDatesCount) {
  // ignore: always_specify_types, strict_raw_type
  ...
  final dynamic currentDate = getFirstDayOfWeekDate(visibleDatesCount + nonWorkingDaysCount, date, firstDayOfWeek);

  for (int i = 0; i < visibleDatesCount; i++) {
    final dynamic visibleDate = addDays(currentDate, i);
    if (nonWorkingDays != null && nonWorkingDays.contains(visibleDate.weekday)) {
      visibleDatesCount++;
      continue;
    }
...
/// week value.
dynamic getFirstDayOfWeekDate(int visibleDatesCount, dynamic date, int firstDayOfWeek) {
  if (visibleDatesCount % 7 != 0) {
    return date;
  }

  if (visibleDatesCount == 7) {
    return date;
  }
