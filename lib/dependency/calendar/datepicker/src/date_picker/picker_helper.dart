...
class DateRangePickerHelper {
  ...
  static int getViewDatesCount(DateRangePickerView pickerView, int numberOfWeeks, bool isHijri) {
  ...
  static bool isRangeEquals(dynamic range1, dynamic range2) {
    ...
    if (range1 == range2 || (isSameDate(range1.startDate, range2.startDate) && isSameDate(range1.endDate, range2.endDate))) {
      return true;
    }
  ...
  static bool isDateRangesEquals(List<dynamic>? rangeCollection1, List<dynamic>? rangeCollection2) {
    if (rangeCollection1 == rangeCollection2) {
      return true;
    }

    if ((rangeCollection1 == null && rangeCollection2 != null && rangeCollection2.isEmpty) ||
        (rangeCollection2 == null && rangeCollection1 != null && rangeCollection1.isEmpty)) {
      return true;
    }
  ...
  static dynamic getNextViewStartDate(DateRangePickerView pickerView, int numberOfWeeksInView, dynamic date, bool isRtl, bool isHijri) {
    if (isRtl) {
      return getPreviousViewStartDate(pickerView, numberOfWeeksInView, date, false, isHijri);
    }

    switch (pickerView) {
      case DateRangePickerView.month:
        {
          return isHijri || numberOfWeeksInView == 6 ? getNextMonthDate(date) : addDays(date, numberOfWeeksInView * DateTime.daysPerWeek);
        }
  ...
  static dynamic getPreviousViewStartDate(DateRangePickerView pickerView, int numberOfWeeksInView, dynamic date, bool isRtl, bool isHijri) {
    if (isRtl) {
      return getNextViewStartDate(pickerView, numberOfWeeksInView, date, false, isHijri);
    }

    switch (pickerView) {
      case DateRangePickerView.month:
        {
          return isHijri || numberOfWeeksInView == 6 ? getPreviousMonthDate(date) : addDays(date, -numberOfWeeksInView * DateTime.daysPerWeek);
        }
      case DateRangePickerView.year:
        {
  ...
  static bool isDateCollectionEquals(List<dynamic>? datesCollection1, List<dynamic>? datesCollection2) {
    if (datesCollection1 == datesCollection2) {
      return true;
    }

    if ((datesCollection1 == null && datesCollection2 != null && datesCollection2.isEmpty) ||
        (datesCollection2 == null && datesCollection1 != null && datesCollection1.isEmpty)) {
      return false;
    }
  ...
  static bool isEnabledDate(dynamic startDate, dynamic endDate, bool enablePastDates, dynamic date, bool isHijri) {
    return isDateWithInDateRange(startDate, endDate, date) &&
        (enablePastDates || (!enablePastDates && isDateWithInDateRange(getToday(isHijri), endDate, date)));
  }

  /// Return the first date of the month, year and decade based on view.
  /// Note: This method not applicable for month view.
  static dynamic getFirstDate(dynamic date, bool isHijri, DateRangePickerView pickerView) {
    if (pickerView == DateRangePickerView.month) {
      return date;
    }
    ...
    if (pickerView == DateRangePickerView.year) {
      ...
      return DateRangePickerHelper.getDate((date.year ~/ 10) * 10, 1, 1, isHijri);
    }
  ...
  static bool isDisableDirectionDate(
      dynamic range, dynamic currentDate, ExtendableRangeSelectionDirection extendableRangeSelectionDirection, DateRangePickerView view, bool isHijri,
    ...
    final dynamic endDate = range.endDate != null ? getFirstDate(range.endDate, isHijri, view) : startDate;
    ...
    switch (extendableRangeSelectionDirection) {
      case ExtendableRangeSelectionDirection.none:
        return !isSameCellDates(startDate, endDate, view) &&
            (!isInBetweenEnabled ||
                (isInBetweenEnabled &&
                    ((startDate.isAfter(currentDateValue) == true && !isSameCellDates(startDate, currentDateValue, view)) ||
                        (endDate.isBefore(currentDateValue) == true && !isSameCellDates(endDate, currentDateValue, view)))));
      case ExtendableRangeSelectionDirection.forward:
        return startDate.isAfter(currentDateValue) == true && !isSameCellDates(startDate, currentDateValue, view);
      case ExtendableRangeSelectionDirection.backward:
        return endDate.isBefore(currentDateValue) == true && !isSameCellDates(endDate, currentDateValue, view);
  ...
  static bool isDateAsCurrentMonthDate(dynamic visibleDate, int rowCount, bool showLeadingAndTrialingDates, dynamic date, bool isHijri) {
    if ((rowCount == 6 && !showLeadingAndTrialingDates || isHijri) && date.month != visibleDate.month) {
      return false;
    }
  ...
  static Map<String, double> getTopAndLeftValues(bool isRtl, double left, double top, double cellWidth, double cellHeight, double width) {
    final Map<String, double> topAndLeft = <String, double>{'left': left, 'top': top};
    if (isRtl) {
  ...
  static bool isDateWithInVisibleDates(List<dynamic> visibleDates, List<dynamic>? dates, dynamic date) {
  ...
  static bool canMoveToPreviousViewRtl(DateRangePickerView view, int numberOfWeeksInView, dynamic minDate, dynamic maxDate, List<dynamic> visibleDates,
      bool isRtl, bool enableMultiView, bool isHijri) {
    if (isRtl) {
      return canMoveToNextView(view, numberOfWeeksInView, maxDate, visibleDates, enableMultiView, isHijri);
    } else {
      return canMoveToPreviousView(view, numberOfWeeksInView, minDate, visibleDates, enableMultiView, isHijri);
    }
  }

  /// Check the right side view have valid dates based on widget direction.
  static bool canMoveToNextViewRtl(DateRangePickerView view, int numberOfWeeksInView, dynamic minDate, dynamic maxDate, List<dynamic> visibleDates, bool isRtl,
      bool enableMultiView, bool isHijri) {
    if (isRtl) {
      return canMoveToPreviousView(view, numberOfWeeksInView, minDate, visibleDates, enableMultiView, isHijri);
    } else {
      return canMoveToNextView(view, numberOfWeeksInView, maxDate, visibleDates, enableMultiView, isHijri);
  ...
  static bool canMoveToPreviousView(
      DateRangePickerView view, int numberOfWeeksInView, dynamic minDate, List<dynamic> visibleDates, bool enableMultiView, bool isHijri) {
    switch (view) {
      case DateRangePickerView.month:
        {
          if (numberOfWeeksInView != 6 && !isHijri) {
            DateTime prevViewDate = DateRangePickerHelper.getDateTimeValue(visibleDates[0]);
            prevViewDate = DateRangePickerHelper.getDateTimeValue(addDays(prevViewDate, -1));
            if (!isSameOrAfterDate(minDate, prevViewDate)) {
              return false;
            }
          } else {
            final dynamic currentDate = visibleDates[visibleDates.length ~/ (enableMultiView ? 4 : 2)];
            final dynamic previousDate = getPreviousMonthDate(currentDate);
            if ((previousDate.month < minDate.month == true && previousDate.year == minDate.year) || previousDate.year < minDate.year == true) {
              return false;
            }
          }
        }
      ...
      case DateRangePickerView.century:
        {
          final int currentYear = visibleDates[visibleDates.length ~/ (enableMultiView ? 4 : 2)].year as int;
  ...
  static bool canMoveToNextView(
      DateRangePickerView view, int numberOfWeeksInView, dynamic maxDate, List<dynamic> visibleDates, bool enableMultiView, bool isHijri) {
    switch (view) {
      case DateRangePickerView.month:
        {
          if (!isHijri && numberOfWeeksInView != 6) {
            DateTime nextViewDate = DateRangePickerHelper.getDateTimeValue(visibleDates[visibleDates.length - 1]);
            nextViewDate = DateRangePickerHelper.getDateTimeValue(addDays(nextViewDate, 1));
            ...
            if ((nextDate.month > maxDate.month == true && nextDate.year == maxDate.year) || nextDate.year > maxDate.year == true) {
              return false;
            }
          }
        }
  ...
  static String getHijriMonthText(dynamic date, SfLocalizations localizations, String format) {
    if (date.month == 1) {
  ...
  static bool canShowLeadingAndTrailingDates(dynamic monthViewSettings, bool isHijri) {
    if (isHijri) {
      return false;
    }
  ...
  /// Note: This method not applicable for month view.
  static bool isLeadingCellDate(int index, int viewStartIndex, List<dynamic> visibleDates, dynamic view) {
    final DateRangePickerView pickerView = getPickerView(view);
    if (pickerView == DateRangePickerView.month || pickerView == DateRangePickerView.year) {
      return false;
    }
  ...
  /// Note: This method not applicable for month view.
  static bool isBetweenMinMaxDateCell(dynamic date, dynamic minDate, dynamic maxDate, bool enablePastDates, dynamic view, bool isHijri) {
    ...
    if (pickerView == DateRangePickerView.year) {
      return ((date.month >= minDate.month == true && date.year == minDate.year) || date.year > minDate.year == true) &&
          ((date.month <= maxDate.month == true && date.year == maxDate.year) || date.year < maxDate.year == true) &&
          (enablePastDates || (!enablePastDates && ((date.month >= today.month == true && date.year == today.year) || date.year > today.year == true)));
    } else if (pickerView == DateRangePickerView.decade) {
      return date.year >= minDate.year == true &&
          date.year <= maxDate.year == true &&
          (enablePastDates || (!enablePastDates && date.year >= today.year == true));
    } else if (pickerView == DateRangePickerView.century) {
      final int currentYear = (date.year as int) ~/ 10;
      return currentYear >= (minDate.year ~/ 10) &&
          currentYear <= (maxDate.year ~/ 10) &&
          (enablePastDates || (!enablePastDates && currentYear >= today.year ~/ 10));
    }
  ...
  static dynamic getLastDate(dynamic date, dynamic view, bool isHijri) {
    ...
    if (pickerView == DateRangePickerView.year) {
      final dynamic currentDate = getDate(date.year, date.month + 1, 1, isHijri);
      return addDays(currentDate, -1);
    } else if (pickerView == DateRangePickerView.decade) {
      final dynamic currentDate = getDate(date.year + 1, 1, 1, isHijri);
      return addDays(currentDate, -1);
    } else if (pickerView == DateRangePickerView.century) {
      final dynamic currentDate = getDate(((date.year ~/ 10) * 10) + 10, 1, 1, isHijri);
      return addDays(currentDate, -1);
    }
  ...
  static int getDateCellIndex(List<dynamic> dates, dynamic date, dynamic view, {int viewStartIndex = -1, int viewEndIndex = -1}) {
    if (date == null) {
      return -1;
    }
  ...
  static int getWeekNumberOfYear(dynamic date, bool isHijri) {
    final dynamic yearEndDate = isHijri ? HijriDateTime(date.year - 1, 12, 31) : DateTime(date.year - 1, 12, 31);
  ...
  static void drawDashedLine(double xPosition, double yPosition, double endXPosition, Canvas canvas, Paint painter) {
    const double dashWidth = 4, dashSpace = 5;
    while (xPosition < endXPosition) {
      canvas.drawLine(Offset(xPosition, yPosition), Offset(xPosition + dashWidth, yPosition), painter);
      xPosition += dashWidth + dashSpace;
    }
  }

  /// Returns the dashed path for teh given path
  static Path getDashedPath(Path sourcePath, bool isStartRange, bool isEndRange, bool isRectSelection) {
    ...
    for (int i = 0; i < sourceMetrics.length; i++) {
      ...
      while (distance < metric.length) {
        ...
        if (canDrawPath) {
          final Path extractedPath = metric.extractPath(distance, distance + length);
          final Rect extractedRect = extractedPath.getBounds();
          if ((isStartRange && extractedRect.right != restrictXPosition) || (isEndRange && extractedRect.left != restrictXPosition)) {
