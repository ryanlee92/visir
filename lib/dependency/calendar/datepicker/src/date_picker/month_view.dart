    ...
    if (widget.cellBuilder != null && _children.isEmpty) {
      ...
      for (int j = 0; j < viewCount; j++) {
        ...
        double viewStartPosition = isVerticalMultiView
            ...
            : (currentViewIndex * width) + (currentViewIndex * widget.multiViewSpacing) + (weekNumberPanelWidth * (viewStartIndex + 1));
        if (widget.isRtl) {
          viewStartPosition =
              isVerticalMultiView ? 0 : (currentViewIndex * width) + (currentViewIndex * webUIPadding) + (weekNumberPanelWidth * currentViewIndex);
        }
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    for (int j = 0; j < viewCount; j++) {
      ...
      double viewXStartPosition = isVerticalMultiView
          ? weekNumberPanelWidth
          : (currentViewIndex * width) + (currentViewIndex * webUIPadding) + (weekNumberPanelWidth * (currentViewIndex + 1));
      if (isRtl) {
        viewXStartPosition =
            isVerticalMultiView ? 0 : (currentViewIndex * width) + (currentViewIndex * webUIPadding) + (weekNumberPanelWidth * currentViewIndex);
      }
  ...
  List<int> getSelectedIndexValues(int viewStartIndex, int viewEndIndex) {
    ...
    if (selectedRanges != null) {
      for (int j = 0; j < selectedRanges!.length; j++) {
        ...
        final List<int> rangeIndex =
            _getSelectedRangeIndex(startDate, endDate, visibleDates, isHijri, monthStartIndex: viewStartIndex, monthEndIndex: viewEndIndex);
...
void _drawSelectedDate(
    Canvas canvas, double radius, double centerXPosition, double cellWidth, double cellHeight, double x, double y, _IMonthView view, double centerYPosition) {
...
void _drawStartAndEndRange(Canvas canvas, _IMonthView view, double cellHeight, double cellWidth, double radius, double centerXPosition, double centerYPosition,
    double x, double y, Color color, double heightDifference, bool isStartRange) {
  switch (view.selectionShape) {
    case DateRangePickerSelectionShape.circle:
      {
        Rect rect;
...
TextStyle _drawBetweenSelection(Canvas canvas, _IMonthView view, double cellWidth, double cellHeight, double radius, double x, double y,
    double heightDifference, TextStyle selectionRangeTextStyle) {
  switch (view.selectionShape) {
    case DateRangePickerSelectionShape.rectangle:
      heightDifference = 1;
      break;
...
List<int> _getSelectedRangeIndex(dynamic startDate, dynamic endDate, List<dynamic> visibleDates, bool isHijri,
...
void _drawFillSelection(Canvas canvas, double x, double y, double width, double height, Paint selectionPainter) {
  ...
  canvas.drawRRect(
      RRect.fromRectAndRadius(
          Rect.fromLTRB(x + padding, y + padding, x + width - padding, y + height - padding), Radius.circular(height / 4 > 10 ? 10 : height / 4)),
      selectionPainter);
}
...
void _drawMonthCellsAndSelection(PaintingContext context, Size size, _IMonthView monthView, double cellWidth, double cellHeight) {
      ...
      for (int i = 0; i < datesCount; i++) {
        ...
        if (weekNumberPanelWidth != 0 &&
            ...
            ((i <= DateTime.daysPerWeek && monthView.visibleDates[viewStartIndex + DateTime.daysPerWeek].month == currentMonth) ||
                (i > DateTime.daysPerWeek &&
                    ...
                    monthView.visibleDates[viewStartIndex + (DateTime.daysPerWeek * 2)].month == currentMonth) ||
        ...
        if (!isDisabledDate &&
            ...
            DateRangePickerHelper.isDisableDirectionDate(
                monthView.selectedRange, date, monthView.extendableRangeSelectionDirection, DateRangePickerView.month, monthView.isHijri,
        ...
        if (weekNumberPanelWidth != 0 &&
            date.weekday == DateTime.monday &&
            (!hideLeadingAndTrailingDates ||
                (hideLeadingAndTrailingDates && (i > (DateTime.daysPerWeek * 2)) && (i < datesCount - (DateTime.daysPerWeek * 2))))) {
          _drawWeekNumber(canvas, size, date, cellHeight, yPosition, weekNumberPanelWidth, monthView, viewStartPosition, viewEndPosition);
        }
        ...
        if (monthView.mouseHoverPosition.value != null && monthView.mouseHoverPosition.value!.offset != null) {
          ...
          if (xPosition <= monthView.mouseHoverPosition.value!.offset!.dx &&
            ...
            canvas.drawRRect(
                RRect.fromRectAndRadius(Rect.fromLTWH(xPosition, yPosition, cellWidth, cellHeight), const Radius.circular(2)), monthView._selectionPainter);
          }
        }
  ...
  for (int j = 0; j < viewCount; j++) {
    ...
    bool isCurrentDate;
    final TextStyle selectionTextStyle = monthView.datePickerTheme.selectionTextStyle!;
    final TextStyle selectedRangeTextStyle = monthView.datePickerTheme.rangeSelectionTextStyle!;
    ...
    if (monthView.mouseHoverPosition.value != null && monthView.mouseHoverPosition.value!.hoveringRange != null) {
      hoveringIndex = _getSelectedRangeIndex(monthView.mouseHoverPosition.value!.hoveringRange.startDate,
          monthView.mouseHoverPosition.value!.hoveringRange.endDate, monthView.visibleDates, monthView.isHijri,
    ...
    for (int i = 0; i < datesCount; i++) {
      ...
      if (weekNumberPanelWidth != 0 &&
          ...
          ((i <= DateTime.daysPerWeek && monthView.visibleDates[viewStartIndex + DateTime.daysPerWeek].month == currentMonthDate.month) ||
              ...
              (i >= datesCount - DateTime.daysPerWeek &&
                  (monthView.visibleDates[viewEndIndex - (DateTime.daysPerWeek - 1)].month == currentMonthDate.month))) &&
          date.weekday == DateTime.monday) {
        _drawWeekNumber(canvas, size, date, cellHeight, yPosition, weekNumberPanelWidth, monthView, viewStartPosition, viewEndPosition);
      }
      ...
      textStyle =
          _updateTextStyle(monthView, isNextMonth, isPreviousMonth, isCurrentDate, isEnableDate, isBlackedDate, isWeekEnd, isSpecialDate, isDisabledDate);
      dateDecoration = _updateDecoration(
          isNextMonth, isPreviousMonth, monthView, isEnableDate, isCurrentDate, isBlackedDate, date, isWeekEnd, isSpecialDate, isDisabledDate);
      ...
      monthView._textPainter.paint(
          canvas, Offset(xPosition + (cellWidth / 2 - monthView._textPainter.width / 2), yPosition + ((cellHeight - monthView._textPainter.height) / 2)));
...
void _drawWeekNumber(Canvas canvas, Size size, dynamic date, double cellHeight, double yPosition, double weekNumberPanelWidth, _IMonthView monthView,
    double viewStartPosition, double viewEndPosition) {
  final String weekNumber = DateRangePickerHelper.getWeekNumberOfYear(date, monthView.isHijri).toString();
  final TextStyle weekNumberTextStyle = monthView.datePickerTheme.weekNumberTextStyle!;
...
void _drawWeekNumberPanel(Canvas canvas, Size size, double weekNumberPanelWidth, _IMonthView monthView, bool isHorizontalMultiView,
  ...
  linePainter.color = monthView.weekNumberStyle.backgroundColor ?? monthView.datePickerTheme.weekNumberBackgroundColor!;
...
void _addRangeHoverEffect(Canvas canvas, double xPosition, double yPosition, _IMonthView monthView, int currentIndex, List<int>? hoveringIndex) {
  ...
  final double radius =
      _getCellRadius(rangeSelectionMonthView.selectionRadius, rangeSelectionMonthView._centerXPosition, rangeSelectionMonthView._centerYPosition);
  ...
  if (isStartRange) {
    switch (monthView.selectionShape) {
      ...
      case DateRangePickerSelectionShape.rectangle:
        ...
        rect = Rect.fromLTRB(
            rightPosition - cornerRadius, yPosition + heightDifference, rightPosition, yPosition + rangeSelectionMonthView._cellHeight - heightDifference);
    }
  } else if (isEndRange) {
    switch (monthView.selectionShape) {
      case DateRangePickerSelectionShape.circle:
        rect = Rect.fromLTRB(xPosition, yPosition + heightDifference, xPosition + rangeSelectionMonthView._centerXPosition,
            yPosition + rangeSelectionMonthView._cellHeight - heightDifference);
        break;
      case DateRangePickerSelectionShape.rectangle:
        {
          rect = Rect.fromLTRB(
              xPosition, yPosition + heightDifference, xPosition + cornerRadius, yPosition + rangeSelectionMonthView._cellHeight - heightDifference);
        }
    }
  } else if (isBetweenRange) {
    rect = Rect.fromLTRB(xPosition, yPosition + heightDifference, xPosition + rangeSelectionMonthView._cellWidth,
        yPosition + rangeSelectionMonthView._cellHeight - heightDifference);
  }
  ...
  monthView._selectionPainter.color =
      monthView.selectionColor != null ? monthView.selectionColor!.withValues(alpha: 0.4) : monthView.datePickerTheme.selectionColor!.withValues(alpha: 0.4);
      ...
      case DateRangePickerSelectionShape.rectangle:
        {
          canvas.drawRRect(
              RRect.fromRectAndRadius(
                  Rect.fromLTWH(xPosition + 1, yPosition + 1, cellWidth - 1, cellHeight - 1), Radius.circular(cellHeight / 4 > 10 ? 10 : cellHeight / 4)),
...
TextStyle _drawCellAndSelection(Canvas canvas, double xPosition, double yPosition, TextStyle selectionTextStyle, TextStyle selectedRangeTextStyle,
...
void _drawDecoration(
    Canvas canvas, double xPosition, double yPosition, double padding, double cellWidth, double cellHeight, Decoration dateDecoration, _IMonthView monthView) {
  final BoxPainter boxPainter = dateDecoration.createBoxPainter(monthView.markNeedsPaint);
  boxPainter.paint(
      canvas, Offset(xPosition + padding, yPosition + padding), ImageConfiguration(size: Size(cellWidth - (2 * padding), cellHeight - (2 * padding))));
}
...
TextStyle _updateTextStyle(_IMonthView monthView, bool isNextMonth, bool isPreviousMonth, bool isCurrentDate, bool isEnableDate, bool isBlackedDate,
    bool isWeekEnd, bool isSpecialDate, bool isDisabledDate) {
  final TextStyle currentDatesTextStyle = monthView.datePickerTheme.activeDatesTextStyle!;
  if (isBlackedDate) {
    return monthView.datePickerTheme.blackoutDatesTextStyle ?? currentDatesTextStyle.copyWith(decoration: TextDecoration.lineThrough);
  }

  if (isSpecialDate) {
    final TextStyle? specialDateTextStyle = monthView.datePickerTheme.specialDatesTextStyle;
...
Decoration? _updateDecoration(bool isNextMonth, bool isPreviousMonth, _IMonthView monthView, bool isEnableDate, bool isCurrentDate, bool isBlackedDate,
    dynamic date, bool isWeekEnd, bool isSpecialDate, bool isDisabledDate) {
