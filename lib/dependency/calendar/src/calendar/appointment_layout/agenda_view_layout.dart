...
class _AgendaViewLayoutState extends State<AgendaViewLayout> {
    ...
    if (_children.isEmpty && widget.appointmentBuilder != null) {
      ...
      for (int i = 0; i < appointmentCount; i++) {
        ...
        final CalendarAppointmentDetails details = CalendarAppointmentDetails(
          widget.selectedDate!,
          List<dynamic>.unmodifiable(<dynamic>[CalendarViewHelper.getAppointmentDetail(view.appointment!, widget.calendar.dataSource)]),
  ...
  void _updateAppointmentDetails() {
    ...
    widget.appointments!.sort((CalendarAppointment app1, CalendarAppointment app2) => app1.actualStartTime.compareTo(app2.actualStartTime));
    ...
    final double agendaItemHeight = CalendarViewHelper.getScheduleAppointmentHeight(widget.monthViewSettings, widget.scheduleViewSettings);
    final double agendaAllDayItemHeight = CalendarViewHelper.getScheduleAllDayAppointmentHeight(widget.monthViewSettings, widget.scheduleViewSettings);
    ...
    for (int i = 0; i < widget.appointments!.length; i++) {
      ...
      final double appointmentHeight = (appointment.isAllDay || isSpanned) && !isLargerScheduleUI ? agendaAllDayItemHeight : agendaItemHeight;
      final Rect rect = Rect.fromLTWH(0, yPosition, widget.width - 8, appointmentHeight);
...
class _AgendaViewRenderObject extends CustomCalendarRenderObject {
    ...
    if (selectedDate == null) {
      semanticsBuilder.add(
        CustomPainterSemantics(
          ...
          properties: const SemanticsProperties(label: 'No selected date', textDirection: TextDirection.ltr),
        ),
      );
  ...
  void _drawDefaultUI(Canvas canvas, bool isLargerScheduleUI, Offset offset) {
    ...
    final TextStyle appointmentTextStyle = monthViewSettings != null
        ? themeData.textTheme.bodyMedium!.copyWith(color: Colors.white, fontSize: 13).merge(monthViewSettings!.agendaStyle.appointmentTextStyle)
        : themeData.textTheme.bodyMedium!
              .copyWith(color: isLargerScheduleUI && calendarTheme.brightness == Brightness.light ? Colors.black87 : Colors.white, fontSize: 13)
              .merge(scheduleViewSettings!.appointmentTextStyle);

    final List<String> appointmentStringFormats = appointmentTimeTextFormat == null
        ? <String>[]
        : CalendarViewHelper.getListFromString(appointmentTimeTextFormat!);
    final List<String> sameDateAppointmentStringFormats = CalendarViewHelper.getListFromString('hh:mm a');
    final List<String> diffDateAppointmentStringFormats = CalendarViewHelper.getListFromString('MMM dd, hh:mm a');
    ...
    for (int i = 0; i < appointmentCollection.length; i++) {
      ...
      if (isLargerScheduleUI) {
        ...
        if (isSpanned) {
          final TextSpan icon = AppointmentHelper.getSpanIcon(appointmentTextStyle.color!, isMobilePlatform ? textSize : textSize / 1.5, !isRTL);
  ...
  double _drawIcon(
    ...
    if (!isLargerScheduleUI) {
      if (isRTL) {
        canvas.drawRRect(
          RRect.fromRectAndRadius(Rect.fromLTRB(rect.left + spanIconWidth, rect.top, rect.left + spanIconWidth + iconSize, rect.bottom), cornerRadius),
          _rectPainter,
        );
      } else {
        canvas.drawRRect(
          RRect.fromRectAndRadius(Rect.fromLTRB(rect.right - spanIconWidth - iconSize, rect.top, rect.right - spanIconWidth, rect.bottom), cornerRadius),
          _rectPainter,
        );
      }
    }
    ...
    if (isRTL) {
      ...
      _textPainter.paint(canvas, Offset(rect.right - _textPainter.width - 8 - spanIconWidth, rect.top + yPosition - iconStartPosition));
    }
  ...
  double _drawNormalAppointmentUI(
    ...
    final List<String> format = appointmentFormatString.isEmpty
        ? (isSameDate(appointment.actualStartTime, appointment.actualEndTime) ? sameDateAppointmentFormatString : diffDateAppointmentFormatString)
        : appointmentFormatString;
    final String startDateText = CalendarViewHelper.getLocalizedString(appointment.actualStartTime, format, locale);
    final String endDateText = CalendarViewHelper.getLocalizedString(appointment.actualEndTime, format, locale);
    final TextSpan span = TextSpan(text: '$startDateText - $endDateText', style: appointmentTextStyle);
    ...
    _textPainter.paint(canvas, Offset(xPosition, yPosition + topPadding + subjectHeight));
  ...
  List<double> _drawSpanningAppointmentForScheduleView(
    ...
    final TextSpan span = TextSpan(text: AppointmentHelper.getSpanAppointmentText(appointment, selectedDate!, localizations), style: appointmentTextStyle);
    ...
    final TextSpan icon = AppointmentHelper.getSpanIcon(appointmentTextStyle.color!, isMobilePlatform ? textSize : textSize / 1.5, !isRTL);
    return <double>[
      _drawIcon(canvas, size, textSize, rect, padding, isLargerScheduleUI, cornerRadius, icon, appointmentHeight, topPadding, true, false, 0),
      topPadding,
    ];
  }
  ...
  void _drawDefaultView(Canvas canvas, Size size, double xPosition, double yPosition, double padding) {
    ...
    _textPainter.paint(canvas, Offset(xPosition + 12, yPosition + 8 + padding));
  }
  ...
  double _addScheduleViewForWeb(
    ...
    canvas.drawCircle(Offset(isRTL ? size.width - circleStartPosition : circleStartPosition, yPosition + centerYPosition), circleRadius, _rectPainter);
    ...
    final double topPadding = (appointmentHeight - _textPainter.height) / 2;
    _textPainter.paint(canvas, Offset(xPosition, yPosition + topPadding));
    final List<String> format = appointmentFormatString.isEmpty ? sameDateAppointmentFormatString : appointmentFormatString;
    final String startDateText = CalendarViewHelper.getLocalizedString(appointment.actualStartTime, format, locale);
    final String endDateText = CalendarViewHelper.getLocalizedString(appointment.actualEndTime, format, locale);
    final TextSpan span = TextSpan(
      text: appointment.isAllDay || appointment.isSpanned ? _localizations.allDayLabel : '$startDateText - $endDateText',
    ...
    _textPainter.paint(canvas, Offset(xPosition, yPosition + ((appointmentHeight - _textPainter.height) / 2)));
    return topPadding;
  }
  ...
  void _addMouseHovering(Canvas canvas, Size size, RRect rect, bool isLargerScheduleUI, double padding) {
    ...
    //             Rect.fromLTWH(rect.left - padding, rect.top + viewPadding, size.width - (isRTL ? viewPadding : padding), rect.height - (2 * viewPadding)),
    //             const Radius.circular(4)),
    //         _rectPainter);
    //   } else {
    //     _rectPainter.color = calendarTheme.selectionBorderColor!.withValues(alpha:0.4);
    //     _rectPainter.style = PaintingStyle.stroke;
    //     _rectPainter.strokeWidth = 2;
    //     if (childCount == 0) {
    //       final Radius cornerRadius = Radius.circular((rect.outerRect.height * 0.1) > 5 ? 5 : (rect.outerRect.height * 0.1));
