...
import 'package:Visir/dependency/calendar/datepicker/datepicker.dart';
...
const double kSchedulePadding = 6;
const String taskCalendarId = 'task';
...
class SfCalendar extends StatefulWidget {
  ...
  SfCalendar({
    ...
    this.appointmentMonthMoreBuilder,
    ...
    this.onDragEnd,
    this.parentSize,
    this.selectionDuration = 60,
    this.selectDateOnScrollAgendaView,
    this.onCreate,
  ...
  final void Function(DateTime targetDate)? selectDateOnScrollAgendaView;
  // final void Function({required Offset globalOffset, required CalendarAppointment appointment})? onUpdateDragAppointment;
  // final void Function()? onEndDragAppointment;

  final int? selectionDuration;

  /// The secondary timezone to display in the calendar time ruler.
  ...
  final CalendarAppointmentMonthMoreBuilder? appointmentMonthMoreBuilder;
  ...
  final AppointmentCreateEndCallback? onCreate;
...
class SfCalendarState extends State<SfCalendar> with SingleTickerProviderStateMixin {
  ...
  List<AppointmentView> _regularAppointmentViewCollection = <AppointmentView>[];
  ...
  /// initial load.
  ValueNotifier<bool> _resourceImageNotifier = ValueNotifier<bool>(false);
  ...
  final GlobalKey<CustomCalendarScrollViewState> _customScrollViewKey = GlobalKey();
  ...
  void initState() {
    ...
    _loadDataBase().then((bool value) {
      _getAppointment();
      checkScrollingThenUpdate();
    });
    ...
    _controller.getCurrentTargetedDisplayDate = _getCurrentTargetedDisplayDate;
    _controller.getCurrentScrollPositionRatio = getCurrentScrollPositionRatio;
    _controller.setCurrentScrollPositionRatio = setCurrentScrollPositionRatio;
    ...
    _controller.hideCreateShadow = _hideCreateShadow;
    _controller.updateColor = _updateCreateColor;
    _controller.updateDate = _updateCreateTime;
    _controller.updateTitle = _updateCreateTitle;
    _controller.updateIsTask = _updateIsTask;
    _controller.showCreateShadow = _showCreateShadow;
    _controller.showDragInboxShadow = _showInboxDragShadow;
    _controller.updateDuration = _updateDuration;
    _controller.isAppointmentCreateViewVisible = _isAppointmentCreateViewVisible;
    _controller.endDragInboxShadow = _endInboxDragShadow;
    _controller.getInboxDragIsAllDay = getInboxDragIsAllDay;
    _controller.getInboxDragDatetime = getInboxDragTime;
    _controller.getCalendarDetailsForId = getCalendarDetailsForId;
    _controller.getCurrentVisibleDates = getCurrentVisibleDates;
    _controller.setProperties(
      selectedDate: _controller.selectedDate ?? widget.initialSelectedDate,
      displayDate: _currentDate,
      view: _controller.view ?? widget.view,
    );
    ...
    if (_view == CalendarView.month && CalendarViewHelper.shouldRaiseViewChangedCallback(widget.onViewChanged)) {
  ...
  void didUpdateWidget(SfCalendar oldWidget) {
    ...
    if (oldWidget.controller != widget.controller) {
      ...
      if (widget.controller != null) {
        ...
        displayDate = widget.controller!.displayDate ?? _currentDate;
        view = widget.controller!.view ?? _view;
        _scheduleDisplayDate = displayDate;
      } else {
      ...
      if (oldWidget.controller!.selectedDate != widget.controller!.selectedDate) {
        ...
        if (_view == CalendarView.month) {
          if (CalendarViewHelper.shouldRaiseViewChangedCallback(widget.onViewChanged)) {
            CalendarViewHelper.raiseViewChangedCallback(widget, <DateTime>[displayDate]);
          }
    ...
    if (oldWidget.controller == widget.controller && widget.controller != null && oldWidget.controller!.displayDate != widget.controller!.displayDate) {
      if (displayDate != null) {
        _currentDate = DateTimeHelper.getDateTimeValue(getValidDate(widget.minDate, widget.maxDate, displayDate));
      }
    ...
    _controller.setProperties(selectedDate: selectedDate, displayDate: displayDate, view: view);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        // NOTE(Ryan): Use Mobile Platform UI
        _isMobilePlatform = CalendarViewHelper.isMobileLayout(Theme.of(context).platform);
        _useMobilePlatformUI = true; //CalendarViewHelper.isMobileLayoutUI(_minWidth, _isMobilePlatform);

        _fadeInController ??= AnimationController(
          duration: Duration(milliseconds: _isMobilePlatform ? 0 : 0),
          vsync: this,
        )..addListener(_updateFadeAnimation);
        _fadeIn ??= Tween<double>(begin: 0.1, end: 1).animate(CurvedAnimation(parent: _fadeInController!, curve: Curves.easeIn));
        ...
        final ValueNotifier<double> agendaHeight = _view == CalendarView.month && widget.monthViewSettings.showAgenda
            ? _getMonthAgendaHeight()
            : ValueNotifier<double>(0);
        final double scheduleHeight = _view == CalendarView.month && widget.parentSize != null ? max(widget.parentSize!.height - 264, 0) : 0;
        height = height - scheduleHeight;

        return VisirButton(
          type: VisirButtonAnimationType.none,
          style: VisirButtonStyle(width: _minWidth, height: _minHeight, backgroundColor: widget.backgroundColor ?? _calendarTheme.backgroundColor),
                  ...
                  children: [
                    Expanded(
                      child: ValueListenableBuilder(
                        valueListenable: agendaHeight,
                        builder: (context, value, child) {
                          return _addChildren(value, height, _minWidth, _isRTL);
                        },
                      ),
                    ),
                    if (_view == CalendarView.month && widget.parentSize != null) addAgenda(scheduleHeight, _isRTL),
                  ],
                ),
          onTap: () {
            _removeDatePicker();
  ...
  void _hideCreateShadow() {
    (_customScrollViewKey.currentState)?.removeAppointmentCreateShadow();
  }

  void _updateCreateTitle(String? title) {
    (_customScrollViewKey.currentState)?.updateCreateTitle(title);
  }

  void _updateIsTask(bool isTask) {
    (_customScrollViewKey.currentState)?.updateCreateIsTask(isTask);
  }

  void _updateCreateColor(Color? color) {
    (_customScrollViewKey.currentState)?.updateCreateColor(color);
  }

  void _updateCreateTime(DateTime startTime, DateTime endTime, bool isAllDay) {
    (_customScrollViewKey.currentState)?.updateCreateTime(startTime, endTime, isAllDay);
  }

  void _showCreateShadow(DateTime startTime, DateTime endTime, bool isAllDay, Offset offset, Function(Rect anchorOffset) onCreated) {
    return (_customScrollViewKey.currentState)?.createAppointmentCreateShadow(startTime, endTime, isAllDay, offset, onCreated);
  }

  void _showInboxDragShadow(Offset offset, Duration duration, Color color, Function(Rect anchorOffset) onCreated, String? title) {
    (_customScrollViewKey.currentState)?.handleInboxDragStart(offset, duration, color, onCreated, title);
  }

  DateTime? getInboxDragTime() {
    return (_customScrollViewKey.currentState)?.getInboxDragTime();
  }

  bool? getInboxDragIsAllDay() {
    return (_customScrollViewKey.currentState)?.getInboxDragIsAllDay();
  }

  void _endInboxDragShadow() {
    (_customScrollViewKey.currentState)?.removeInboxDragShadow();
  }

  void _updateDuration(int minutes) {
    (_customScrollViewKey.currentState)?.updateDuration(minutes);
  }

  bool _isAppointmentCreateViewVisible() {
    return (_customScrollViewKey.currentState)?.isAppointmentCreateViewVisible() ?? false;
  }

  CalendarTapDetails? getCalendarDetailsForId(String id, DateTime date) {
    return (_customScrollViewKey.currentState)?.getCalendarDetailsForId(id, date);
  }

  List<DateTime> getCurrentVisibleDates() {
    return (_customScrollViewKey.currentState)?.getCurrentVisibleDates() ?? [];
  }

  DateTime? _getCurrentTargetedDisplayDate() {
    return (_customScrollViewKey.currentState)?.getCurrentTargetedDisplayDate();
  }

  double? getCurrentScrollPositionRatio() {
    return (_customScrollViewKey.currentState)?.getCurrentScrollPositionRatio();
  }

  void setCurrentScrollPositionRatio(double ratio) {
    return (_customScrollViewKey.currentState)?.setCurrentScrollPositionRatio(ratio);
  }
  ...
  CalendarDetails? _getCalendarDetails(Offset position, Offset? localOffset) {
    if (position.dy < widget.headerHeight) {
      ...
      return CalendarDetails(null, _getTappedHeaderDate(), CalendarElement.header, null, position, localOffset);
    ...
    switch (_controller.view!) {
      ...
      case CalendarView.workWeek:
        {
          return (_customScrollViewKey.currentWidget! as CustomCalendarScrollView).getCalendarDetails(updatedPosition, localOffset);
        }
      case CalendarView.month:
        {
          if (widget.monthViewSettings.showAgenda) {
            final double agendaHeight = _getMonthAgendaHeight().value;
            if (position.dy > _minHeight - agendaHeight) {
              DateTime? currentSelectedDate;
              if (_selectedDate != null) {
                currentSelectedDate =
                    isDateWithInDateRange(widget.minDate, widget.maxDate, _selectedDate) &&
              ...
              if (currentSelectedDate == null) {
                return CalendarDetails(null, null, CalendarElement.agenda, null, position, localOffset);
              }

              final List<dynamic> selectedAppointments = _getSelectedAppointments(
                Offset(position.dx, position.dy - (_minHeight - agendaHeight)),
                currentSelectedDate,
              );
              ...
              return CalendarDetails(
                ...
                currentSelectedDate,
                selectedAppointments.isNotEmpty ? CalendarElement.appointment : CalendarElement.agenda,
                null,
                position,
                localOffset,
              );
      ...
      case CalendarView.timelineMonth:
        {
          ...
          if ((!_isRTL && updatedPosition.dx < resourceViewSize) || (_isRTL && updatedPosition.dx > _minWidth - resourceViewSize)) {
            ...
            return CalendarDetails(resourceAppointments, null, CalendarElement.resourceHeader, resource, position, localOffset);
          }
      ...
      case CalendarView.schedule:
        return _getScheduleViewDetails(updatedPosition, localOffset);
    }
  }

  //// Get the calendar details for schedule view in SfCalendar.
  CalendarDetails? _getScheduleViewDetails(Offset position, Offset? localOffset) {
    ...
    appointmentCollection.sort((CalendarAppointment app1, CalendarAppointment app2) {
      return AppointmentHelper.sortAppintment(app1, app2);
    });

    /// Get the collection of appointment collection listed by date.
    final Map<DateTime, List<CalendarAppointment>> dateAppointments = _getAppointmentCollectionOnDateBasis(
    ...
    final double viewTopPadding = _useMobilePlatformUI ? 0 : 0;
    ...
    /// view.
    // final bool isNeedMonthBuilder = false;
    ...
    if (_useMobilePlatformUI) {
      ...
      //     return CalendarDetails(null, DateTime(startDate.year, startDate.month), CalendarElement.header, null, position);
      //   }
      ...
      if (topPosition <= yPosition && topPosition + weekHeaderHeight > yPosition) {
        ...
        return CalendarDetails(null, DateTime(startDate.year, startDate.month, startDate.day), CalendarElement.viewHeader, null, position, localOffset);
      }
    ...
    if (isNeedInBetweenMonthBuilder) {
      ...
      isNextMonthHasNoAppointment =
          ...
          (lastAppointmentDate.month != endDate.month && nextWeekDate != null && nextWeekDate.month == endDate.month && nextWeekDate.year == endDate.year);
    ...
    CalendarDetails triggerDisplayDateDetails(DateTime date) {
      if ((!_isRTL && viewPadding >= position.dx) || (_isRTL && _minWidth - viewPadding < position.dx)) {
        /// Return calendar details while the [getCalendarDetailsAtOffset]
        /// position placed on view header in schedule view.
        return CalendarDetails(null, date, CalendarElement.viewHeader, null, position, localOffset);
      } else {
        /// Return calendar details while the [getCalendarDetailsAtOffset]
        /// position placed on calendar cell in schedule view.
        return CalendarDetails(null, date, CalendarElement.calendarCell, null, position, localOffset);
      }
    }
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      if (!isMonthHeaderAddedOnView && _useMobilePlatformUI && (startDate.month != currentDate.month || startDate.year != currentDate.year)) {
        ...
        if (topPosition <= yPosition && topPosition + monthHeaderHeight > yPosition) {
          /// Return calendar details while the [getCalendarDetailsAtOffset]
          /// position placed on header in schedule view.
          return CalendarDetails(null, DateTime(currentDate.year, currentDate.month), CalendarElement.header, null, position, localOffset);
        }
      ...
      if (topPosition <= yPosition && yPosition < topPosition + panelHeight + appointmentViewPadding + dividerHeight) {
        ...
        currentAppointments.sort(
          (CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isAllDay, app2.isAllDay),
        );
        currentAppointments.sort(
          (CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.orderAppointmentsAscending(app1.isSpanned, app2.isSpanned),
        );
        ...
        if ((!_isRTL && viewPadding >= position.dx) || (_isRTL && _minWidth - viewPadding < position.dx)) {
          ...
          for (int k = 0; k < currentAppointments.length; k++) {
            ...
            final double currentAppointmentHeight =
                (_useMobilePlatformUI && _isAllDayAppointmentView(appointment) ? allDayAppointmentHeight : appointmentViewHeight) + padding;
    ...
    if (_useMobilePlatformUI && isNeedInBetweenMonthBuilder && isNextMonthHasNoAppointment && isSameOrBeforeDate(_maxDate, endDate)) {
      ...
      if (topPosition <= yPosition && topPosition + monthHeaderHeight > yPosition) {
        /// Return calendar details while the [getCalendarDetailsAtOffset]
        /// position placed on header in schedule view.
        return CalendarDetails(null, DateTime(endDate.year, endDate.month), CalendarElement.header, null, position, localOffset);
      }
      topPosition += monthHeaderHeight;
    }
  ...
  SfCalendarThemeData _getThemeDataValue(SfCalendarThemeData calendarThemeData, ThemeData themeData) {
    ...
    return calendarThemeData.copyWith(
      ...
      weekNumberBackgroundColor: calendarThemeData.weekNumberBackgroundColor ?? colorScheme.onSurface.withValues(alpha: 0.04),
      cellBorderColor: calendarThemeData.cellBorderColor ?? colorScheme.onSurface.withValues(alpha: 0.16),
      ...
      trailingDatesTextStyle: themeData.textTheme.bodyMedium!
          .copyWith(color: colorScheme.onSurface.withValues(alpha: 0.54), fontSize: 13)
      ...
      headerTextStyle: themeData.textTheme.bodyLarge!
          .copyWith(color: colorScheme.onSurface.withValues(alpha: 0.87), fontSize: 18, fontWeight: FontWeight.w400)
          // ignore: lines_longer_than_80_chars
      ...
      activeDatesTextStyle: themeData.textTheme.bodyMedium!
          .copyWith(color: colorScheme.onSurface.withValues(alpha: 0.87), fontSize: 13)
          // ignore: lines_longer_than_80_chars
      ...
      timeTextStyle: themeData.textTheme.bodySmall!
          .copyWith(color: colorScheme.onSurface.withValues(alpha: 0.54), fontSize: 10, fontWeight: FontWeight.w500)
          // ignore: lines_longer_than_80_chars
      ...
      viewHeaderDateTextStyle: themeData.textTheme.bodyMedium!
          .copyWith(color: colorScheme.onSurface.withValues(alpha: 0.87), fontSize: 15)
          // ignore: lines_longer_than_80_chars
      ...
      viewHeaderDayTextStyle: themeData.textTheme.bodySmall!
          .copyWith(color: colorScheme.onSurface.withValues(alpha: 0.87), fontSize: 11)
          // ignore: lines_longer_than_80_chars
      ...
      displayNameTextStyle: themeData.textTheme.bodySmall!
          .copyWith(color: colorScheme.onSurface, fontSize: 10, fontWeight: FontWeight.w500)
          // ignore: lines_longer_than_80_chars
      ...
      timeIndicatorTextStyle: themeData.textTheme.bodySmall!
          .copyWith(color: colorScheme.primary, fontSize: 10, fontWeight: FontWeight.w500)
          // ignore: lines_longer_than_80_chars
  ...
  Future<bool> _loadDataBase() async {
    final ByteData byteData = await rootBundle.load('assets/latest_all.tzf');
  ...
  void updateVisibleAppointments() {
    _updateVisibleAppointments();
  }
  ...
  void _updateVisibleAppointments() async {
    ...
    if (_view != CalendarView.schedule) {
      ...
      final dayMargin = _view == CalendarView.day
          ? 1
          : _view == CalendarView.twoDays
          ? 2
          : _view == CalendarView.threeDays
          ? 3
          : _view == CalendarView.fourDays
          ? 4
          : _view == CalendarView.fiveDays
          ? 5
          : _view == CalendarView.sixDays
          ? 6
      ...
      final List<CalendarAppointment> tempVisibleAppointment = AppointmentHelper.getVisibleAppointments(
        viewStartDate.subtract(Duration(days: dayMargin)),
        viewEndDate.add(Duration(days: dayMargin)),
        _appointments,
        widget.timeZone,
        false,
      );
      ...
      //   if (CalendarViewHelper.isCollectionEqual(_currentViewVisibleDates, _prevViewVisibleDates)) {
      //     return;
      //   }
      // }
  ...
  void _handleScheduleViewScrolled() {
    ...
    if (_view != CalendarView.schedule && _view != CalendarView.month) {
      return;
    }
    ...
    if (scrolledPosition >= 0) {
      for (int i = 0; i < _forwardWidgetHeights.length; i++) {
        ...
        if (scrolledPosition >= widgetPosition && scrolledPosition < widgetHeight) {
          ...
          final splitLength = (details?.date ?? []).length;
          if (splitLength == 0) {
            widget.selectDateOnScrollAgendaView?.call(currentViewDate);
          } else {
            for (int i = 0; i < splitLength; i++) {
              if (i == splitLength - 1 && scrolledPosition >= details!.heights[i]) {
                widget.selectDateOnScrollAgendaView?.call(details.date[i]);
              } else if (scrolledPosition >= details!.heights[i] && scrolledPosition < details.heights[i + 1]) {
                widget.selectDateOnScrollAgendaView?.call(details.date[i]);
              }
            }
          }
      ...
      for (int i = 0; i < _backwardWidgetHeights.length; i++) {
        ...
        if (-scrolledPosition > widgetPosition && -scrolledPosition <= widgetHeight) {
          ...
          final DateTime currentViewDate = DateTimeHelper.getDateTimeValue(getValidDate(widget.minDate, widget.maxDate, date));
          _currentDate = currentViewDate;
          // NOTE(Ryan): update selected date on scroll
  ...
  void _calendarValueChangedListener(String property) {
    ...
    if (!CalendarViewHelper.isSameTimeSlot(_tappedDate, _controller.tappedDate)) {
      _tappedDate = _controller.tappedDate;
    }
    ...
    if (_view != _controller.view) {
      ...
      if (_view == CalendarView.schedule) {
        _scheduleDisplayDate = _controller.selectedDate!;
        if (CalendarViewHelper.shouldRaiseViewChangedCallback(widget.onViewChanged)) {
          CalendarViewHelper.raiseViewChangedCallback(widget, <DateTime>[_controller.displayDate!]);
        }
      ...
      final details = UpdateCalendarStateDetails();
      ...
      details.currentViewVisibleDates = getVisibleDates(_currentDate, nonWorkingDays, widget.firstDayOfWeek, visibleDatesCount).cast();
      ...
      details.regularAppointmentViewCollection = _regularAppointmentViewCollection;
  ...
  void _updateDisplayDate() {
    if (!isSameOrAfterDate(widget.minDate, _controller.displayDate)) {
      _controller.setProperties(displayDate: widget.minDate);
      return;
    }

    if (!isSameOrBeforeDate(widget.maxDate, _controller.displayDate)) {
      _controller.setProperties(displayDate: widget.maxDate);
      return;
    }
    ...
    switch (_view) {
      ...
      case CalendarView.month:
        {
          if (isSameDate(_currentDate, _controller.displayDate) ||
              (isDateWithInDateRange(_currentViewVisibleDates[0], _currentViewVisibleDates[_currentViewVisibleDates.length - 1], _controller.displayDate) &&
                  (widget.monthViewSettings.numberOfWeeksInView != 6 ||
                      (widget.monthViewSettings.numberOfWeeksInView == 6 &&
                          _controller.displayDate!.month == _currentViewVisibleDates[_currentViewVisibleDates.length ~/ 2].month)))) {
            _currentDate = _controller.displayDate!;
            return;
          }
  ...
  void checkScrollingThenUpdate() {
    if (!mounted) return;
    if (_customScrollViewKey.currentState?.isHorizontalScrolling == true) return;
    setState(() {});
  }
  ...
  void _updateRegularAppointmentView(List<CalendarAppointment> regularAppointments) {
    final prevCollection = [..._regularAppointmentViewCollection];
    List<AppointmentView> newCollection = [];

    for (int i = 0; i < regularAppointments.length; i++) {
      final appointment = regularAppointments[i];
      final prevAppointmentView = prevCollection
          .where(
            (e) =>
                e.appointment?.id == appointment.id &&
                e.appointment?.actualStartTime.isAtSameMomentAs(appointment.actualStartTime) == true &&
                e.appointment?.actualEndTime.isAtSameMomentAs(appointment.actualEndTime) == true,
          )
          .firstOrNull;

      if (prevAppointmentView == null) {
        ...
        newCollection.add(appointmentView);
      } else {
        prevAppointmentView.canReuse = false;
        prevAppointmentView.appointment = appointment;
        prevAppointmentView.position = -1;
        prevAppointmentView.startIndex = -1;
        prevAppointmentView.endIndex = -1;
        prevAppointmentView.maxPositions = -1;
        newCollection.add(prevAppointmentView);
      }
    }

    _regularAppointmentViewCollection = newCollection;
  }

  void _updateAllDayAppointmentView(List<CalendarAppointment> allDayAppointments) {
    final prevCollection = [..._allDayAppointmentViewCollection];
    List<AppointmentView> newCollection = [];

    for (int i = 0; i < allDayAppointments.length; i++) {
      final appointment = allDayAppointments[i];
      final prevAppointmentView = prevCollection
          .where(
            (e) =>
                e.appointment?.id == appointment.id &&
    ...
    _allDayAppointmentViewCollection = newCollection;
  }

  void _updateAllDayAppointmentViewPosition() {
    for (final AppointmentView appointmentView in _allDayAppointmentViewCollection) {
      if (appointmentView.appointment == null) {
        continue;
      }

      int startIndex = DateTimeHelper.getIndex(_currentViewVisibleDates, appointmentView.appointment!.actualStartTime);
      int endIndex = DateTimeHelper.getIndex(_currentViewVisibleDates, appointmentView.appointment!.actualEndTime) + 1;

      int startPosition = 0;
      int endPosition = 0;

      if (startIndex == -1) {
        final prevDates = _currentViewVisibleDates.map((e) => e.subtract(Duration(days: _currentViewVisibleDates.length))).toList();

        startIndex = DateTimeHelper.getIndex(prevDates, appointmentView.appointment!.actualStartTime);

        if (startIndex == -1) {
          final nextDates = _currentViewVisibleDates.map((e) => e.add(Duration(days: _currentViewVisibleDates.length))).toList();

          startIndex = DateTimeHelper.getIndex(nextDates, appointmentView.appointment!.actualStartTime);
      ...
      if (endIndex == 0) {
        final prevDates = _currentViewVisibleDates.map((e) => e.subtract(Duration(days: _currentViewVisibleDates.length))).toList();
        endIndex = DateTimeHelper.getIndex(prevDates, appointmentView.appointment!.actualEndTime) + 1;

        if (endIndex == 0) {
          final nextDates = _currentViewVisibleDates.map((e) => e.add(Duration(days: _currentViewVisibleDates.length))).toList();
          endIndex = DateTimeHelper.getIndex(nextDates, appointmentView.appointment!.actualEndTime) + 1;

          if (endIndex == 0) {
            endIndex = _currentViewVisibleDates.length;
          } else {
            endPosition = 1;
          }
        } else {
      ...
      if (appointmentView.appointment!.startTime.isBefore(_currentViewVisibleDates.first) &&
          appointmentView.appointment!.endTime.isAfter(_currentViewVisibleDates.last.add(Duration(days: 1)))) {
        startIndex = 0;
        endIndex = _currentViewVisibleDates.length;
      }

      if (startPosition == 0 && appointmentView.appointment!.endTime.isAfter(_currentViewVisibleDates.last.add(Duration(days: 1)))) {
        endIndex = _currentViewVisibleDates.length;
      }

      if (appointmentView.appointment!.startTime.isBefore(_currentViewVisibleDates.first) && endPosition == 0) {
        startIndex = 0;
      }
  ...
  void _updateAllDayAppointmentPositionAndMaxPosition(List<List<AppointmentView>> allDayAppointmentView) {
    final visibleDates = [];
    for (int i = 0; i < _currentViewVisibleDates.length; i++) {
      visibleDates.add(_currentViewVisibleDates[i]);
      visibleDates.add(_currentViewVisibleDates[i].subtract(Duration(days: _currentViewVisibleDates.length)));
      visibleDates.add(_currentViewVisibleDates[i].add(Duration(days: _currentViewVisibleDates.length)));
    }
    ...
    for (int i = 0; i < allDayAppointmentView.length; i++) {
      ...
      intersectingAppointments.sort((app1, app2) {
        return AppointmentHelper.sortAppintment(app1.appointment, app2.appointment);
      });

      for (int j = 0; j < intersectingAppointments.length; j++) {
        final AppointmentView currentView = intersectingAppointments[j];

        if (currentView.position == -1) currentView.position = 0;
        for (int k = 0; k < j; k++) {
      ...
      if (intersectingAppointments.isNotEmpty) {
        final list = intersectingAppointments.where((e) {
          final startDate = visibleDates.first;
          final endDate = visibleDates.last;
          return (e.appointment!.actualStartTime.isBefore(endDate.add(Duration(days: 1))) && !e.appointment!.actualStartTime.isBefore(startDate)) ||
              (e.appointment!.actualEndTime.isBefore(endDate.add(Duration(days: 1))) && !e.appointment!.actualEndTime.isBefore(startDate)) ||
              (e.appointment!.actualStartTime.isBefore(startDate) && !e.appointment!.actualEndTime.isBefore(endDate.add(Duration(days: 1))));
        }).toList();
        final int maxPosition = list.isEmpty
            ? 0
            : list.length == 1
            ? list.first.position + 1
            : list
                      .reduce(
  ...
  void _updateIntersectAppointmentViewCollection(List<List<AppointmentView>> allDayAppointmentView) {
    for (int i = 0; i < _currentViewVisibleDates.length; i++) {
      DateTime date = _currentViewVisibleDates[i].subtract(Duration(days: _currentViewVisibleDates.length));
      ...
      for (int j = 0; j < _allDayAppointmentViewCollection.length; j++) {
        ...
        if (widget.view == CalendarView.month) {
          ...
          if (currentView.appointment != null &&
                  (currentView.appointment!.actualStartTime.isBefore(date.add(Duration(days: 1))) &&
                      !currentView.appointment!.actualStartTime.isBefore(date)) ||
              (currentView.appointment!.actualEndTime.isBefore(date.add(Duration(days: 1))) && !currentView.appointment!.actualEndTime.isBefore(date)) ||
              (currentView.appointment!.actualStartTime.isBefore(date) && !currentView.appointment!.actualEndTime.isBefore(date.add(Duration(days: 1))))) {
            intersectingAppointments.add(currentView);
          }
        }
      }
    ...
    for (int i = 0; i < _currentViewVisibleDates.length; i++) {
      DateTime date = _currentViewVisibleDates[i];
      final List<AppointmentView> intersectingAppointments = <AppointmentView>[];
      for (int j = 0; j < _allDayAppointmentViewCollection.length; j++) {
        final AppointmentView currentView = _allDayAppointmentViewCollection[j];
        if (currentView.appointment == null) {
        ...
        if (widget.view == CalendarView.month) {
          ...
          if ((currentView.appointment!.actualStartTime.isBefore(date.add(Duration(days: 1))) && !currentView.appointment!.actualStartTime.isBefore(date)) ||
              (currentView.appointment!.actualEndTime.isBefore(date.add(Duration(days: 1))) && !currentView.appointment!.actualEndTime.isBefore(date)) ||
              (currentView.appointment!.actualStartTime.isBefore(date) && !currentView.appointment!.actualEndTime.isBefore(date.add(Duration(days: 1))))) {
            intersectingAppointments.add(currentView);
          }
    ...
    for (int i = 0; i < _currentViewVisibleDates.length; i++) {
      DateTime date = _currentViewVisibleDates[i].add(Duration(days: _currentViewVisibleDates.length));
      final List<AppointmentView> intersectingAppointments = <AppointmentView>[];
      for (int j = 0; j < _allDayAppointmentViewCollection.length; j++) {
        final AppointmentView currentView = _allDayAppointmentViewCollection[j];
        if (currentView.appointment == null) {
  ...
  void _updateRegularAppointment() {
    if (_view == CalendarView.month) {
      return;
    }
  ...
  void _updateAllDayAppointment() {
    ...
    //// Update the appointment view collection with visible appointments.
    _updateAllDayAppointmentView(allDayAppointments);
    ...
    _updateAllDayAppointmentPositionAndMaxPosition(allDayAppointmentView);
  ...
  void _updateAllDayPanelHeight() {
    ...
    if (_allDayAppointmentViewCollection.isNotEmpty) {
      final list = _allDayAppointmentViewCollection.where((e) {
        if (e.appointment == null) {
          return false;
        }

        final appointment = e.appointment!;
        final visibleDates = _currentViewVisibleDates;
        return (appointment.actualStartTime.isBefore(visibleDates.last.add(Duration(days: 1))) && !appointment.actualStartTime.isBefore(visibleDates.first)) ||
      ...
      maxPosition = list.isEmpty
          ? -1
          : list.length == 1
          ? list.first.maxPositions
          : list
                .reduce(
                  (AppointmentView currentAppView, AppointmentView nextAppView) =>
  ...
  ValueNotifier<double> _getMonthAgendaHeight() {
    return widget.monthViewSettings.agendaViewHeight == -1
        ? ValueNotifier<double>(_minHeight / 3)
        : widget.monthViewSettings.agendaViewHeight ?? ValueNotifier<double>(0);
  }
  ...
  void _updateMouseHoverPosition(
    ...
    if (localPosition.dy < widget.headerHeight) {
      ...
      if (isResourceEnabled &&
          ((isRTL && localPosition.dx > (_minWidth - widget.resourceViewSettings.size)) || (!isRTL && localPosition.dx < widget.resourceViewSettings.size)) &&
          localPosition.dy > startPosition! &&
          (CalendarViewHelper.shouldRaiseCalendarTapCallback(widget.onTap) || CalendarViewHelper.shouldRaiseCalendarLongPressCallback(widget.onLongPress))) {
  ...
  DateTime _getMaxAppointmentDate(
    ...
    for (int j = 0; j < appointments.length; j++) {
      ...
      final List<DateTime> recursiveDates = RecurrenceHelper.getRecurrenceDateTimeCollection(appointment.recurrenceRule!, appointment.actualStartTime);
  ...
  Widget? _getItem(BuildContext context, int index, bool isRTL, Offset position, Offset? localOffset) {
    /// Assign display date and today date,
    /// schedule display date always hold the minimum date compared to
    /// display date and today date.
    /// schedule current date always hold the maximum date compared to
    ...
    appointmentCollection.sort((CalendarAppointment app1, CalendarAppointment app2) => AppointmentHelper.sortAppintment(app1, app2));

    /// Get the collection of appointment collection listed by date.
    final Map<DateTime, List<CalendarAppointment>> dateAppointments = _getAppointmentCollectionOnDateBasis(appointmentCollection, startDate, endDate, position);
    final List<DateTime> dateAppointmentKeys = dateAppointments.keys.toList();
    const double padding = kSchedulePadding;
    ...
    if (isNeedMonthBuilder && widget.scheduleViewSettings.hideEmptyScheduleWeek && startDate.month != endDate.month) {
      ...
      if ((startDate.month != scheduleDisplayDate.month || startDate.year != scheduleDisplayDate.year) &&
          ...
          (firstAppointmentDate == null || (startDate.month != firstAppointmentDate.month || startDate.year != firstAppointmentDate.year))) {
    ...
    dividerColor = dividerColor.withValues(alpha: dividerColor.opacity * 0.5);
    ...
    if (_useMobilePlatformUI) {
      if (isNeedMonthBuilder) {
        ...
        widgets.add(_getMonthOrWeekHeader(startDate, endDate, isRTL, true, position: position, localOffset: localOffset));
    ...
    for (int i = 0; i < dateAppointmentKeys.length; i++) {
      ...
      void addMonthHeaderView() {
        ...
        if (_useMobilePlatformUI) {
          ...
          widgets.add(_getMonthOrWeekHeader(currentDate, null, isRTL, true, isNeedTopPadding: true, position: position, localOffset: localOffset));
        }
      }
      ...
      void addDisplayOrCurrentDateView({bool isDisplayDate = true}) {
        ...
        if (!_useMobilePlatformUI) {
          widgets.add(Divider(height: dividerHeight, thickness: 1, color: dividerColor));
        }
      ...
      currentAppointments.sort((app1, app2) {
        return AppointmentHelper.sortAppintment(app1, app2);
      });

      //NOTE(Ryan): set date
      scheduleViewDetails.date.add(currentDate);
      scheduleViewDetails.heights.add(viewStartPosition);
        ...
        MouseRegion(
          ...
          child: GestureDetector(
            child: _ScheduleAppointmentView(
              header: IntrinsicWidth(
                child: VisirButton(
                  type: VisirButtonAnimationType.none,
                  style: VisirButtonStyle(),
                  onTap: () => widget.scheduleViewSettings.dayHeaderSettings.onTap?.call(currentDate),
            ...
            onTapUp: (TapUpDetails details) {
              ...
              if (widget.allowViewNavigation &&
                  ((!_isRTL && details.localPosition.dx < viewPadding) || (_isRTL && details.localPosition.dx > _minWidth - viewPadding))) {
                _controller.setProperties(displayDate: currentDate, view: CalendarView.day);
              }
              ...
              _raiseCallbackForScheduleView(currentDate, details.localPosition, details.localPosition, currentAppointments, viewPadding, padding, true);
            },
            onLongPressStart: (LongPressStartDetails details) {
              ...
              _raiseCallbackForScheduleView(currentDate, details.localPosition, details.localPosition, currentAppointments, viewPadding, padding, false);
            },
          ),
        ),
      );
    ...
    if (_useMobilePlatformUI &&
      ...
      widgets.add(_getMonthOrWeekHeader(endDate, endDate, isRTL, true, isNeedTopPadding: true, position: position, localOffset: localOffset));
    }
  ...
  Widget _getMonthOrWeekHeader(
    ...
    if (isMonthLabel) return SizedBox.shrink();
    return RepaintBoundary(
      child: Padding(
        padding: const EdgeInsets.only(left: 28),
        child: VisirListSection(
          onTap: () => isMonthLabel
              ? widget.scheduleViewSettings.monthHeaderSettings.onTap?.call(startDate)
              : widget.scheduleViewSettings.weekHeaderSettings.onTap?.call(startDate),
          titleBuilder: (height, style, subStyle, horizontalSpacing) => TextSpan(
            text: DateFormat('MMM').format(startDate),
            style: style,
            children: isMonthLabel ? null : [TextSpan(text: ' ${DateFormat('dd').format(startDate)} - ${DateFormat('dd').format(endDate!)}', style: subStyle)],
          ),
        ),
      ),
    );
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        child: _ScheduleAppointmentView(
          header: IntrinsicWidth(
            child: VisirButton(
              ...
              onTap: () => widget.scheduleViewSettings.dayHeaderSettings.onTap?.call(currentDisplayDate),
              child: RepaintBoundary(
                child: CustomPaint(
                  painter: _AgendaDateTimePainter(
                    currentDisplayDate,
          ...
          content: Container(
            ...
            child: IntrinsicWidth(
              child: VisirButton(
                type: VisirButtonAnimationType.none,
                style: VisirButtonStyle(),
                onTap: () => widget.scheduleViewSettings.weekHeaderSettings.onTap?.call(currentDisplayDate),
                child: RepaintBoundary(
                  child: CustomPaint(
        ...
        onTapUp: (TapUpDetails details) {
          _removeDatePicker();
          if (widget.allowViewNavigation &&
              ((!_isRTL && details.localPosition.dx < viewHeaderWidth) || (_isRTL && details.localPosition.dx > _minWidth - viewHeaderWidth))) {
            _controller.setProperties(displayDate: currentDisplayDate, view: CalendarView.day);
          }
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    final DateTime scheduleDisplayDate = _tappedDate ?? DateTimeHelper.getDateTimeValue(getValidDate(widget.minDate, widget.maxDate, _scheduleDisplayDate));
    final DateTime scheduleCurrentDate = _tappedDate ?? DateTime.now();
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(
        viewStartDate,
      ...
      const double padding = kSchedulePadding;
      Map<DateTime, List<CalendarAppointment>> dateAppointments = _getAppointmentCollectionOnDateBasis(
        appointmentCollection,
        viewStartDate,
        viewEndDate,
        Offset.zero,
      ...
      while (totalHeight < height && (_previousDates.isNotEmpty || totalHeight == 0)) {
        ...
        if (totalHeight != 0) {
          ...
          dateAppointments = _getAppointmentCollectionOnDateBasis(appointmentCollection, viewStartDate, viewEndDate, Offset.zero);
        ...
        totalHeight =
            ((numberOfEvents + 1) * padding) + ((numberOfEvents - allDayCount) * appointmentViewHeight) + (allDayCount * allDayAppointmentHeight) + labelHeight;
      }
    ...
    //         _getInitialScrollPosition(viewStartDate, viewEndDate, scheduleCurrentDate, appointmentViewHeight, allDayAppointmentHeight);
    ...
    final DateTime viewStartDate = _nextDates.first;
    final DateTime viewEndDate = DateTimeHelper.getDateTimeValue(addDays(viewStartDate, 6));
    // _agendaScrollController?.dispose();

    _getItem(context, 0, isRTL, Offset.zero, Offset.zero);

    final initialScrollPosition =
        _getInitialScrollPosition(
          viewStartDate,
          viewEndDate,
          DateUtils.dateOnly(_tappedDate ?? scheduleCurrentDate),
          appointmentViewHeight,
          allDayAppointmentHeight,
        ) -
        6;

    final prevAgendaScrollController = _agendaScrollController;
    _agendaScrollController = ScrollController(initialScrollOffset: initialScrollPosition)..addListener(_handleScheduleViewScrolled);
    prevAgendaScrollController?.removeListener(_handleScheduleViewScrolled);
    prevAgendaScrollController?.dispose();

    return KeyboardListener(
      focusNode: _focusNode,
      onKeyEvent: _onKeyDown,
      child: Container(
        key: ValueKey(_tappedDate),
        height: height,
        child: Builder(
          builder: (context) {
                ...
                slivers: <Widget>[
                  SliverList(
                    delegate: SliverChildBuilderDelegate((BuildContext context, int index) {
                      ...
                      return _getItem(context, -(index + 1), isRTL, Offset.zero, Offset.zero);
                    }),
                  ),
                  SliverList(
                    delegate: SliverChildBuilderDelegate((BuildContext context, int index) {
                      ...
                      return _getItem(context, index, isRTL, Offset.zero, Offset.zero);
                    }),
  ...
  double _getInitialScrollPosition(
    ...
    if (DateUtils.isSameDay(viewStartDate, scheduleCurrentDate)) {
      return 0;
    }
    ...
    for (int i = 0; i < _forwardWidgetHeights.length; i++) {
      ...
      for (int j = 0; j < (details?.date ?? []).length; j++) {
        DateTime date = details!.date[j];
        bool isAfterOrSame = DateUtils.isSameDay(date, scheduleCurrentDate) || DateUtils.dateOnly(date).isBefore(DateUtils.dateOnly(scheduleCurrentDate));
        bool isBeforeNext = details.date.length > j + 1 ? DateUtils.dateOnly(details.date[j + 1]).isAfter(DateUtils.dateOnly(scheduleCurrentDate)) : true;

        if (isAfterOrSame && isBeforeNext) {
          return details.heights[j];
        }
      }
    }
    ...
    if (_nextDates.isNotEmpty &&
      ...
      while (viewStartDate.isBefore(scheduleDisplayDate) && !isSameDate(viewStartDate, scheduleDisplayDate)) {
        ...
        double todayNewEventHeight =
            !isSameDate(scheduleCurrentDate, scheduleDisplayDate) && isDateWithInDateRange(appStartDate, appEndDate, scheduleCurrentDate)
            ? displayEventHeight
            : 0;
        final List<CalendarAppointment> appointmentCollection = AppointmentHelper.getVisibleAppointments(
          appStartDate,
        ...
        final bool isNeedMonthBuilder =
            _useMobilePlatformUI && ((viewStartDate.month != appEndDate.month || viewStartDate.year != appEndDate.year) || viewStartDate.day == 1);
    ...
    final List<Widget> children = <Widget>[
      ...
      Positioned(
        ...
        child: NotificationListener<OverscrollNotification>(
          ...
          child: CustomScrollView(
            key: _scrollKey,
            physics: const AlwaysScrollableScrollPhysics(parent: BouncingScrollPhysics(parent: RangeMaintainingScrollPhysics())),
            controller: _agendaScrollController,
            center: _scheduleViewKey,
            slivers: <Widget>[
              SliverList(
  ...
  Widget _getCalendarViewPopup() {
    ...
    for (int i = 0; i < allowedViewLength; i++) {
      ...
      children.add(
        IntrinsicWidth(
          child: VisirButton(
            type: VisirButtonAnimationType.scaleAndOpacity,
            style: VisirButtonStyle(padding: const EdgeInsets.symmetric(horizontal: 10.0), height: calendarViewTextHeight, alignment: alignment),
            onTap: () {
              _viewChangeNotifier.value = false;
              _controller.setProperties(view: view);
            },
            child: Text(text, style: isSelected ? style.copyWith(color: todayColor) : style, maxLines: 1),
          ),
        ),
      );
    }
    ...
    final bool isCenterAlignment =
        !_isMobilePlatform && (widget.headerStyle.textAlign == TextAlign.center || widget.headerStyle.textAlign == TextAlign.justify);

    /// Calculate the calendar view button width that placed on header view
    final double calendarViewWidth = _useMobilePlatformUI
        ? iconWidth
        : _getTextWidgetWidth(calendarViews[_view]!, widget.headerHeight, _minWidth - totalArrowWidth, context, style: style).width +
              padding +
              headerIconTextWidth;
    ...
    if (widget.showTodayButton) {
      todayWidth = _useMobilePlatformUI
          ? iconWidth
          : _getTextWidgetWidth(_localizations.todayLabel, widget.headerHeight, _minWidth - totalArrowWidth, context, style: style).width + padding;
      ...
      child: _PopupWidget(
        ...
        child: Container(
          ...
          child: Material(
            ...
            child: SuperListView(padding: EdgeInsets.zero, controller: calendarViewPopupScrollController, children: children),
          ),
        ),
      ),
    );
  ...
  Widget _addResourcePanel(bool isResourceEnabled, double resourceViewSize, double height, bool isRTL) {
    if (!isResourceEnabled) {
      return Positioned(left: 0, right: 0, top: 0, bottom: 0, child: Container());
    }
    ...
    final Widget verticalDivider = VerticalDivider(width: 0.5, thickness: 0.5, color: widget.cellBorderColor ?? _calendarTheme.cellBorderColor);
  ...
  Widget _addCustomScrollView(double top, double resourceViewSize, bool isRTL, bool isResourceEnabled, double width, double height, double agendaHeight) {
    return Positioned(
      ...
      child: CustomCalendarScrollView(
        ...
        _view,
        width - resourceViewSize,
        height - agendaHeight - (widget.view == CalendarView.month ? 0 : scrollViewBottomPadding.bottom),
        _agendaSelectedDate,
        isRTL,
        _locale,
        _calendarTheme,
      ...
      child: _PopupWidget(
        child: Container(
          ...
          decoration: _isMobilePlatform
              ? BoxDecoration(
                  color: _calendarTheme.brightness == Brightness.dark ? Colors.grey[850] : Colors.white,
                  boxShadow: const <BoxShadow>[BoxShadow(offset: Offset(0.0, 3.0), blurRadius: 2.0, color: Color(0x24000000))],
                )
              : BoxDecoration(
                  color: _calendarTheme.brightness == Brightness.dark ? Colors.grey[850] : Colors.white,
                  boxShadow: kElevationToShadow[6],
          ...
          child: SfDateRangePicker(
            ...
            headerStyle: DateRangePickerHeaderStyle(textAlign: _isMobilePlatform ? TextAlign.center : TextAlign.left),
            monthViewSettings: DateRangePickerMonthViewSettings(viewHeaderHeight: pickerHeight / 8, firstDayOfWeek: widget.firstDayOfWeek),
  ...
  void _updateCalendarState(UpdateCalendarStateDetails details) {
    ...
    if (!ListEquality().equals(_currentViewVisibleDates, details.currentViewVisibleDates)) {
      // if (!DateUtils.isSameDay(_currentViewVisibleDates.first, details.currentViewVisibleDates.first)) {
    ...
    _controller.setProperties(displayDate: displayDate, selectedDate: selectedDate);
  }
  ...
  void _handleOnTapForHeader(double width) {
    ...
    CalendarViewHelper.raiseCalendarTapCallback(widget, _getTappedHeaderDate(), null, CalendarElement.header, null, Offset.zero, Offset.zero, null);
  }
  ...
  void _handleOnLongPressForHeader(double width) {
    ...
    CalendarViewHelper.raiseCalendarLongPressCallback(widget, _getTappedHeaderDate(), null, CalendarElement.header, null, Offset.zero, Offset.zero);
  }
  ...
  void _handleTapForAgenda(TapUpDetails details, DateTime? selectedDate) {
    _removeDatePicker();
    if (widget.allowViewNavigation &&
        ((!_isRTL && details.localPosition.dx < _agendaDateViewWidth) || (_isRTL && details.localPosition.dx > _minWidth - _agendaDateViewWidth))) {
      _controller.setProperties(displayDate: selectedDate, view: CalendarView.day);
    }
  ...
  Widget _addAgendaView(double height, double startPosition, double width, bool isRTL) {
    ...
    if (_selectedDate != null) {
      currentSelectedDate =
          isDateWithInDateRange(widget.minDate, widget.maxDate, _selectedDate) && !CalendarViewHelper.isDateInDateCollection(_blackoutDates, _selectedDate!)
          ? _selectedDate
          : null;
    }
    ...
    if (agendaAppointments.isNotEmpty) {
      ...
      painterHeight =
          ((count * (allDayAppointmentHeight + topPadding)) + ((agendaAppointments.length - count) * (appointmentHeight + topPadding))) + bottomPadding;
    }
...
class _OpacityWidgetState extends State<_OpacityWidget> {
  ...
  void _update() {
    if (widget.opacity.value == 1 || widget.opacity.value == 0) {
      // setState(() {});
    }
  }
...
class _CalendarHeaderViewState extends State<_CalendarHeaderView> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color arrowColor = headerTextColor.withValues(alpha: headerTextColor.opacity * 0.6);
    ...
    if (!DateTimeHelper.canMoveToNextView(widget.view, widget.numberOfWeeksInView, widget.minDate, widget.maxDate, dates, widget.nonWorkingDays)) {
      nextArrowColor = nextArrowColor.withValues(alpha: nextArrowColor.opacity * 0.5);
    }
    ...
    if (!DateTimeHelper.canMoveToPreviousView(widget.view, widget.numberOfWeeksInView, widget.minDate, widget.maxDate, dates, widget.nonWorkingDays)) {
      prevArrowColor = prevArrowColor.withValues(alpha: prevArrowColor.opacity * 0.5);
    }
                        ...
                        ? <Widget>[
                            Flexible(
                              child: Text(
                                ...
                                softWrap: false,
                                textDirection: CalendarViewHelper.getTextDirectionBasedOnLocale(widget.locale),
                              ),
                            ),
                            Icon(widget.isPickerShown ? Icons.arrow_drop_up : Icons.arrow_drop_down, color: arrowColor, size: headerTextStyle.fontSize ?? 14),
                          ]
                        : <Widget>[
                            Flexible(
                              child: Text(
            ...
            child: Material(
              ...
              child: IntrinsicWidth(
                child: VisirButton(
                  ...
                  style: VisirButtonStyle(
                    clipBehavior: Clip.antiAlias,
                    backgroundColor: widget.showDatePickerButton && widget.isPickerShown ? Colors.grey.withValues(alpha: 0.3) : headerBackgroundColor,
        ...
        ? Container(
            ...
            child: Material(
              ...
              child: IntrinsicWidth(
                child: VisirButton(
                  type: VisirButtonAnimationType.scaleAndOpacity,
                  style: VisirButtonStyle(width: arrowWidth, height: headerHeight, alignment: Alignment.center, clipBehavior: Clip.antiAlias),
                  onTap: _backward,
                  child: Icon(
            ...
            child: Material(
              ...
              child: IntrinsicWidth(
                child: VisirButton(
                  type: VisirButtonAnimationType.scaleAndOpacity,
                  style: VisirButtonStyle(
                    clipBehavior: useMobilePlatformUI ? Clip.antiAlias : null,
                    height: useMobilePlatformUI ? headerHeight : null,
                    border: useMobilePlatformUI ? null : Border.all(color: widget.cellBorderColor ?? widget.calendarTheme.cellBorderColor!),
                    borderRadius: useMobilePlatformUI ? null : BorderRadius.circular(5.0),
                    width: todayIconWidth,
                    alignment: Alignment.center,
                  ),
                  onTap: () {
                    ...
                    widget.removePicker();
                    widget.controller.setProperties(displayDate: DateTime.now());
                  },
                  child: useMobilePlatformUI
                      ? Icon(Icons.today, color: style.color, size: style.fontSize)
                      : Text(
                          todayText,
    ...
    if (widget.headerStyle.textAlign == TextAlign.left || widget.headerStyle.textAlign == TextAlign.start) {
      if (widget.isMobilePlatform) {
        rowChildren = <Widget>[headerText, weekNumberWidget, todayIcon, calendarViewIcon, leftArrow, rightArrow];
      } else {
        rowChildren = <Widget>[leftArrow, rightArrow, headerText, weekNumberWidget, todayIcon, dividerWidget];
      ...
      if (widget.isMobilePlatform) {
        rowChildren = <Widget>[leftArrow, rightArrow, calendarViewIcon, todayIcon, weekNumberWidget, headerText];
      } else {
        useMobilePlatformUI ? rowChildren.add(calendarViewIcon) : rowChildren.addAll(children);
      ...
      if (widget.isMobilePlatform) {
        rowChildren = <Widget>[leftArrow, headerText, weekNumberWidget, todayIcon, dividerWidget, calendarViewIcon, rightArrow];
      } else {
        rowChildren = <Widget>[leftArrow, headerText, weekNumberWidget, rightArrow, todayIcon, dividerWidget];
        useMobilePlatformUI ? rowChildren.add(calendarViewIcon) : rowChildren.addAll(children);
      }
  ...
  Widget _getCalendarViewWidget(
    ...
    return Container(
      ...
      padding: EdgeInsets.all(useMobilePlatformUI ? 2 : 4),
      child: Material(
        color: isHighlighted && (isNeedIcon || useMobilePlatformUI) ? Colors.grey.withValues(alpha: 0.3) : headerBackgroundColor,
        child: IntrinsicWidth(
          child: VisirButton(
            type: VisirButtonAnimationType.scaleAndOpacity,
            style: VisirButtonStyle(
              ...
              height: height,
              alignment: Alignment.center,
              border: useMobilePlatformUI
                  ? null
                  : isNeedIcon
                  ? Border.all(color: highlightColor!)
                  : Border.all(color: allowedViewsColor!),
              borderRadius: useMobilePlatformUI ? null : BorderRadius.circular(5.0),
            ),
            onTap: () {
              if (!widget.enableInteraction) {
              ...
              if (isNeedIcon || useMobilePlatformUI) {
                ...
                widget.controller.setProperties(view: view);
              }
            },
            child: useMobilePlatformUI
                ? Icon(Icons.more_vert, color: style.color, size: style.fontSize)
                : (isNeedIcon
                      ? Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: <Widget>[
  ...
  String _getHeaderText() {
    ...
    final List<String> headerFormatString = headerDateFormat == null ? <String>[] : CalendarViewHelper.getListFromString(headerDateFormat);
      ...
      case CalendarView.schedule:
        {
          if (headerDateFormat != null) {
            return CalendarViewHelper.getLocalizedString(widget.valueChangeNotifier.value!, headerFormatString, widget.locale);
          }
          // ignore: lines_longer_than_80_chars
...
class _ScheduleLabelPainter extends CustomPainter {
  ...
  void _addDisplayDateLabel(Canvas canvas, Size size) {
    ...
    final TextSpan span = TextSpan(
      ...
      style: themeData.textTheme.bodyMedium!.merge(scheduleViewSettings.placeholderTextStyle),
    );
    ...
    _textPainter.paint(canvas, Offset(xPosition, 26));
    ...
    if (isDisplayDate && agendaViewNotifier.value != null && isSameDate(agendaViewNotifier.value!.hoveringDate, startDate)) {
      ...
      if (useMobilePlatformUI) {
        ...
        _backgroundPainter.color = Colors.transparent;
  ...
  void _addWeekLabel(Canvas canvas, Size size) {
    ...
    final String startDateFormat = scheduleViewSettings.weekHeaderSettings.startDateFormat ?? 'dd';
    ...
    final String month = DateFormat.MMM().format(startDate);
    ...
    final TextSpan span = TextSpan(
      text: month,
      style: themeData.textTheme.titleLarge!.copyWith(color: Colors.grey).merge(scheduleViewSettings.weekHeaderSettings.weekTextStyle),
      children: [
        TextSpan(
          text: '  $firstDate - $lastDate',
          style: themeData.textTheme.titleSmall!.copyWith(color: themeData.colorScheme.inverseSurface),
        ),
      ],
    );
    ...
    _textPainter.paint(canvas, Offset(xPosition, yPosition + 32));
  }
  ...
  List<CustomPainterSemantics> _getSemanticsBuilder(Size size) {
    ...
    semanticsBuilder.add(
      CustomPainterSemantics(
        ...
        properties: SemanticsProperties(label: accessibilityText, textDirection: TextDirection.ltr),
      ),
    );
...
class _ScheduleAppointmentView extends Stack {
  _ScheduleAppointmentView({required Widget content, required Widget header, AlignmentDirectional? alignment, Key? key})
    : super(
        key: key,
        children: <Widget>[
...
class _AppointmentViewHeaderRenderObject extends RenderStack {
  ...
  double _getHeaderOffset(double contentSize, double offset, double headerSize) {
...
class _CustomSplash extends InteractiveInkFeature {
  ...
  _CustomSplash({
    ...
    _radiusController = AnimationController(duration: _kUnconfirmedRippleSplashDuration, vsync: controller.vsync)
      ...
      ..forward();
    _radius = _radiusController.drive(Tween<double>(begin: 0.0, end: _targetRadius));
...
class _AgendaDateTimePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final TextStyle agendaDateThemeTextStyle = themeData.textTheme.bodyMedium!.copyWith(color: themeData.colorScheme.onSurface, fontSize: 18);
    if (monthViewSettings != null) {
      dayTextStyle = agendaDayThemeTextStyle.merge(calendarTheme.agendaDayTextStyle).merge(monthViewSettings!.agendaStyle.dayTextStyle);
      dateTextStyle = agendaDateThemeTextStyle.merge(calendarTheme.agendaDateTextStyle).merge(monthViewSettings!.agendaStyle.dateTextStyle);
    } else {
      if (useMobilePlatformUI) {
        dayTextStyle = agendaDayThemeTextStyle.merge(calendarTheme.agendaDayTextStyle).merge(scheduleViewSettings!.dayHeaderSettings.dayTextStyle);
        dateTextStyle = agendaDateThemeTextStyle.merge(calendarTheme.agendaDateTextStyle).merge(scheduleViewSettings!.dayHeaderSettings.dateTextStyle);
      } else {
  ...
  void _addDayLabelForMobile(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday, bool isMobile) {
    ...
    _textPainter.paint(canvas, Offset(((size.width - _textPainter.width) / 2), padding));
    ...
    final double xPosition = ((size.width - _textPainter.width) / 2);
  ...
  void _addDayLabelForWeb(Canvas canvas, Size size, double padding, TextStyle dayTextStyle, TextStyle dateTextStyle, bool isToday) {
    ...
    final String dayText = DateFormat(
      isRTL ? '${scheduleViewSettings!.dayHeaderSettings.dayFormat}, MMM' : 'MMM, ${scheduleViewSettings!.dayHeaderSettings.dayFormat}',
      locale,
    ...
    if (agendaDateNotifier.value != null && isSameDate(agendaDateNotifier.value!.hoveringDate, selectedDate)) {
      if (dateTextStartPosition < (isRTL ? size.width - agendaDateNotifier.value!.hoveringOffset.dx : agendaDateNotifier.value!.hoveringOffset.dx) &&
          (dateTextStartPosition + _textPainter.width) >
              (isRTL ? size.width - agendaDateNotifier.value!.hoveringOffset.dx : agendaDateNotifier.value!.hoveringOffset.dx) &&
          yPosition < agendaDateNotifier.value!.hoveringOffset.dy &&
          (yPosition + _textPainter.height) > agendaDateNotifier.value!.hoveringOffset.dy) {
        _linePainter.color = isToday
            ? Colors.black.withValues(alpha: 0.1)
  ...
  void _drawTodayCircle(Canvas canvas, double xPosition, double yPosition, double padding) {
    canvas.drawRRect(
      RRect.fromLTRBR(xPosition - 5.5, yPosition - 2, xPosition + (_textPainter.width) + 5.5, yPosition + (_textPainter.height) + 2, Radius.circular(4)),
      _linePainter,
    );
    // canvas.drawCircle(Offset(xPosition + (_textPainter.width / 2), yPosition + (_textPainter.height / 2)),
    //     _textPainter.width > _textPainter.height ? (_textPainter.width / 2) + padding : (_textPainter.height / 2) + padding, _linePainter);
  }
...
Size _getTextWidgetWidth(String text, double height, double width, BuildContext context, {TextStyle? style}) {
  ...
  renderObject!.layout(BoxConstraints(minWidth: width, maxWidth: width, minHeight: height, maxHeight: height));
...
Map<CalendarView, String> _getCalendarViewsText(SfLocalizations localizations) {
  ...
  calendarViews[CalendarView.twoDays] = localizations.allowedViewWeekLabel;
  calendarViews[CalendarView.threeDays] = localizations.allowedViewWeekLabel;
  calendarViews[CalendarView.fourDays] = localizations.allowedViewWeekLabel;
  calendarViews[CalendarView.fiveDays] = localizations.allowedViewWeekLabel;
  calendarViews[CalendarView.sixDays] = localizations.allowedViewWeekLabel;
...
double _getAgendaViewDayLabelWidth(ScheduleViewSettings scheduleViewSettings, bool useMobilePlatformUI) {
  if (scheduleViewSettings.dayHeaderSettings.width == -1) {
    return useMobilePlatformUI ? 44 : 150;
  }
