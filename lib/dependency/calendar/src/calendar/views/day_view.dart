  ...
  void _updateSpecialRegionDetails() {
    ...
    for (int i = 0; i < widget.specialRegion!.length; i++) {
      ...
      for (int j = startIndex; j <= endIndex; j++) {
        ...
        if ((startPosition <= 0 && endPosition <= 0) || (startPosition >= widget.height && endPosition >= widget.height) || (startPosition == endPosition)) {
          continue;
        }
...
class _TimeSlotRenderWidget extends MultiChildRenderObjectWidget {
  ...
  _TimeSlotRenderObject createRenderObject(BuildContext context) {
    return _TimeSlotRenderObject(visibleDates, horizontalLinesCount, timeIntervalHeight, timeLabelWidth, cellBorderColor, calendarTheme, themeData,
        timeSlotViewSettings, isRTL, specialRegion, calendarCellNotifier, textScaleFactor, width, height, specialRegionBounds, minDate, maxDate);
  }
...
class _TimeSlotRenderObject extends CustomCalendarRenderObject {
  ...
  void _drawDisabledDate(DateTime disabledStartDate, DateTime disabledEndDate, Canvas canvas, int visibleDatesCount) {
    ...
    final double startYPosition =
        CalendarViewHelper.getTimeToPosition(Duration(hours: disabledStartDate.hour, minutes: disabledStartDate.minute), timeSlotViewSettings, minuteHeight);
    final double endYPosition =
        CalendarViewHelper.getTimeToPosition(Duration(hours: disabledEndDate.hour, minutes: disabledEndDate.minute), timeSlotViewSettings, minuteHeight);
    for (int i = startIndex; i <= endIndex; i++) {
      final double topPosition = i == startIndex ? startYPosition : 0;
      final double bottomPosition = i == endIndex ? endYPosition : height;

      if ((topPosition <= 0 && bottomPosition <= 0) || (topPosition >= height && bottomPosition >= height) || (topPosition == bottomPosition)) {
        continue;
      }
  ...
  void _addMouseHoveringForTimeSlot(Canvas canvas, Size size) {
    ...
    //
    // canvas.drawRect(
    //     Rect.fromLTWH(left, top, left + _cellWidth == size.width ? _cellWidth - padding : _cellWidth, top + height == size.height ? height - padding : height),
    //     _linePainter);
  }
