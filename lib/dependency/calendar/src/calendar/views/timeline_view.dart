...
import 'package:Visir/dependency/calendar/src/calendar/sfcalendar.dart';
...
class _TimelineWidgetState extends State<TimelineWidget> {
  ...
  Widget build(BuildContext context) {
    if (_children.isEmpty && widget.timeRegionBuilder != null && _specialRegionViews.isNotEmpty) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        final Widget child = widget.timeRegionBuilder!(context, TimeRegionDetails(view.region.data, widget.visibleDates[view.visibleIndex], view.bound));
  ...
  void _updateSpecialRegionDetails() {
    ...
    for (int i = 0; i < widget.specialRegion!.length; i++) {
      ...
      double endXPosition =
          CalendarViewHelper.getTimeToPosition(Duration(hours: regionEndTime.hour, minutes: regionEndTime.minute), widget.timeSlotViewSettings, minuteHeight);
      if (endIndex == -1) {
      ...
      if ((startPosition <= 0 && endPosition <= 0) || (startPosition >= widget.width && endPosition >= widget.width) || (startPosition == endPosition)) {
        continue;
      }
  ...
  void _updateSpecialRegionRect(CalendarTimeRegion region, double startPosition, double endPosition, double topPosition, double bottomPosition, int index) {
    Rect rect;
    if (widget.isRTL) {
...
class _TimelineRenderObject extends CustomCalendarRenderObject {
  ...
  void performLayout() {
    ...
    for (int i = 0; i < count; i++) {
      ...
      child.layout(constraints.copyWith(minHeight: rect.height, maxHeight: rect.height, minWidth: rect.width, maxWidth: rect.width));
  ...
  void _minMaxExceeds(int visibleDatesCount, bool isTimelineMonth, DateTime minDate, DateTime maxDate, List<DateTime>? blackoutDates, Canvas canvas) {
  ...
  void _drawDisabledDate(
      DateTime disabledDate, bool isMaxDate, bool isBlackOutDate, Canvas canvas, bool isTimelineMonth, int timeInterval, int visibleDatesCount) {
    ...
    final double xPosition = isTimelineMonth
        ? 0
        : CalendarViewHelper.getTimeToPosition(Duration(hours: disabledDate.hour, minutes: disabledDate.minute), timeSlotViewSettings, minuteHeight);
  ...
  void _addMouseHovering(Canvas canvas, Size size, bool isResourceEnabled) {
    ...
    // }
    //
    // if ((size.width - scrollController.offset) < scrollController.position.viewportDimension && (left + timeIntervalWidth).round() == size.width.round()) {
    //   width -= padding;
    // }
    //
    // _linePainter.style = PaintingStyle.stroke;
    // _linePainter.strokeWidth = 2;
    // _linePainter.color = calendarTheme.selectionBorderColor!.withValues(alpha:0.4);
  ...
  void _addSpecialRegion(Canvas canvas, bool isResourceEnabled, bool isTimelineMonth) {
    ...
    _linePainter.style = PaintingStyle.fill;
    final int count = specialRegionBounds.length;
    final TextStyle defaultTextStyle = TextStyle(color: calendarTheme.brightness == Brightness.dark ? Colors.white54 : Colors.black45);
    for (int i = 0; i < count; i++) {
      final TimeRegionView view = specialRegionBounds[i];
      final CalendarTimeRegion region = view.region;
      _linePainter.color = region.color ?? Colors.grey.withValues(alpha: 0.2);
      final TextStyle textStyle = themeData.textTheme.bodyMedium!.copyWith(fontSize: 14).merge(region.textStyle ?? defaultTextStyle);
      final Rect rect = view.bound;
      canvas.drawRect(rect, _linePainter);
      ...
      if (region.iconData == null) {
        ...
        painter.text = TextSpan(text: String.fromCharCode(region.iconData!.codePoint), style: textStyle.copyWith(fontFamily: region.iconData!.fontFamily));
      }
  ...
  List<CustomPainterSemantics> _getAccessibilityDates(Size size, double top, double height, List<CustomPainterSemantics> semanticsBuilder,
...
class TimelineViewHeaderView extends CustomPainter {
  ...
  final double textScaleFactor;
  final double _padding = kSchedulePadding;
  double _xPosition = 0;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      if (isSameDate(currentDate, today)) {
        // dayTextStyle =
        //     todayTextStyle != null ? calendarTheme.todayTextStyle!.copyWith(fontSize: viewHeaderDayStyle.fontSize, color: todayTextColor) : viewHeaderDayStyle;
  ...
  void _drawTimelineTimeSlotsViewHeader(Canvas canvas, Size size, double childWidth, int index, int i) {
    ...
    if (isRTL) {
      _dateTextPainter.paint(canvas, Offset(size.width - _xPosition - _padding - _dateTextPainter.width, viewHeaderHeight / 2 - _dateTextPainter.height / 2));
      _dayTextPainter.paint(canvas,
          Offset(size.width - _xPosition - (_padding * 2) - _dayTextPainter.width - _dateTextPainter.width, viewHeaderHeight / 2 - _dayTextPainter.height / 2));
    } else {
      _dateTextPainter.paint(canvas, Offset(_padding + _xPosition, viewHeaderHeight / 2 - _dateTextPainter.height / 2));
      _dayTextPainter.paint(canvas, Offset(_dateTextPainter.width + _xPosition + (_padding * 2), viewHeaderHeight / 2 - _dayTextPainter.height / 2));
    }
  ...
  void _drawTimelineMonthViewHeader(Canvas canvas, double childWidth, Size size, bool isBlackoutDate) {
    ...
    final double startYPosition = (size.height - (_dayTextPainter.height > _dateTextPainter.height ? _dayTextPainter.height : _dateTextPainter.height)) / 2;
  ...
  void _addMouseHovering(Canvas canvas, Size size, [double? cellWidth]) {
    // double difference = 0;
    // if (isRTL && (size.width - timelineViewHeaderScrollController.offset) < timelineViewHeaderScrollController.position.viewportDimension) {
    //   difference = timelineViewHeaderScrollController.position.viewportDimension - size.width;
    // }
    // final double leftPosition =
    //     isRTL && cellWidth == null ? size.width - _xPosition - (_padding * 2) - _dayTextPainter.width - _dateTextPainter.width - _padding : _xPosition;
    ...
    //   _hoverPainter.color = (calendarTheme.brightness == Brightness.dark ? Colors.white : Colors.black87).withValues(alpha:0.04);
  ...
  bool shouldRepaint(TimelineViewHeaderView oldDelegate) {
    ...
    return oldWidget.visibleDates != visibleDates ||
        ...
        (isTimelineMonth &&
            (oldWidget.blackoutDatesTextStyle != blackoutDatesTextStyle || !CalendarViewHelper.isDateCollectionEqual(oldWidget.blackoutDates, blackoutDates)));
  }
