...
class _MonthViewWidgetState extends State<MonthViewWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.builder != null) {
      ...
      final bool showTrailingLeadingDates = CalendarViewHelper.isLeadingAndTrailingDatesVisible(widget.rowCount, widget.showTrailingAndLeadingDates);
      for (int i = 0; i < visibleDatesCount; i++) {
        ...
        final List<CalendarAppointment> appointments = AppointmentHelper.getSpecificDateVisibleAppointment(
          currentVisibleDate,
        ...
        final Widget child = widget.builder!(
          ...
          MonthCellDetails(
            ...
            Rect.fromLTWH(widget.isRTL ? widget.width - xPosition - cellWidth : xPosition, yPosition, cellWidth, cellHeight),
          ),
        );
...
class _MonthViewRenderObject extends CustomCalendarRenderObject {
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (!isNeedCustomPaint) {
      ...
      final bool showTrailingLeadingDates = CalendarViewHelper.isLeadingAndTrailingDatesVisible(rowCount, showTrailingAndLeadingDates);
      ...
      for (int i = 0; i < visibleDatesCount; i++) {
        ...
        if (currentVisibleDate.weekday == DateTime.monday) {
          ...
          if (weekNumberPanelWidth != 0 && (showTrailingLeadingDates || (!showTrailingLeadingDates && isCurrentMonthWeek))) {
        ...
        if (child.parentData != null) {
          (child.parentData! as CalendarParentData).offset = Offset(isRTL ? size.width - xPosition - cellWidth : xPosition, yPosition);
        }
        child = childAfter(child);

        if (calendarCellNotifier.value != null && !_blackoutDatesIndex.contains(i)) {
          _addMouseHovering(context.canvas, size, cellWidth, cellHeight, isRTL ? xPosition - weekNumberPanelWidth : xPosition, yPosition);
        }
  ...
  void _drawMonthCells(Canvas canvas, Size size) {
    const double viewPadding = kSchedulePadding;
    ...
    final Color nextMonthBackgroundColor = monthCellStyle.leadingDatesBackgroundColor ?? calendarTheme.leadingDatesBackgroundColor!;
    final Color previousMonthBackgroundColor = monthCellStyle.trailingDatesBackgroundColor ?? calendarTheme.trailingDatesBackgroundColor!;
    ...
    for (int i = 0; i < visibleDatesCount; i++) {
      ...
      if (currentVisibleDate.weekday == DateTime.monday) {
        ...
        /// Used to check the start and end date is current month date or not.
        final bool isCurrentMonthWeek = startDate.month == currentMonthDate.month || endDate.month == currentMonthDate.month;
      ...
      final TextSpan span = TextSpan(text: currentVisibleDate.day.toString(), style: textStyle);
      ...
      if (isCurrentDate) {
        ...
        canvas.drawCircle(Offset(xPosition + cellWidth / 2, yPosition + circlePadding + textHeight), textHeight + viewPadding, _linePainter);
      }
  ...
  void _addMouseHovering(Canvas canvas, Size size, double cellWidth, double cellHeight, double xPosition, double yPosition) {
  ...
  void _drawVerticalAndHorizontalLines(Canvas canvas, Size size, double yPosition, double xPosition, double cellHeight, double cellWidth) {
    yPosition = cellHeight;
    _linePainter.strokeWidth = linePadding;
    _linePainter.color = cellBorderColor ?? calendarTheme.cellBorderColor!;
      ...
      if (showTrailingLeadingDates || currentMonth == currentVisibleDate.month) {
        semanticsBuilder.add(
          CustomPainterSemantics(
            ...
            properties: SemanticsProperties(label: _getAccessibilityText(currentVisibleDate, i), textDirection: TextDirection.ltr),
          ),
        );
      }
