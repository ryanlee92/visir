...
import 'package:Visir/dependency/calendar/datepicker/datepicker.dart' show IterableDiagnostics;
...
abstract class CalendarDataSource<T extends Object?> extends CalendarDataSourceChangeNotifier {
  ...
  /// Cache for recurring appointments to avoid recalculation
  final Map<Object, List<CalendarAppointment>> _recurrenceCache = {};
  DateTime? _cacheStartDate;
  DateTime? _cacheEndDate;
  final Set<Object> _usedKeys = {};

  /// Resets the cache if the date range changes.
  void resetCache(DateTime start, DateTime end) {
    if (_cacheStartDate != start || _cacheEndDate != end) {
      _recurrenceCache.clear();
      _cacheStartDate = start;
      _cacheEndDate = end;
    }
    _usedKeys.clear();
  }

  /// Retrieves cached recurrence appointments for a given data object.
  List<CalendarAppointment>? getCachedRecurrence(Object data) {
    if (_recurrenceCache.containsKey(data)) {
      _usedKeys.add(data);
      // Return a copy to avoid external modification affecting the cache,
      // though deep copy might be too expensive.
      // For now, assuming the consumer won't mutate the list structure itself.
      // We might need to clone appointments if they are mutated.
      return _recurrenceCache[data];
    }
    return null;
  }

  /// Stores calculated recurrence appointments in the cache.
  void setCachedRecurrence(Object data, List<CalendarAppointment> appointments) {
    _recurrenceCache[data] = appointments;
    _usedKeys.add(data);
  }

  /// Cleans up unused cache entries to prevent memory leaks.
  void cleanCache() {
    _recurrenceCache.removeWhere((key, value) => !_usedKeys.contains(key));
  }
  ...
  List<Appointment> getVisibleAppointments(DateTime startDate, String calendarTimeZone, [DateTime? endDate]) {
    ...
    // Initialize/Reset cache for this view update
    resetCache(startDate, endDate);
  ...
  Object? getPatternAppointment(Object? occurrenceAppointment, String calendarTimeZone) {
    ...
    if ((occurrenceCalendarAppointment.recurrenceRule == null || occurrenceCalendarAppointment.recurrenceRule!.isEmpty) &&
    ...
    for (int i = 0; i < dataSourceAppointments.length; i++) {
      ...
      if ((dataSourceAppointment.id == occurrenceCalendarAppointment.recurrenceId) ||
          (occurrenceCalendarAppointment.recurrenceId == null && dataSourceAppointment.id == occurrenceCalendarAppointment.id)) {
