...
import 'package:Visir/dependency/calendar/src/calendar/settings/time_slot_view_settings.dart';
...
class AppointmentHelper {
  ...
  static bool isCalendarAppointment(CalendarDataSource dataSource) {
    if (dataSource.appointments == null || dataSource.appointments!.isEmpty || dataSource.appointments![0] is CalendarAppointment) {
      return true;
    }
  ...
  static bool canAddSpanIcon(
    ...
    switch (view) {
      ...
      case CalendarView.workWeek:
        {
          return getDifference(appStartTime, appEndTime).inDays <= 0 && appStartTime.day != appEndTime.day && appEndTime.hour != 0;
        }
  ...
  static TextSpan getRecurrenceIcon(Color color, double textSize, bool isRecurrenceAppointment) {
    ...
    return TextSpan(
      ...
      style: TextStyle(color: color, fontSize: textSize, fontFamily: recurrenceIconData.fontFamily),
    );
  }
  ...
  static String getSpanAppointmentText(CalendarAppointment appointment, DateTime date, SfLocalizations localization) {
    ...
    final String totalDays = (getDifference(exactStartTime, convertToEndTime(appointment.exactEndTime)).inDays + 1).toString();
  ...
  static TextSpan getSpanIcon(Color color, double textSize, bool isForward) {
    ...
    return TextSpan(
      text: isForward ? '\u00BB' : '\u00AB',
      style: TextStyle(color: color, fontSize: textSize * 2, fontFamily: 'Roboto'),
    );
  }

  /// Check and returns whether the forward icon can be added for the spanning
  /// appointment.
  static bool canAddForwardSpanIcon(DateTime appStartTime, DateTime appEndTime, DateTime viewStartDate, DateTime viewEndDate) {
  ...
  /// appointment.
  static bool canAddBackwardSpanIcon(DateTime appStartTime, DateTime appEndTime, DateTime viewStartDate, DateTime viewEndDate) {
  ...
  static List<CalendarAppointment> getSpecificDateVisibleAppointment(DateTime? date, List<CalendarAppointment>? visibleAppointments) {
  ...
  static List<CalendarAppointment> getSelectedDateAppointments(List<CalendarAppointment>? appointments, String? timeZone, DateTime? date) {
    final List<CalendarAppointment> appointmentCollection = <CalendarAppointment>[];
    ...
    for (int j = 0; j < count; j++) {
      ...
      appointment.actualStartTime = convertTimeToAppointmentTimeZone(appointment.startTime, appointment.startTimeZone, timeZone);
      appointment.actualEndTime = convertTimeToAppointmentTimeZone(appointment.endTime, appointment.endTimeZone, timeZone);
  ...
  static bool isAppointmentWithinVisibleDateRange(CalendarAppointment appointment, DateTime visibleStart, DateTime visibleEnd) {
    return isDateRangeWithinVisibleDateRange(appointment.actualStartTime, appointment.actualEndTime, visibleStart, visibleEnd);
  }

  /// Check the date range in between the visible date range.
  static bool isDateRangeWithinVisibleDateRange(DateTime startDate, DateTime endDate, DateTime visibleStart, DateTime visibleEnd) {
    if (startDate.isAfter(visibleStart)) {
      if (startDate.isBefore(visibleEnd)) {
        return true;
      }
    } else if (startDate.day == visibleStart.day && startDate.month == visibleStart.month && startDate.year == visibleStart.year) {
      return true;
  ...
  static bool _isAppointmentInVisibleDateRange(CalendarAppointment appointment, DateTime visibleStart, DateTime visibleEnd) {
    ...
    if ((appStartTime.isAfter(visibleStart) ||
            (appStartTime.day == visibleStart.day && appStartTime.month == visibleStart.month && appStartTime.year == visibleStart.year)) &&
        (appStartTime.isBefore(visibleEnd) ||
            (appStartTime.day == visibleEnd.day && appStartTime.month == visibleEnd.month && appStartTime.year == visibleEnd.year)) &&
        (appEndTime.isAfter(visibleStart) ||
            (appEndTime.day == visibleStart.day && appEndTime.month == visibleStart.month && appEndTime.year == visibleStart.year)) &&
        (appEndTime.isBefore(visibleEnd) || (appEndTime.day == visibleEnd.day && appEndTime.month == visibleEnd.month && appEndTime.year == visibleEnd.year))) {
      return true;
    }
  ...
  static bool _isAppointmentDateWithinVisibleDateRange(DateTime appointmentDate, DateTime visibleStart, DateTime visibleEnd) {
  ...
  static double timeToPosition(SfCalendar calendar, DateTime date, double timeIntervalHeight) {
    final double singleIntervalHeightForAnHour = (60 / 15) * timeIntervalHeight / 4;

    final double startHour = calendar.timeSlotViewSettings.startHour;
    return ((date.hour + (date.minute / 60) + (date.second / 3600)) * singleIntervalHeightForAnHour) - (startHour * singleIntervalHeightForAnHour);
  }

  /// Returns the appointment height from the duration passed.
  static double getAppointmentHeightFromDuration(Duration? minimumDuration, SfCalendar calendar, double timeIntervalHeight) {
    if (minimumDuration == null || minimumDuration.inMinutes <= 0) {
      return 0;
    }

    final double hourHeight = (60 / CalendarViewHelper.getTimeInterval(calendar.timeSlotViewSettings)) * timeIntervalHeight;
  ...
  static bool _isIntersectingAppointmentInDayView(
    ...
    int minimumAppointmentMinutes = timeSlotViewSettings.minimumAppointmentDuration != null ? timeSlotViewSettings.minimumAppointmentDuration!.inMinutes : 0;
    ...
    double dateTitleHeightThreshold =
        ...
        2;
    double titleHeightThreshold = (context.bodyMedium!.height! * context.textScaler.scale(context.bodyMedium!.fontSize!)) + 2;
    final appointmentHeight = timeIntervalHeight / 60 * getDifference(appointmentStartTime, appointmentEndTime).inMinutes;

    if (appointmentHeight > dateTitleHeightThreshold) {
      heightThreshold = dateTitleHeightThreshold;
    } else {
      heightThreshold = titleHeightThreshold;
    }

    final timeThreshold = (heightThreshold / timeIntervalHeight * 60).floor();

    if (minimumAppointmentMinutes > 0 && !isTimelineMonth) {
      final int timeIntervalMinutes = timeSlotViewSettings.timeInterval.inMinutes;
      minimumAppointmentMinutes = minimumAppointmentMinutes > timeIntervalMinutes ? timeIntervalMinutes : minimumAppointmentMinutes;
    ...
    if (currentAppointmentStartTime.isBefore(appointmentStartTime.add(Duration(minutes: timeThreshold))) &&
        currentAppointmentStartTime.isAfter(appointmentStartTime)) {
      return true;
    }

    if (appointmentStartTime.isBefore(currentAppointmentStartTime.add(Duration(minutes: timeThreshold))) &&
        appointmentStartTime.isAfter(currentAppointmentStartTime)) {
      return true;
    }

    // if (currentAppointmentEndTime.isAfter(appointmentEndTime) && currentAppointmentStartTime.isBefore(appointmentStartTime)) {
    //   return true;
    // }
    ...
    if (isTimelineMonth) {
      return isSameDate(currentApp.actualStartTime, appointment.actualStartTime) || isSameDate(currentApp.actualEndTime, appointment.actualEndTime);
    }
  ...
  static int sortAppintment(CalendarAppointment? app1, CalendarAppointment? app2) {
    final event1 = (app1?.data as TaskEntity?);
    final event2 = (app2?.data as TaskEntity?);
    if (event1 == null || event2 == null) {
      if (event1 != null) return 1;
      if (event2 != null) return -1;
      return 0;
    }

    if (event1.isEvent != event2.isEvent) {
      if (event1.isEvent) {
        return event2.isDone || event2.isCancelled ? -1 : 1;
      } else {
        return event1.isDone || event1.isCancelled ? 1 : -1;
      }
    }

    if (!event1.isEvent && !event2.isEvent) {
      final statusSortValue = event1.status.sortValue.compareTo(event2.status.sortValue);
      if (statusSortValue != 0) return statusSortValue;

      final aProject = Utils.ref.read(projectListControllerProvider).where((element) => element.isPointedProjectId(event1.projectId)).firstOrNull;
      final bProject = Utils.ref.read(projectListControllerProvider).where((element) => element.isPointedProjectId(event2.projectId)).firstOrNull;
      final sortedProject = Utils.ref.read(projectListControllerProvider).sortedProjectWithDepth;
      ...
      final projectSorter = aProjectIndex < 0 || bProjectIndex < 0
          ? (event1.projectId ?? '').compareTo(event2.projectId ?? '')
          : aProjectIndex.compareTo(bProjectIndex);
      if (projectSorter != 0) return projectSorter;
    }

    if (app1!.actualStartTime.compareTo(app2!.actualStartTime) == 0) {
      if (event1.isEvent != event2.isEvent) {
        return event1.isEvent ? -1 : 1;
      }

      if (!event1.isEvent) {
        if (event1.updatedAt != null || event2.updatedAt != null) {
          return (event2.updatedAt ?? event2.createdAt ?? DateTime.now()).compareTo(event1.updatedAt ?? event1.createdAt ?? DateTime.now());
        }

        return (event1.createdAt ?? DateTime.now()).compareTo(event2.createdAt ?? DateTime.now());
      }

      final diff1 = app1.actualStartTime.difference(app1.actualEndTime).inDays;
      final diff2 = app2.actualStartTime.difference(app2.actualEndTime).inDays;

      if (diff1 == diff2) {
        if (event1.calendarId == event2.calendarId) {
          return (event1.eventId).compareTo(event2.eventId);
        }
        return event1.calendarId.compareTo(event2.calendarId);
      }
      return diff1 - diff2;
    }
  ...
  static AppointmentView _getAppointmentView(CalendarAppointment appointment, List<AppointmentView> appointmentCollection, int? resourceIndex) {
  ...
  static void setAppointmentPositionAndMaxPosition(
    ...
    final List<CalendarAppointment> normalAppointments = visibleAppointments
        .where((CalendarAppointment app) => _iterateAppointment(app, isTimeline, isAllDay))
        .toList()
        .where((appointment) {
          if (appointment.data is TaskEntity) {
            final task = appointment.data as TaskEntity;
            return !task.isCancelled;
          }
          return true;
        })
        .toList();
    normalAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => app1.actualStartTime.compareTo(app2.actualStartTime));
    if (!isTimeline) {
      normalAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => _orderAppointmentsDescending(app1.isSpanned, app2.isSpanned));
      normalAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => _orderAppointmentsDescending(app1.isAllDay, app2.isAllDay));
      ...
      normalAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));
      normalAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) => orderAppointmentsAscending(app1.isSpanned, app2.isSpanned));
      normalAppointments.sort((CalendarAppointment app1, CalendarAppointment app2) {
        return AppointmentHelper.sortAppintment(app1, app2);
      });
    }
    ...
    final int timeIntervalMinutes = CalendarViewHelper.getTimeInterval(timeSlotViewSettings);
    for (int count = 0; count < normalAppointments.length; count++) {
      ...
      if ((view == CalendarView.workWeek || view == CalendarView.timelineWorkWeek) &&
          timeSlotViewSettings.nonWorkingDays.contains(currentAppointment.actualStartTime.weekday) &&
          timeSlotViewSettings.nonWorkingDays.contains(currentAppointment.actualEndTime.weekday)) {
        continue;
      }

      List<AppointmentView>? intersectingApps;
      final AppointmentView currentAppView = _getAppointmentView(currentAppointment, appointmentCollection, resourceIndex);
  ...
  static DateTime convertTimeToAppointmentTimeZone(DateTime date, String? appTimeZoneId, String? calendarTimeZoneId) {
    if (((appTimeZoneId == null || appTimeZoneId == '') && (calendarTimeZoneId == null || calendarTimeZoneId == '')) || calendarTimeZoneId == appTimeZoneId) {
      return date;
    }
  ...
  static List<CalendarAppointment> getVisibleAppointments(
    ...
    for (int j = 0; j < count; j++) {
      ...
      calendarAppointment.actualEndTime = convertTimeToAppointmentTimeZone(calendarAppointment.endTime, calendarAppointment.endTimeZone, calendarTimeZone);
      ...
      if (calendarAppointment.recurrenceRule != null && calendarAppointment.recurrenceRule != '') {
        ...
        List<CalendarAppointment>? cached;
        if (calendarAppointment.data != null) {
          cached = dataSource?.getCachedRecurrence(calendarAppointment.data!);
        }

        if (cached != null) {
          for (var cachedApp in cached) {
            // We need a way to clone.
            // Since I am editing this file, I can just change _copy to public `copy`
            // or add a `copy` method that calls `_copy`.
            tempAppointments.add(_copy(cachedApp));
          }
        } else {
          _getRecurrenceAppointments(calendarAppointment, tempAppointments, startDate, endDate, calendarTimeZone);
          // Store in cache. We should store a COPY of the generated appointments
          // because the ones in tempAppointments will be modified below.
          // So we need to clone them BEFORE they are modified.

          if (calendarAppointment.data != null) {
            List<CalendarAppointment> toCache = [];
            for (var app in tempAppointments) {
              toCache.add(_copy(app));
            }
            dataSource?.setCachedRecurrence(calendarAppointment.data!, toCache);
          }
        }
      } else {
          ...
          if (canCreateNewAppointment &&
            ...
            for (int i = 0; i < 2; i++) {
              ...
              if (i == 0) {
                ...
                spannedAppointment.actualStartTime = DateTime(appointment.exactEndTime.year, appointment.exactEndTime.month, appointment.exactEndTime.day);
              }

              spannedAppointment.startTime = spannedAppointment.isAllDay
                  ? appointment.actualStartTime
                  : convertTimeToAppointmentTimeZone(appointment.actualStartTime, calendarTimeZone, appointment.startTimeZone);
              spannedAppointment.endTime = spannedAppointment.isAllDay
                  ? appointment.actualEndTime
                  : convertTimeToAppointmentTimeZone(appointment.actualEndTime, calendarTimeZone, appointment.endTimeZone);
  ...
  static CalendarAppointment _cloneRecurrenceAppointment(CalendarAppointment appointment, DateTime recursiveDate, String? calendarTimeZone) {
    ...
    occurrenceAppointment.startTime = occurrenceAppointment.isAllDay
        ...
        : convertTimeToAppointmentTimeZone(occurrenceAppointment.actualStartTime, calendarTimeZone, occurrenceAppointment.startTimeZone);

    final int minutes = getDifference(appointment.actualStartTime, appointment.actualEndTime).inMinutes;
    occurrenceAppointment.actualEndTime = DateTimeHelper.getDateTimeValue(addDuration(occurrenceAppointment.actualStartTime, Duration(minutes: minutes)));
    occurrenceAppointment.endTime = occurrenceAppointment.isAllDay
        ? occurrenceAppointment.actualEndTime
        : convertTimeToAppointmentTimeZone(occurrenceAppointment.actualEndTime, calendarTimeZone, occurrenceAppointment.endTimeZone);
    occurrenceAppointment.isSpanned =
        _isSpanned(occurrenceAppointment) && getDifference(occurrenceAppointment.startTime, occurrenceAppointment.endTime).inDays > 0;
  ...
  /// collection.
  static List<CalendarAppointment> generateCalendarAppointments(CalendarDataSource? calendarData, String? calendarTimeZone, [List<dynamic>? appointments]) {
    ...
    if (dataSource.isNotEmpty && dataSource[0] is CalendarAppointment) {
      for (int i = 0; i < dataSource.length; i++) {
        ...
        item.data = item;
        item.actualStartTime = !item.isAllDay ? convertTimeToAppointmentTimeZone(item.startTime, item.startTimeZone, calendarTimeZone) : item.startTime;
        item.actualEndTime = !item.isAllDay ? convertTimeToAppointmentTimeZone(item.endTime, item.endTimeZone, calendarTimeZone) : item.endTime;
  ...
  static CalendarAppointment _createAppointment(Object appointmentObject, CalendarDataSource calendarData, String? calendarTimeZone) {
    ...
    app.actualStartTime = !app.isAllDay ? convertTimeToAppointmentTimeZone(app.startTime, app.startTimeZone, calendarTimeZone) : app.startTime;
    app.actualEndTime = !app.isAllDay ? convertTimeToAppointmentTimeZone(app.endTime, app.endTimeZone, calendarTimeZone) : app.endTime;
  ...
  static void _getRecurrenceAppointments(
    ...
    // final startTime = appStartTime.isAfter(visibleStartDate) ? appStartTime : visibleStartDate;
    // final List<DateTime> recursiveDates = rrule
    //     .getAllInstances(
    //         start: visibleStartDate.toUtc(), after: DateUtils.dateOnly(startTime).toUtc(), includeBefore: true, before: visibleEndDate.toUtc(), includeAfter: true)
    //     .map((e) => e.add(Duration(hours: appStartTime.hour, minutes: appStartTime.minute)))
    //     .toList();
    ...
    //   recDates =
    //       RecurrenceHelper.getRecurrenceDateTimeCollection(countRule, appointment.actualStartTime, specificStartDate: appointment.startTime, maxCount: 1);
    // }
    ...
    for (int j = 0; j < recursiveDates.length; j++) {
      ...
      if (appointment.recurrenceExceptionDates != null) {
        ...
        for (int i = 0; i < appointment.recurrenceExceptionDates!.length; i++) {
          final DateTime date = convertTimeToAppointmentTimeZone(appointment.recurrenceExceptionDates![i], '', scheduleTimeZone);
      ...
      final CalendarAppointment occurrenceAppointment = _cloneRecurrenceAppointment(appointment, recursiveDate, scheduleTimeZone);
