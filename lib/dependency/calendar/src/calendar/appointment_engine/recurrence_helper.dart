...
class RecurrenceHelper {
  ...
  static bool _isRecurrenceInBetweenSpecificRange(DateTime appointmentDate, Duration duration, DateTime visibleStartDate, DateTime visibleEndTime) {
  ...
  static List<DateTime> _getDailyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
    ...
    if (rRule.contains('UNTIL')) {
      ...
      if (isSpecificDateRange) {
        final DateTime startTime = DateTime(endDate.year, endDate.month, endDate.day, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      ...
      endDate = AppointmentHelper.addDaysWithTime(
          recurrenceStartDate, (recurrenceCount - 1) * dailyDayGap, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      ...
      if (isSpecificDateRange && specificStartDate.isAfter(recurrenceEndDate) && !isSameDate(specificStartDate, recurrenceEndDate)) {
        return recDateCollection;
      }
    ...
    if (isSpecificDateRange && recurrenceStartDate.isBefore(specificStartDate)) {
      final DateTime recurrenceInitialDate = DateTime(recurrenceStartDate.year, recurrenceStartDate.month, recurrenceStartDate.day);
      final DateTime visibleInitialDate = DateTime(specificStartDate.year, specificStartDate.month, specificStartDate.day);
      ...
      if (dayDifference == 0) {
        addDate =
            DateTime(specificStartDate.year, specificStartDate.month, specificStartDate.day, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      } else {
        ...
        addDate = AppointmentHelper.addDaysWithTime(visibleInitialDate, -dayDifference, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      }

      final DateTime recurrenceEndDate = addDate.add(recurrenceDuration);
      if (incrementCount > 0 && !isSameDate(addDate, recurrenceEndDate)) {
        final int durationDifference = recurrenceEndDate.hour > addDate.hour ? recurrenceDuration.inDays : recurrenceDuration.inDays + 1;
        final int intervalCount = ((durationDifference ~/ dailyDayGap) * dailyDayGap) + (durationDifference % dailyDayGap == 0 ? 0 : dailyDayGap);
        addDate = AppointmentHelper.addDaysWithTime(addDate, -intervalCount, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
    ...
    while (recurrenceIncrementCount < recurrenceCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
      if (isSpecificDateRange) {
      ...
      addDate = AppointmentHelper.addDaysWithTime(addDate, dailyDayGap, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
  ...
  static List<DateTime> _getWeeklyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
    ...
    List<String> weeklyRule = rRule.split(weeklySeparator);
    List<String> weeklyByDayRules = _findWeeklyRule(weeklyRule);
    int weeklyByDayPos = weeklyByDayRules.isNotEmpty ? int.parse(weeklyByDayRules[1]) : -1;

    if (weeklyByDayPos == -1) {
      // return recDateCollection;
      weeklyRule.add('BYDAY=${DateFormat(DateFormat.ABBR_WEEKDAY).format(recurrenceStartDate).substring(0, 2).toUpperCase()}');
      weeklyByDayRules = _findWeeklyRule(weeklyRule);
      weeklyByDayPos = weeklyByDayRules.isNotEmpty ? int.parse(weeklyByDayRules[1]) : -1;
    }
    ...
    if (rRule.contains('UNTIL')) {
      ...
      endDate = AppointmentHelper.addDaysWithTime(recurrenceStartDate, totalDays, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
    ...
    if (isSpecificDateRange && recurrenceStartDate.isBefore(specificStartDate)) {
      ...
      addDate = AppointmentHelper.addDaysWithTime(
          addDate,
          (totalWeeks * DateTime.daysPerWeek * weeklyWeekGap) + (isOccurrenceInInitialWeek ? daysDifference : DateTime.daysPerWeek - weekDay) + initialWeekGap,
          recurrenceStartHour,
    ...
    /// dates.
    while ((tempCount < recurrenceCount && isWeeklySelected) || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
      final bool isRecurrenceDate = weekDays.contains(addDate.weekday % DateTime.daysPerWeek);
      if (isSpecificDateRange) {
        if (_isRecurrenceInBetweenSpecificRange(addDate, recurrenceDuration, specificStartDate, specificEndDate) && isRecurrenceDate) {
          recDateCollection.add(addDate);
        }
      ...
      addDate = addDate.weekday == DateTime.saturday
          ? AppointmentHelper.addDaysWithTime(
              addDate, ((weeklyWeekGap - 1) * DateTime.daysPerWeek) + 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond)
          : AppointmentHelper.addDaysWithTime(addDate, 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
    }
  ...
  static List<DateTime> _getMonthlyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
    ...
    if (isSpecificDateRange && !rRule.contains('COUNT')) {
      ...
      if (addedDateYear < viewStartDateYear || (viewStartDateMonth >= addedDateMonth && viewStartDateYear == addedDateYear)) {
    ...
    if (byMonthDay == 'BYMONTHDAY') {
      ...
      final DateTime temp = DateTime(addDate.year, addDate.month, monthDate, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      ...
      if (temp.day == monthDate && (temp.isAfter(recurrenceStartDate) || isSameDate(temp, recurrenceStartDate))) {
        ...
        addDate = DateTime(addDate.year, addDate.month + monthlyMonthGap, 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        final DateTime tempDate = DateTime(addDate.year, addDate.month, monthDate, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      ...
      while (tempCount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
        if (addDate.day != monthDate) {
          ...
          addDate = DateTime(yearValue, monthValue, monthDate, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
          continue;
        }
        ...
        monthValue += monthlyMonthGap;
        monthDate = byMonthDay == -1 ? AppointmentHelper.getMonthEndDate(DateTime(yearValue, monthValue)).day : monthDate;
        addDate = DateTime(yearValue, monthValue, monthDate, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      ...
      final int bySetPosValue = bySetPosCount.isNotEmpty ? int.parse(bySetPosCount) : 0;
      final int? nthWeekNumber = int.tryParse(byDayValue.replaceAll(new RegExp(r'[^0-9]'), ''));

      void updateValidDate() {
        final DateTime monthStart = DateTime(addDate.year, addDate.month, 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        final int monthStartWeekday = monthStart.weekday % DateTime.daysPerWeek;
        final DateTime weekStartDate =
            AppointmentHelper.addDaysWithTime(monthStart, -monthStartWeekday, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        int nthWeek = nthWeekNumber ?? bySetPosValue;
        if (monthStartWeekday <= nthWeekDay) {
          nthWeek = (nthWeekNumber ?? bySetPosValue) - 1;
        }
        ...
        if (bySetPosValue.isNegative) {
          ...
          addDate = AppointmentHelper.addDaysWithTime(
              weekStartDate, (nthWeek * DateTime.daysPerWeek) + nthWeekDay, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        }
      }
  ...
  static List<DateTime> _getYearlyRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
    ...
    if (byMonthDay == 'BYMONTHDAY') {
      ...
      final int daysInMonth = DateTimeHelper.getDateTimeValue(addDays(DateTime(addDate.year, addDate.month + 1), -1)).day;
      if (daysInMonth < dayIndex) {
        return recDateCollection;
      }

      final DateTime specificDate = DateTime(addDate.year, monthIndex, dayIndex, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      if (specificDate.isBefore(recurrenceStartDate)) {
        addDate = DateTime(
            specificDate.year + yearlyYearGap, specificDate.month, specificDate.day, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
      } else {
      ...
      while (tempCount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
        ...
        dayIndex = byMonthDay == -1 ? AppointmentHelper.getMonthEndDate(DateTime(addDate.year + yearlyYearGap, monthIndex)).day : addDate.day;

        addDate = DateTime(addDate.year + yearlyYearGap, addDate.month, dayIndex, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        tempCount++;
      }
    } else if (byDay == 'BYDAY') {
      int tempCount = 0;
      ...
      void updateValidNextDate() {
        while (true) {
          DateTime monthStart = DateTime(addDate.year, monthIndex, 1, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
          final int monthStartWeekday = monthStart.weekday % DateTime.daysPerWeek;
          final DateTime weekStartDate =
              AppointmentHelper.addDaysWithTime(monthStart, -monthStartWeekday, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
          int nthWeek = nthWeekNumber == null ? bySetPosValue : nthWeekNumber;
          if (monthStartWeekday <= nthWeekDay) {
          ...
          if (monthStart.month != monthIndex || monthStart.isBefore(recurrenceStartDate)) {
            addDate =
                DateTime(monthStart.year + yearlyYearGap, monthStart.month, monthStart.day, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
            continue;
          }
      ...
      while (tempCount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
        ...
        addDate = DateTime(addDate.year + yearlyYearGap, addDate.month, addDate.day, recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
  ...
  static List<DateTime> getRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
    ...
    if (rRule.contains('DAILY')) {
      return _getDailyRecurrenceDateTimeCollection(rRule, recurrenceStartDate,
          recurrenceDuration: recurrenceDuration, specificStartDate: specificStartDate, specificEndDate: specificEndDate);
  ...
  static String _getRRuleForDaily(
      int recCount, RecurrenceProperties recurrenceProperties, DateTime startDate, DateTime? endDate, bool isValidRecurrence, Duration diffTimeSpan) {
    String rRule = '';
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
        ...
        (recurrenceProperties.recurrenceRange == RecurrenceRange.endDate && (startDate.isBefore(endDate!) || startDate == endDate))) {
      rRule = 'FREQ=DAILY';
  ...
  static String _getRRuleForWeekly(DateTime startDate, DateTime? endDate, int recCount, RecurrenceProperties recurrenceProperties, bool isValidRecurrence,
      Duration diffTimeSpan, DateTime prevDate) {
    String rRule = '';
    ...
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      while ((count < recCount && isValidRecurrence && recurrenceProperties.weekDays.isNotEmpty) ||
          (recurrenceProperties.recurrenceRange == RecurrenceRange.endDate && (addDate.isBefore(endDate!) || isSameDate(addDate, endDate))) ||
        ...
        addDate = addDate.weekday == DateTime.saturday
            ? AppointmentHelper.addDaysWithTime(
                addDate, ((recurrenceProperties.interval - 1) * DateTime.daysPerWeek) + 1, startDate.hour, startDate.minute, startDate.second)
  ...
  static String _getRRuleForMonthly(
    ...
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      if (AppointmentHelper.getDifference(startDate,
              DateTime(startDate.year, startDate.month + recurrenceProperties.interval, startDate.day, startDate.hour, startDate.minute, startDate.second)) <
          diffTimeSpan) {
        isValidRecurrence = false;
      }
    }
  ...
  static String _getRRuleForYearly(
      int recCount, RecurrenceProperties recurrenceProperties, DateTime startDate, DateTime? endDate, Duration diffTimeSpan, bool isValidRecurrence) {
    String rRule = '';
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
        recurrenceProperties.recurrenceRange == RecurrenceRange.noEndDate ||
        (recurrenceProperties.recurrenceRange == RecurrenceRange.endDate && (startDate.isBefore(endDate!) || startDate == endDate))) {
      ...
      if (AppointmentHelper.getDifference(startDate,
              DateTime(startDate.year + recurrenceProperties.interval, startDate.month, startDate.day, startDate.hour, startDate.minute, startDate.second)) <
          diffTimeSpan) {
        isValidRecurrence = false;
      }
    }
  ...
  static String generateRRule(RecurrenceProperties recurrenceProperties, DateTime appStartTime, DateTime appEndTime) {
    ...
    switch (recurrenceProperties.recurrenceType) {
      ...
      case RecurrenceType.weekly:
        return _getRRuleForWeekly(startDate, endDate, recCount, recurrenceProperties, isValidRecurrence, diffTimeSpan, prevDate);
  ...
  static int _getWeekDay(String weekDay) {
    int index = 1;
    weekDay = weekDay.replaceAll(new RegExp(r"[0-9]+"), "");
    final DateTime firstDate = DateTimeHelper.getDateTimeValue(addDays(DateTime.now(), -(DateTime.now().weekday - 1)));
    final List<String> dayNames = List<int>.generate(DateTime.daysPerWeek, (int index) => index)
        .map((int value) => DateFormat(DateFormat.ABBR_WEEKDAY).format(addDays(firstDate, value)))
        .toList();
    for (int i = 0; i < DateTime.daysPerWeek; i++) {
      ...
      /// char count.
      if (dayName.toUpperCase() == weekDay || weekDay.length == 2 && dayName.substring(0, dayName.length - 1).toUpperCase() == weekDay) {
        index = i;
      }
    }
  ...
  static DateTime getUntilEndDate(String untilString) {
    ...
    if (untilString.contains('T')) {
      ...
      final DateTime endDate = untilString.contains('Z') ? DateTime.parse(untilString).toLocal() : DateTime.parse(untilString);
      return endDate;
    } else {
  ...
  static DateTime _getRecurrenceDateForNegativeValue(int bySetPosCount, DateTime date, int weekDay) {
    ...
    return _getLastWeekDay(DateTime(lastDate.year, lastDate.month, lastDate.day, date.hour, date.minute, date.second), weekDay);
  }
