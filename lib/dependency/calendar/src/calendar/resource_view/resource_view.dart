    ...
    if (widget.resourceViewHeaderBuilder != null) {
      ...
      for (int i = 0; i < resourceLength; i++) {
        ...
        final Widget child = widget.resourceViewHeaderBuilder!(
            context, ResourceViewHeaderDetails(currentResource, Rect.fromLTWH(0, yPosition, widget.width, widget.resourceItemHeight)));
...
class _ResourceViewRenderObjectWidget extends MultiChildRenderObjectWidget {
  const _ResourceViewRenderObjectWidget(this.resources, this.resourceViewSettings, this.resourceItemHeight, this.cellBorderColor, this.calendarTheme,
      this.themeData, this.notifier, this.isRTL, this.textScaleFactor, this.mouseHoverPosition, this.imagePainterCollection, this.width, this.panelHeight,
  ...
  _ResourceViewRenderObject createRenderObject(BuildContext context) {
    return _ResourceViewRenderObject(resources, resourceViewSettings, resourceItemHeight, cellBorderColor, calendarTheme, themeData, notifier, isRTL,
        textScaleFactor, mouseHoverPosition, imagePainterCollection, width, panelHeight);
  }
...
class _ResourceViewRenderObject extends CustomCalendarRenderObject {
  _ResourceViewRenderObject(this._resources, this._resourceViewSettings, this._resourceItemHeight, this._cellBorderColor, this._calendarTheme, this._themeData,
      this._notifier, this._isRTL, this._textScaleFactor, this._mouseHoverPosition, this._imagePainterCollection, this._width, this._panelHeight);
  ...
  void performLayout() {
    ...
    size = Size(widgetSize.width.isInfinite ? width : widgetSize.width, widgetSize.height.isInfinite ? panelHeight : widgetSize.height);

    for (dynamic child = firstChild; child != null; child = childAfter(child)) {
      child.layout(constraints.copyWith(minWidth: width, minHeight: resourceItemHeight, maxWidth: width, maxHeight: resourceItemHeight));
    }
  }
    ...
    if (!isNeedCustomPaint) {
      ...
      for (int i = 0; i < resourceLength; i++) {
        ...
        if (mouseHoverPosition != null) {
          final Color resourceHoveringColor = (calendarTheme.brightness == Brightness.dark ? Colors.white : Colors.black87).withValues(alpha: 0.04);
  ...
  void _drawResourceBorder(CalendarResource resource, Canvas canvas, Size size, double actualItemHeight, double yPosition, double radius) {
  ...
  void _drawDisplayName(
      CalendarResource resource, TextStyle displayNameTextStyle, Canvas canvas, Size size, double yPosition, double actualItemHeight, double radius) {
  ...
  void _drawImage(Canvas canvas, Size size, CalendarResource resource, double innerCircleYPosition, double innerCircleXPosition, double innerCircleWidth,
  ...
  DecorationImagePainter? _getImagePainter(CalendarResource resource) {
    ...
    } else if (imagePainterCollection.containsKey(resource.id) && !imagePainterCollection[resource.id].toString().contains(resource.image.toString())) {
  ...
  void _drawInnerCircle(CalendarResource resource, Canvas canvas, Size size, double actualItemWidth, double actualItemHeight, double yPosition) {
    ...
    if (resource.image != null) {
      _drawImage(canvas, size, resource, innerCircleYPosition, innerCircleXPosition, innerCircleWidth, innerCircleHeight);
      return;
    }
    ...
    final String shortName = splitName.length > 1 ? splitName[0].substring(0, 1) + splitName[1].substring(0, 1) : splitName[0].substring(0, 1);
    final TextSpan span =
        TextSpan(text: shortName, style: themeData.textTheme.bodyLarge!.copyWith(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w500));
    _updateNamePainter(span);
