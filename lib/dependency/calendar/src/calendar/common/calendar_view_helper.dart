...
const double kAllDayAppointmentHeight = 21;
...
class CalendarViewHelper {
  ...
  static double getScheduleAppointmentHeight(MonthViewSettings? monthViewSettings, ScheduleViewSettings? scheduleViewSettings) {
    return monthViewSettings != null
        ? (monthViewSettings.agendaItemHeight == -1 ? 50 : monthViewSettings.agendaItemHeight)
        : (scheduleViewSettings == null || scheduleViewSettings.appointmentItemHeight == -1 ? 38 : scheduleViewSettings.appointmentItemHeight);
  }

  /// Return schedule view all day appointment height and its value based on
  /// schedule view settings and month view settings.
  static double getScheduleAllDayAppointmentHeight(MonthViewSettings? monthViewSettings, ScheduleViewSettings? scheduleViewSettings) {
    return monthViewSettings != null
        ? (monthViewSettings.agendaItemHeight == -1 ? 25 : monthViewSettings.agendaItemHeight)
        : (scheduleViewSettings == null || scheduleViewSettings.appointmentItemHeight == -1 ? 24 : scheduleViewSettings.appointmentItemHeight);
  }

  /// Returns the height for an resource item to render the resource within
  /// it in the resource panel.
  static double getResourceItemHeight(double resourceViewSize, double timelineViewHeight, ResourceViewSettings resourceViewSettings, int resourceCount) {
  ...
  static bool isResourceEnabled(CalendarDataSource? dataSource, CalendarView view) {
    return isTimelineView(view) && dataSource != null && dataSource.resources != null && dataSource.resources!.isNotEmpty;
  }
  ...
  static String getAppointmentSemanticsText(CalendarAppointment appointment) {
    ...
    } else if (appointment.isSpanned || AppointmentHelper.getDifference(appointment.startTime, appointment.endTime).inDays > 0) {
  ...
  /// style.
  static Color? getTodayHighlightTextColor(Color? todayHighlightColor, TextStyle? todayTextStyle, SfCalendarThemeData calendarTheme) {
  ...
  static double getTimeToPosition(Duration duration, TimeSlotViewSettings timeSlotViewSettings, double minuteHeight) {
    ...
    final Duration startDuration = Duration(hours: startHour, minutes: ((timeSlotViewSettings.startHour - startHour) * 60).toInt());
  ...
  static int getTimeInterval(TimeSlotViewSettings settings) {
    ...
    if (timeIntervalMinutes >= 0 && timeIntervalMinutes <= totalMinutes && totalMinutes.round() % timeIntervalMinutes == 0) {
  ...
  static double getCellEndPadding(double cellWidth, bool isMonth) {
    if (isMonth) return 0;
    return cellWidth > 100 ? 10 : 0;
  ...
  static void raiseCalendarTapCallback(
    ...
    calendar.onTap!(CalendarTapDetails(appointments, date, element, resource, position, localOffset, appointmentRect));
  }
  ...
  static void raiseCalendarLongPressCallback(
    ...
    calendar.onLongPress!(CalendarLongPressDetails(appointments, date, element, resource, position, localOffset));
  }
  ...
  /// parameters.
  static void raiseAppointmentResizeStartCallback(SfCalendar calendar, dynamic appointment, CalendarResource? resource) {
  ...
  static void raiseAppointmentResizeUpdateCallback(
    ...
    calendar.onAppointmentResizeUpdate!(AppointmentResizeUpdateDetails(appointment, resource, resizingTime, resizingOffset));
  }

  /// Method  that raises the appointment resize end callback with the given
  /// parameters.
  static void raiseAppointmentResizeEndCallback(SfCalendar calendar, dynamic appointment, CalendarResource? resource, DateTime? startTime, DateTime? endTime) {
  ...
  static KeyEventResult handleViewSwitchKeyBoardEvent(KeyEvent event, CalendarController controller, List<CalendarView>? allowedViews) {
    ...
    // controller.setProperties(view: view);
  ...
  static bool isDraggingAppointmentHasDisabledCell(
    ...
    if ((isMonthView && (!isDateWithInDateRange(minDate, maxDate, appStartTime) || !isDateWithInDateRange(minDate, maxDate, appEndTime))) ||
        (!isMonthView &&
    ...
    if (!isMonthView) {
      for (int i = 0; i < timeRegions.length; i++) {
        ...
        if (!region.enablePointerInteraction &&
            (isSameOrBeforeDateTime(appEndTime, region.actualStartTime) && isSameOrAfterDateTime(appStartTime, region.actualEndTime))) {
          if (resourceIndex != -1 &&
...
class UpdateCalendarStateDetails {
  ...
  List<AppointmentView> regularAppointmentViewCollection = <AppointmentView>[];
...
class CalendarAppointment {
  ...
  @override
  // ignore: avoid_equals_and_hash_code_on_mutable_classes, non_nullable_equals_parameter
...
abstract class CustomCalendarRenderObject extends RenderBox with ContainerRenderObjectMixin<RenderBox, CalendarParentData> {
  @override
