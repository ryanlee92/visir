...
class _CustomDraggableFloatState extends State<DraggableFloatWidget> with TickerProviderStateMixin<DraggableFloatWidget> {
  ...
  _initBorderSize() {
    ...
    _top = widget.config.borderTop + (widget.config.isFullScreen && widget.config.borderTopContainTopBar ? _variableTop : 0);
    _bottom =
        _screenHeight -
        widget.height -
        widget.config.borderBottom -
        _bottomBarHeight -
        (widget.config.isFullScreen ? 0 : _variableTop); // The bottom bar height of the full screen needs to be subtracted.
    ...
    positionY = widget.config.initPositionYInTop ? _top + widget.config.initPositionYMarginBorder : _bottom - widget.config.initPositionYMarginBorder;
    // The sum of the width and the border cannot exceed the screen width,
    // otherwise unexpected errors will occur.
    assert(
      widget.width + widget.config.borderLeft + widget.config.borderRight < _screenWidth,
    ...
    assert(
      widget.height + widget.config.borderTop + widget.config.borderBottom + _variableTop + _bottomBarHeight < _screenHeight,
  ...
  _initEventSubscription() {
    ...
    eventSubscription = widget.eventStreamController!.stream.listen((event) {
      ...
      // will not be handled.
      if (currentState == DraggableFloatWidgetState.DRAG_IN_PROGRESS || currentState == DraggableFloatWidgetState.AUTO_ATTACH_IN_PROGRESS) return;
      // 2. Handle the event.
  ...
  _initAnimation() {
    ...
    animationController = AnimationController(duration: widget.config.animDuration, vsync: this);
    ...
    animation.addStatusListener((status) {
      ...
      var _tempAnimEndState = currentState == DraggableFloatWidgetState.ANIM_HIDE_IN_PROGRESS ? DraggableFloatWidgetState.HIDE : DraggableFloatWidgetState.SHOW;
      ...
      var _tempNextState = _event == OperateEvent.OPERATE_HIDE ? DraggableFloatWidgetState.HIDE : DraggableFloatWidgetState.SHOW;
  ...
  _handleAnimValueChange() {
    ...
    if (currentState == DraggableFloatWidgetState.AUTO_ATTACH_IN_PROGRESS) {
      double _tempX = animStartPx < animEndPx ? animStartPx + _tempAbsX * _value : animEndPx + _tempAbsX * (1 - _value);
    ...
    if (currentState == DraggableFloatWidgetState.ANIM_SHOW_IN_PROGRESS) {
      positionX = animEndPx <= _halfScreenW ? animStartPx + _tempAbsX * _value : animEndPx + _tempAbsX * (1 - _value);
      _safeSetState();
      return;
    }
    ...
    if (currentState == DraggableFloatWidgetState.ANIM_HIDE_IN_PROGRESS) {
      positionX = animEndPx <= _halfScreenW ? animEndPx + _tempAbsX * (1 - _value) : animStartPx + _tempAbsX * _value;
      _safeSetState();
      return;
    }
  }
  ...
  _buildDragWidget() => Container(
    ...
    child: GestureDetector(onTap: () => widget.onTap?.call(), child: widget.child),
  );
  ...
  _handleReceivedEvent(OperateEvent event) {
    ...
    if (currentState == DraggableFloatWidgetState.ANIM_SHOW_IN_PROGRESS || currentState == DraggableFloatWidgetState.ANIM_HIDE_IN_PROGRESS) {
  ...
  _handleScrollEvent(OperateEvent event) {
    ...
    if ((currentState == DraggableFloatWidgetState.SHOW && event == OperateEvent.OPERATE_SHOW) ||
        (currentState == DraggableFloatWidgetState.HIDE && event == OperateEvent.OPERATE_HIDE))
      return;
    ...
    if (event == OperateEvent.OPERATE_HIDE) {
      animEndPx = positionX <= _halfScreenW ? widget.config.exposedPartWidthWhenHidden - widget.width : _screenWidth - widget.config.exposedPartWidthWhenHidden;
    } else {
      animEndPx = positionX <= _halfScreenW ? _left : _right;
    }
    animEndPy = positionY;
    // 3. Start animation.
    if (currentState == DraggableFloatWidgetState.SHOW && event == OperateEvent.OPERATE_HIDE) {
      _startAnimation(DraggableFloatWidgetState.ANIM_HIDE_IN_PROGRESS);
    } else if (currentState == DraggableFloatWidgetState.HIDE && event == OperateEvent.OPERATE_SHOW) {
