...
class TaskWidgetProvider : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == "com.wavetogether.fillin.home_widget.TOUCH") {
            ...
            if (uri?.host == "toggletaskstatus") {
                ...
                val startAtMs = uri.getQueryParameter("startAtMs")?.toLongOrNull()
                val endAtMs = uri.getQueryParameter("endAtMs")?.toLongOrNull()
                if (taskId != null) {
                    // 위젯 데이터 업데이트
    ...
    internal fun updateAppWidget(
        ...
        val views = RemoteViews(context.packageName, R.layout.task_widget)
            .apply {
                ...
                val backgroundIntent = HomeWidgetLaunchIntent.getActivity(
                    context,
                    MainActivity::class.java,
                    Uri.parse("com.wavetogether.fillin://switchtab?tab=task")
                )
        ...
        if (userEmail.isNullOrEmpty()) {
            ...
            views.setViewVisibility(R.id.task_list_container, android.view.View.GONE)
            appWidgetManager.updateAppWidget(appWidgetId, views)
            return
        }
        
        views.setViewVisibility(R.id.no_login_text, android.view.View.GONE)
        views.setViewVisibility(R.id.task_list_container, android.view.View.VISIBLE)
        ...
        val intent = Intent(context, VisirWidgetAppointmentListView::class.java).apply {
            ...
            putExtra("isTaskWidget", true)
            data = Uri.parse(toUri(Intent.URI_INTENT_SCHEME))
        }
        views.setRemoteAdapter(R.id.appointment_list, intent)
        val pendingIntentWithData = WidgetUtils.getPendingIntent(context, TaskWidgetProvider::class.java)
